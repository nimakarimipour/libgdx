...
public class ParticleSystem {
      ...
      if (m_internalAllocatedCapacity < capacity) {
        ...
        m_accumulationBuffer =
            BufferUtils.reallocateBuffer(
                m_accumulationBuffer, 0, m_internalAllocatedCapacity, capacity, false);
        m_accumulation2Buffer =
            BufferUtils.reallocateBuffer(
                Vec2.class, m_accumulation2Buffer, 0, m_internalAllocatedCapacity, capacity, true);
        m_depthBuffer =
            BufferUtils.reallocateBuffer(
  ...
  public void addContact(int a, int b) {
    ...
    if (d2 < m_squaredDiameter) {
      if (m_contactCount >= m_contactCapacity) {
        ...
        m_contactBuffer =
            BufferUtils.reallocateBuffer(
                ParticleContact.class, m_contactBuffer, oldCapacity, newCapacity);
  ...
  void solvePressure(TimeStep step) {
    ...
    for (int i = 0; i < m_count; i++) {
      ...
      float h =
          pressurePerWeight
              * MathUtils.max(
                  0.0f, MathUtils.min(w, Settings.maxParticleWeight) - Settings.minParticleWeight);
      ...
      if ((group.m_groupFlags & ParticleGroupType.b2_rigidParticleGroup) != 0) {
        ...
        for (int i = group.m_firstIndex; i < group.m_lastIndex; i++) {
          Transform.mulToOutUnsafe(
              velocityTransform, m_positionBuffer.data[i], m_velocityBuffer.data[i]);
        }
      }
    }
  }
  ...
  // reallocate a buffer
  static <T> T[] reallocateBuffer(
      ParticleBuffer<T> buffer, int oldCapacity, int newCapacity, boolean deferred) {
  ...
  static int[] reallocateBuffer(
      ParticleBufferInt buffer, int oldCapacity, int newCapacity, boolean deferred) {
    assert (newCapacity > oldCapacity);
    return BufferUtils.reallocateBuffer(
        buffer.data, buffer.userSuppliedCapacity, oldCapacity, newCapacity, deferred);
  }
      ...
      if (dabx * dabx + daby * daby < maxDistanceSquared
        ...
        if (system.m_triadCount >= system.m_triadCapacity) {
          ...
          system.m_triadBuffer =
              BufferUtils.reallocateBuffer(
                  Triad.class, system.m_triadBuffer, oldCapacity, newCapacity);
  ...
  static class DestroyParticlesInShapeCallback implements ParticleQueryCallback {
    ...
    public void init(
        ParticleSystem system, Shape shape, Transform xf, boolean callDestructionListener) {
      this.system = system;
      this.shape = shape;
      this.xf = xf;
      this.destroyed = 0;
