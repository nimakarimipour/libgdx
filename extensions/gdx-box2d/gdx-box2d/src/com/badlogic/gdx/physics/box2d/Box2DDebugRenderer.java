...
public class Box2DDebugRenderer implements Disposable {
  ...
  private Color getColorByBody(Body body) {
    if (body.isActive() == false) return SHAPE_NOT_ACTIVE;
    else if (body.getType() == BodyType.StaticBody) return SHAPE_STATIC;
    else if (body.getType() == BodyType.KinematicBody) return SHAPE_KINEMATIC;
    else if (body.isAwake() == false) return SHAPE_NOT_AWAKE;
    else return SHAPE_AWAKE;
  }
  ...
  private void drawSolidCircle(Vector2 center, float radius, Vector2 axis, Color color) {
    ...
    for (int i = 0; i < 20; i++, angle += angleInc) {
      v.set(
          (float) Math.cos(angle) * radius + center.x, (float) Math.sin(angle) * radius + center.y);
      if (i == 0) {
        lv.set(v);
        f.set(v);
        continue;
