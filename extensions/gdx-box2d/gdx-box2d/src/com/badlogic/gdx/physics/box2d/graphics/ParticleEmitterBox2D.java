...
/**
 * @author kalle_h
 *     <p>ParticleEmitterBox2D use box2d rayCast:ing to achieve continuous collision detection
 *     against box2d fixtures. If particle detect collision it change it's direction before actual
 *     collision would occur. Velocity is 100% reflected.
 *     <p>These particles does not have any other physical attributes or functionality. Particles
 *     can't collide to other particles.
 */
public class ParticleEmitterBox2D extends ParticleEmitter {
  ...
  float normalAngle;
  /**
   * If velocities squared is shorter than this it could lead 0 length rayCast that cause c++
   * assertion at box2d
   */
  ...
  final RayCastCallback rayCallBack =
      new RayCastCallback() {
        public float reportRayFixture(
          ...
          ParticleEmitterBox2D.this.normalAngle =
              MathUtils.atan2(normal.y, normal.x) * MathUtils.radiansToDegrees;
          return fraction;
        }
      };

  /**
   * Constructs default ParticleEmitterBox2D. Box2d World is used for rayCasting. Assumes that
  ...
  /**
   * /**Constructs ParticleEmitterBox2D using bufferedReader. Box2d World is used for rayCasting.
  ...
  /**
   * Constructs ParticleEmitterBox2D fully copying given emitter attributes. Box2d World is used for
  ...
  private class ParticleBox2D extends Particle {
    ...
    /**
     * translate particle given amount. Continuous collision detection achieved by using RayCast
    ...
    public void translate(float velocityX, float velocityY) {
      /**
       * If velocities squares summed is shorter than Epsilon it could lead ~0 length rayCast that
       * cause nasty c++ assertion inside box2d. This is so short distance that moving particle has
       * no effect so this return early.
       */
