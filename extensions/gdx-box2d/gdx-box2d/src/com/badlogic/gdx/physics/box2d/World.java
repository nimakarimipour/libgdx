...
public final class World implements Disposable {
  ...
  public World(Vector2 gravity, boolean doSleep) {
    ...
    for (int i = 0; i < contactAddrs.length; i++) freeContacts.add(new Contact(this, 0));
  }
   ...
   * filter is used (b2_defaultFilter). The listener is owned by you and must remain in scope.
   */
  ...
  /**
   * Create a rigid body given a definition. No reference to the definition is retained. Bodies
   * created by this method are pooled internally by the World object. They will be freed upon
   * calling {@link World#destroyBody(Body)}
   *
   * @see Pool
  ...
  private native long jniCreateBody(
      long addr,
      int type,
      float positionX,
      float positionY,
  ...
  public Joint createJoint(JointDef def) {
    ...
    if (def.type == JointType.GearJoint)
      joint =
          new GearJoint(this, jointAddr, ((GearJointDef) def).joint1, ((GearJointDef) def).joint2);
  ...
  private native long jniCreateWheelJoint(
      long addr,
      long bodyA,
      long bodyB,
  ...
  private native long jniCreateRopeJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreateDistanceJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreateFrictionJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreateGearJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreateMotorJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreateMouseJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreatePrismaticJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreatePulleyJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreateRevoluteJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native long jniCreateWeldJoint(
      long addr,
      long bodyA,
      long bodyB,
      boolean collideConnected,
  ...
  private native void jniStep(
      long addr, float timeStep, int velocityIterations, int positionIterations); /*
		b2World* world = (b2World*)addr;
		CustomContactFilter contactFilter(env, object);
		CustomContactListener contactListener(env,object);
		world->SetContactFilter(&contactFilter);
   ...
   * after each call to Step. The default behavior is modified by calling SetAutoClearForces. The
   * purpose of this function is to support sub-stepping. Sub-stepping is often used to maintain a
   * fixed sized time step under a variable frame-rate. When you perform sub-stepping you will
   * disable auto clearing of forces and instead call ClearForces after all sub-steps are complete
   * in one pass of your game loop. {@link #setAutoClearForces(boolean)}
   */
  ...
  public void QueryAABB(
      QueryCallback callback, float lowerX, float lowerY, float upperX, float upperY) {
  ...
  /**
   * Returns the list of {@link Contact} instances produced by the last call to {@link #step(float,
   * int, int)}. Note that the returned list will have O(1) access times when using indexing.
   * contacts are created and destroyed in the middle of a time step. Use {@link ContactListener} to
   * avoid missing contacts
   *
  ...
  /**
   * Ray-cast the world for all fixtures in the path of the ray. The ray-cast ignores shapes that
   ...
   */
  public void rayCast(
      RayCastCallback callback, float point1X, float point1Y, float point2X, float point2Y) {
  ...
  private float reportRayFixture(
      long addr, float pX, float pY, float nX, float nY, float fraction) {
