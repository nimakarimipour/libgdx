...
/**
 * Generates {@link BitmapFont} and {@link BitmapFontData} instances from TrueType, OTF, and other
 * FreeType supported fonts. Usage example:
 *
 * <pre>
...
public class FreeTypeFontGenerator implements Disposable {
  ...
  /**
   * The maximum texture size allowed by generateData, when storing in a texture atlas. Multiple
   * texture pages will be created if necessary. Default is 1024.
   *
  ...
  /**
   * Creates a new generator from the given font file. Uses {@link FileHandle#length()} to determine
   * the file size. If the file length could not be determined (it was 0), an extra copy of the font
   * bytes is performed. Throws a {@link GdxRuntimeException} if loading did not succeed.
   */
   ...
   * GdxRuntimeException if the font could not be generated. Using big sizes might cause such an
   * exception.
   *
  ...
  public BitmapFont generateFont(FreeTypeFontParameter parameter, FreeTypeBitmapFontData data) {
    ...
    if (updateTextureRegions)
      parameter.packer.updateTextureRegions(
          data.regions, parameter.minFilter, parameter.magFilter, parameter.genMipMaps);
  ...
  /**
   * Called by generateFont to create a new {@link BitmapFont} instance. This allows injecting a
   * customized {@link BitmapFont}, eg for a RTL font.
   *
   * @see BitmapFont#BitmapFont(BitmapFontData, Array, boolean)
   */
  protected BitmapFont newBitmapFont(
      BitmapFontData data, Array<TextureRegion> pageRegions, boolean integer) {
    return new BitmapFont(data, pageRegions, integer);
  }

  /**
   * Uses ascender and descender of font to calculate real height that makes all glyphs to fit in
   * given pixel size. Source: http://nothings.org/stb/stb_truetype.h / stbtt_ScaleForPixelHeight
   */
  ...
  /**
   * Uses max advance, ascender and descender of font to calculate real height that makes any n
   * glyphs to fit in given pixel width.
   *
  ...
  /**
   * Uses max advance, ascender and descender of font to calculate real height that makes any n
   * glyphs to fit in given pixel width and height.
   *
   * @param width the max width to fit (in pixels)
   ...
   * with various space characters, then {@link GlyphAndBitmap#bitmap} will be null.
   */
  public @Null GlyphAndBitmap generateGlyphAndBitmap(int c, int size, boolean flip) {
    ...
    glyph.yoffset =
        flip ? -slot.getBitmapTop() + baseline : -(glyph.height - slot.getBitmapTop()) - baseline;
   ...
   * @param parameter configures how the font is generated
   */
  public FreeTypeBitmapFontData generateData(
      FreeTypeFontParameter parameter, FreeTypeBitmapFontData data) {
    ...
    if (packer == null) {
      ...
      if (incremental) {
        ...
        size =
            MathUtils.nextPowerOfTwo(
                (int) Math.sqrt(maxGlyphHeight * maxGlyphHeight * charactersLength));
    ...
    for (int i = 0; i < charactersLength; i++) {
      ...
      int height =
          loadChar(c, flags) ? FreeType.toInt(face.getGlyph().getMetrics().getHeight()) : 0;
      heights[i] = height;
    ...
    if (parameter.kerning) {
      for (int i = 0; i < charactersLength; i++) {
        ...
        for (int ii = i; ii < charactersLength; ii++) {
          ...
          int kerning =
              face.getKerning(
                  firstIndex, secondIndex, 0); // FT_KERNING_DEFAULT (scaled then rounded).
          if (kerning != 0) first.setKerning(secondChar, FreeType.toInt(kerning));

          kerning =
              face.getKerning(
                  secondIndex, firstIndex, 0); // FT_KERNING_DEFAULT (scaled then rounded).
  ...
  protected @Null Glyph createGlyph(
      char c,
    ...
    try {
      mainGlyph.toBitmap(
          parameter.mono ? FreeType.FT_RENDER_MODE_MONO : FreeType.FT_RENDER_MODE_NORMAL);
    ...
    if (mainBitmap.getWidth() != 0 && mainBitmap.getRows() != 0) {
      ...
      if (parameter.borderWidth > 0) {
        ...
        Pixmap borderPixmap =
            borderBitmap.getPixmap(Format.RGBA8888, parameter.borderColor, parameter.borderGamma);
      ...
      if (parameter.padTop > 0
          || parameter.padLeft > 0
          || parameter.padBottom > 0
          || parameter.padRight > 0) {
        Pixmap padPixmap =
            new Pixmap(
                mainPixmap.getWidth() + parameter.padLeft + parameter.padRight,
    ...
    glyph.xadvance =
        FreeType.toInt(metrics.getHoriAdvance()) + (int) parameter.borderWidth + parameter.spaceX;
  ...
  /**
   * Sets the maximum size that will be used when generating texture atlases for glyphs with
   * <tt>generateData()</tt>. The default is 1024. By specifying {@link #NO_MAXIMUM}, the texture
   * atlas will scale as needed.
   *
   * <p>The power-of-two square texture size will be capped to the given <tt>texSize</tt>. It's
   * recommended that a power-of-two value be used here.
   *
   * <p>Multiple pages may be used to fit all the generated glyphs. You can query the resulting
   * number of pages by calling <tt>bitmapFont.getRegions().length</tt> or
   * <tt>freeTypeBitmapFontData.getTextureRegions().length</tt>.
   *
   * <p>If PixmapPacker is specified when calling generateData, this parameter is ignored.
   *
  ...
  /**
   * Returns the maximum texture size that will be used by generateData() when creating a texture
   * atlas for the glyphs.
   *
   ...
   * texture storing the glyphs is held in memory, thus the {@link #getImagePaths()} and {@link
   * #getFontFile()} methods will return null.
   *
   * @author mzechner
   * @author Nathan Sweet
   */
  public static class FreeTypeBitmapFontData extends BitmapFontData implements Disposable {
    ...
    public void getGlyphs(GlyphRun run, CharSequence str, int start, int end, Glyph lastGlyph) {
      if (packer != null)
        packer.setPackToTexture(
            true); // All glyphs added after this are packed directly to the texture.
      super.getGlyphs(run, str, start, end, lastGlyph);
      if (dirty) {
        dirty = false;
        packer.updateTextureRegions(
            regions, parameter.minFilter, parameter.magFilter, parameter.genMipMaps);
      }
    }
  ...
  public static enum Hinting {
    ...
    Full,
    /**
     * Light hinting with fuzzy edges, but close to the original shape. Uses the FreeType
     * auto-hinter.
     */
    AutoSlight,
    ...
    AutoMedium,
    /**
     * Strong hinting with crisp edges at the expense of shape fidelity. Uses the FreeType
     * auto-hinter.
     */
    AutoFull,
  }
  ...
  /**
   * Parameter container class that helps configure how {@link FreeTypeBitmapFontData} and {@link
   * BitmapFont} instances are generated.
   *
   * <p>The packer field is for advanced usage, where it is necessary to pack multiple BitmapFonts
   ...
   * and the resulting {@link FreeTypeBitmapFontData} will have a valid {@link TextureRegion} which
  ...
  public static class FreeTypeFontParameter {
    ...
    /**
     * Number of times to render the glyph. Useful with a shadow or border, so it doesn't show
     * through the glyph.
     */
    ...
    /**
     * Shadow color; only used if shadowOffset > 0. If alpha component is 0, no shadow is drawn but
    ...
    /**
     * The characters the font should contain. If '\0' is not included then {@link
     * BitmapFontData#missingGlyph} is not set.
     */
    ...
    /**
     * When true, glyphs are rendered on the fly to the font's glyph page textures as they are
     * needed. The FreeTypeFontGenerator must not be disposed until the font is no longer needed.
     * The FreeTypeBitmapFontData must be disposed (separately from the generator) when the font is
     * no longer needed. The FreeTypeFontParameter should not be modified after creating a font. If
     * a PixmapPacker is not specified, the font glyph page textures will use {@link
