...
public class FreeType {
  ...
  public static class Library extends Pointer implements Disposable {
    ...
    public void dispose() {
      ...
      for (ByteBuffer buffer : fontData.values()) {
        if (BufferUtils.isUnsafeByteBuffer(buffer)) BufferUtils.disposeUnsafeByteBuffer(buffer);
      }
    }
    ...
    public Face newMemoryFace(ByteBuffer buffer, int faceIndex) {
      ...
      if (face == 0) {
        ...
        throw new GdxRuntimeException(
            "Couldn't load font, FreeType error code: " + getLastErrorCode());
      } else {
    ...
    private static native long newMemoryFace(
        long library, ByteBuffer data, int dataSize, int faceIndex); /*
			FT_Face face = 0;
  ...
  public static class Face extends Pointer implements Disposable {
    ...
    private static native boolean setCharSize(
    ...
    private static native int getKerning(
  ...
  public static class GlyphSlot extends Pointer {
    ...
    public Glyph getGlyph() {
      ...
      if (glyph == 0)
        throw new GdxRuntimeException(
            "Couldn't get glyph, FreeType error code: " + getLastErrorCode());
  ...
  public static class Glyph extends Pointer implements Disposable {
    ...
    public void toBitmap(int renderMode) {
      ...
      if (bitmap == 0)
        throw new GdxRuntimeException(
            "Couldn't render glyph, FreeType error code: " + getLastErrorCode());
      address = bitmap;
      rendered = true;
    }
        ...
        if (pixelMode == FT_PIXEL_MODE_MONO) {
          ...
          for (int y = 0; y < rows; y++) {
            ...
            for (int x = 0; x < width; x++) {
              ...
              if (alpha == 0) dstRow[x] = rgb;
              else if (alpha == 255) dstRow[x] = rgb | a;
              else
                dstRow[x] =
                    rgb | (int) (a * (float) Math.pow(alpha / 255f, gamma)); // Inverse gamma.
            }
            dst.put(dstRow);
          }
        }
  ...
  public static class Stroker extends Pointer implements Disposable {
    ...
    private static native void set(
        long stroker, int radius, int lineCap, int lineJoin, int miterLimit); /*
  ...
  //		String chars =
  // "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890\"!`?'.,;:()[]{}<>|/@\\^$-%+=#_&~*�?�?�?�?�? ¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿À�?ÂÃÄÅÆÇÈÉÊËÌ�?Î�?�?ÑÒÓÔÕÖ×ØÙÚÛÜ�?Þßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ";
  //
  ...
  //		System.out.println(toInt(faceMetrics.getAscender()) + ", " + toInt(faceMetrics.getDescender())
  // + ", " + toInt(faceMetrics.getHeight()));
  //
  ...
  //			System.out.println(toInt(glyphMetrics.getHoriBearingX()) + ", " +
  // toInt(glyphMetrics.getHoriBearingY()));
  //			System.out.println(toInt(glyphMetrics.getWidth()) + ", " + toInt(glyphMetrics.getHeight()) +
  // ", " + toInt(glyphMetrics.getHoriAdvance()));
  //			System.out.println(bitmap.getWidth() + ", " + bitmap.getRows() + ", " + bitmap.getPitch() +
  // ", " + bitmap.getNumGray());
