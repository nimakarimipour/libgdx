...
/**
 * Contains extra information that can only be calculated after a Tiled Map's tile set images are
 * loaded.
 *
 * @author David Fraska
 */
public class TileSetLayout {
   ...
   * original tile set images before being processed by {@link TiledMapPacker} (the ones actually
   * read by Tiled).
   *
   * @param tileset the tile set to process
   * @param baseDir the directory in which the tile set image is stored
   */
  protected TileSetLayout(int firstgid, TiledMapTileSet tileset, FileHandle baseDir)
      throws IOException {
    ...
    image =
        ImageIO.read(
            baseDir.child(tileset.getProperties().get("imagesource", String.class)).read());
