...
public class ShadowEffect implements ConfigurableEffect {
  ...
  public void draw(BufferedImage image, Graphics2D g, UnicodeFont unicodeFont, Glyph glyph) {
    ...
    g.setColor(
        new Color(color.getRed(), color.getGreen(), color.getBlue(), Math.round(opacity * 255)));
    ...
    for (Iterator iter = unicodeFont.getEffects().iterator(); iter.hasNext(); ) {
      ...
      if (effect instanceof OutlineEffect) {
        ...
        g.setComposite(
            AlphaComposite.Src); // Prevent shadow and outline shadow alpha from combining.
  ...
  /**
   * Sets the pixels to offset the shadow on the x axis. The glyphs will need padding so the shadow
   * doesn't get clipped.
   */
  ...
  /**
   * Sets the pixels to offset the shadow on the y axis. The glyphs will need padding so the shadow
   * doesn't get clipped.
   */
  ...
  public List getValues() {
    ...
    values.add(
        EffectUtil.floatValue(
            "Opacity", opacity, 0, 1, "This setting sets the translucency of the shadow."));
    values.add(
        EffectUtil.floatValue(
            "X distance",
            xDistance,
