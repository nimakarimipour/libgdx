 ...
 *
 * <p>Thanks to the Apache FOP project for their inspiring work!
 *
 * @author Nathan Sweet
 */
public class Kerning {
  ...
  public void load(InputStream inputStream, int fontSize) throws IOException {
    ...
    // By reading the 'kern' table last, it takes precedence over the 'GPOS' table. We are more
    // likely to interpret
    // the GPOS table incorrectly because we ignore most of it, since BMFont doesn't support its
    // features.
    if (gposOffset != -1) {
   ...
   *     glyph codes: the high 16 bits form the first glyph code, and the low 16 bits form the
   *     second.
   */
  ...
  private void readPairPositioningAdjustmentFormat2(int subTablePosition) throws IOException {
    ...
    input.seek(position);
    IntArray[] glyphsByClass1 =
        readClassDefinition(subTablePosition + classDefOffset1, class1Count);
    IntArray[] glyphsByClass2 =
        readClassDefinition(subTablePosition + classDefOffset2, class2Count);
    input.seek(position);
