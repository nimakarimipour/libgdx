...
// BOZO - Look at actual pixels to determine glyph size, current size sometimes selects blank pixels
// (eg Calibri, 45, 'o').

/**
 * A bitmap font that can display unicode glyphs from a TrueTypeFont.
 *
 * <p>For efficiency, glyphs are packed on to textures. Glyphs can be loaded to the textures on the
 * fly, when they are first needed for display. However, it is best to load the glyphs that are
 * known to be needed at startup.
 *
 * @author Nathan Sweet
 */
public class UnicodeFont {
  ...
  private void initializeFont(Font baseFont, int size, boolean bold, boolean italic) {
    ...
    attributes.put(
        TextAttribute.WEIGHT, bold ? TextAttribute.WEIGHT_BOLD : TextAttribute.WEIGHT_REGULAR);
    attributes.put(
   ...
   *
   * <p>Some characters like combining marks and non-spacing marks can only be rendered with the
   * context of other glyphs. In this case, use {@link #addGlyphs(String)}.
   */
   ...
   * that the glyphs are not actually loaded until {@link #loadGlyphs()} is called.
   */
  ...
  /**
   * Queues the glyphs in the NEHE character set (codepoints 32 through 128) to be loaded. Note that
   ...
   * should be added and loaded at the same time so that they are stored on the same backing
   * texture. This reduces the number of backing texture binds required to draw glyphs.
   */
   ...
   * called from the game loop to load glyphs on the fly that were requested for display but have
   * not yet been loaded.
   */
   ...
   * UnicodeFont instance is no longer needed.
   */
  public void dispose() {
    for (Iterator iter = glyphPages.iterator(); iter.hasNext(); ) {
  ...
  private static final int X = 0, Y = 1, U = 3, V = 4;
  private static final int X2 = 10, Y2 = 11, U2 = 13, V2 = 14;
  ...
  private void drawUnicode(String text, int startIndex, int endIndex) {
    ...
    for (int glyphIndex = 0, n = vector.getNumGlyphs(); glyphIndex < n; glyphIndex++) {
      ...
      bounds.x += offsetX;
      Glyph glyph =
          getGlyph(vector.getGlyphCode(glyphIndex), codePoint, bounds, vector, glyphIndex);
  ...
  public Glyph getGlyph(
      int glyphCode, int codePoint, Rectangle bounds, GlyphVector vector, int index) {
  ...
  private Rectangle getGlyphBounds(GlyphVector vector, int index, int codePoint) {
    ...
    if (renderType == RenderType.Native) {
      if (bounds.width == 0 || bounds.height == 0) bounds = new Rectangle();
        ...
        bounds =
            metrics.getStringBounds("" + (char) codePoint, GlyphPage.scratchGraphics).getBounds();
    }
  ...
  /**
   * Sets the additional amount to offset glyphs on the x axis. This is typically set to a negative
   * number when left or right padding is used so that glyphs are not spaced too far apart.
   */
   ...
   * negative number when top or bottom padding is used so that lines of text are not spaced too far
   * apart.
   */
  ...
  /**
   * Returns the distance from one line of text to the next. This is the sum of the descent, ascent,
   * leading, padding top, padding bottom, and padding advance y. To change the line height, use
  ...
  /**
   * Returns the path to the TTF file for this UnicodeFont, or null. If this UnicodeFont was created
   * without specifying the TTF file, it will try to determine the path using Sun classes. If this
    ...
    if (ttfFileRef == null) {
      ...
      try {
        ...
        try {
          // Java 7+.
          font2D =
              Class.forName("sun.font.FontUtilities")
                  .getDeclaredMethod("getFont2D", new Class[] {Font.class})
