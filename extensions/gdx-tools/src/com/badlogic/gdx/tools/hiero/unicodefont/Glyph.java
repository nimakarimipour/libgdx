...
public class Glyph {
  ...
  Glyph(int codePoint, Rectangle bounds, GlyphVector vector, int index, UnicodeFont unicodeFont) {
    ...
    if (unicodeFont.renderType == RenderType.FreeType && unicodeFont.bitmapFont != null) {
      ...
      // spacing, but this is the
      // best we can do with the BMFont format.
      char[] chars = Character.toChars(codePoint);
      GlyphVector charVector =
          unicodeFont
              .getFont()
              .layoutGlyphVector(
      ...
      xOffset =
          charVector.getGlyphPixelBounds(0, GlyphPage.renderContext, 0, 0).x
              - unicodeFont.getPaddingLeft();
      xAdvance =
          (int)
              (metrics.getAdvanceX()
      ...
      shape =
          vector.getGlyphOutline(
              index,
              -bounds.x + unicodeFont.getPaddingLeft(),
