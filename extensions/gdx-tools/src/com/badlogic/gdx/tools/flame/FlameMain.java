...
public class FlameMain extends JFrame implements AssetErrorListener {
  ...
  public enum ControllerType {
    ...
    ParticleController(
        ...
        new InfluencerWrapper[] {
          new InfluencerWrapper(
              "Single Particle Controller", ParticleControllerInfluencer.Single.class),
          new InfluencerWrapper(
              "Random Particle Controller", ParticleControllerInfluencer.Random.class),
          new InfluencerWrapper("Scale", ScaleInfluencer.class),
          new InfluencerWrapper("Spawn", SpawnInfluencer.class),
          new InfluencerWrapper("Dynamics", DynamicsInfluencer.class)
        });
        ...
        new Runnable() {
          public void run() {
            ...
            addRow(
                ...
                new NumericPanel(FlameMain.this, fovValue, "Field of View", ""));
            addRow(
                editorPropertiesPanel,
                new NumericPanel(FlameMain.this, deltaMultiplier, "Delta multiplier", ""));
            addRow(
                editorPropertiesPanel,
                new GradientPanel(FlameMain.this, backgroundColor, "Background color", "", true));
            ...
            addRow(
                editorPropertiesPanel,
                new BillboardBatchPanel(FlameMain.this, renderer.billboardBatch),
                1,
                1);
            addRow(
                editorPropertiesPanel,
                new PointSpriteBatchPanel(FlameMain.this, renderer.pointSpriteBatch),
                1,
                1);
  ...
  protected JPanel getPanel(Influencer influencer) {
    ...
    if (influencer instanceof ColorInfluencer.Random) {
      return new InfluencerPanel<ColorInfluencer.Random>(
          this,
          (ColorInfluencer.Random) influencer,
      ...
      return new ModelInfluencerPanel(
          this,
      ...
      String name =
          single
              ? "Particle Controller Single Influencer"
              : "Particle Controller Random Influencer";
      ...
      return new RegionInfluencerPanel(
          this,
          "Billboard Single Region Influencer",
          "Assign the chosen region to the particles",
          (RegionInfluencer.Single) influencer);
    } else if (influencer instanceof RegionInfluencer.Animated) {
      return new RegionInfluencerPanel(
          this,
          "Billboard Animated Region Influencer",
          "Animates the region of the particles",
  ...
  protected JPanel getPanel(ParticleBatch renderer) {
    ...
    if (renderer instanceof BillboardParticleBatch) {
      ...
      return new EmptyPanel(
          this, "Model Instance Batch", "It renders particles as model instances.");
    }
  ...
  public ParticleController getEmitter() {
    return effectPanel.editIndex >= 0
        ? controllersData.get(effectPanel.editIndex).controller
        : null;
  }
    ...
    getContentPane().add(splitPane, BorderLayout.CENTER);
    {
...

      {
        ...
        propertiesPanel.setBorder(
            new CompoundBorder(
                BorderFactory.createEmptyBorder(3, 0, 6, 6),
                BorderFactory.createTitledBorder("Influencers")));
        {
          JScrollPane scroll = new JScrollPane();
          propertiesPanel.add(
              scroll,
          ...
          scroll.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
          {
            ...
            addInfluencerButton.addActionListener(
                new ActionListener() {
                  @Override
  ...
  protected void addInfluencer(Class<Influencer> type, ParticleController controller) {
    ...
    try {
      ...
      if (ColorInfluencer.class.isAssignableFrom(type)) {
        replaced =
            controller.replaceInfluencer(ColorInfluencer.class, (ColorInfluencer) newInfluencer);
      } else if (RegionInfluencer.class.isAssignableFrom(type)) {
        replaced =
            controller.replaceInfluencer(RegionInfluencer.class, (RegionInfluencer) newInfluencer);
      } else if (ModelInfluencer.class.isAssignableFrom(type)) {
        ModelInfluencer newModelInfluencer = (ModelInfluencer) newInfluencer;
        ModelInfluencer currentInfluencer =
            (ModelInfluencer) controller.findInfluencer(ModelInfluencer.class);
        if (currentInfluencer != null) {
          newModelInfluencer.models.add(currentInfluencer.models.first());
        }
        replaced =
            controller.replaceInfluencer(ModelInfluencer.class, (ModelInfluencer) newInfluencer);
      } else if (ParticleControllerInfluencer.class.isAssignableFrom(type)) {
        ParticleControllerInfluencer newModelInfluencer =
            (ParticleControllerInfluencer) newInfluencer;
        ParticleControllerInfluencer currentInfluencer =
            (ParticleControllerInfluencer)
                controller.findInfluencer(ParticleControllerInfluencer.class);
        if (currentInfluencer != null) {
          newModelInfluencer.templates.add(currentInfluencer.templates.first());
        }
        replaced =
            controller.replaceInfluencer(
                ParticleControllerInfluencer.class, (ParticleControllerInfluencer) newInfluencer);
      }
  ...
  class AppRenderer implements ApplicationListener {
    ...
    public void create() {
      ...
      Model
          xyzModel =
              builder.createXYZCoordinates(10, new Material(), Usage.Position | Usage.ColorPacked),
          planeModel =
              builder.createLineGrid(
                  10,
                  10,
                  1,
                  1,
      ...
      ParticleEffectLoader.ParticleEffectLoadParameter params =
      ...
      assetManager.setLoader(
          ParticleEffect.class, new ParticleEffectLoader(new AbsoluteFileHandleResolver()));
      assetManager
          .get(DEFAULT_MODEL_PARTICLE, Model.class)
          .materials
          .get(0)
    ...
    private void update(float delta) {
      ...
      if (isUpdate) {
        ...
        stringBuilder.delete(0, stringBuilder.length);
        stringBuilder
            .append("Model Instances : ")
            .append(modelInstanceParticleBatch.getBufferedCount());
  ...
  public boolean isUsingDefaultTexture() {
    return renderer.billboardBatch.getTexture()
        == assetManager.get(DEFAULT_BILLBOARD_PARTICLE, Texture.class);
  }

  public Array<ParticleEffect> getParticleEffects(
      Array<ParticleController> controllers, Array<ParticleEffect> out) {
    out.clear();
  ...
  public void saveEffect(File file) {
    ...
    try {
      ParticleEffectLoader loader =
          (ParticleEffectLoader) assetManager.getLoader(ParticleEffect.class);
      loader.save(
          effect,
          new ParticleEffectSaveParameter(
              new FileHandle(file.getAbsolutePath()), assetManager, particleSystem.getBatches()));
    } catch (Exception ex) {
  ...
  public <T> T load(
      String resource, Class<T> type, AssetLoader loader, AssetLoaderParameters<T> params) {
