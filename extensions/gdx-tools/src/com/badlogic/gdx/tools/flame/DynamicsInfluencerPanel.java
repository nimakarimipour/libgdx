...
public class DynamicsInfluencerPanel extends InfluencerPanel<DynamicsInfluencer> {
  ...
  protected void initializeComponents() {
    ...
    velocitiesPanel.setLayout(new GridBagLayout());
    {
      ...
      {
        sideButtons.add(
            velocityBox = new JComboBox(new DefaultComboBoxModel()),
            new GridBagConstraints(
                0,
                -1,
                1,
    ...
    velocityTableModel =
        new DefaultTableModel(new String[0][0], new String[] {"Velocity", "Active"});
    ...
    velocityTableModel.addTableModelListener(
        new TableModelListener() {
          public void tableChanged(TableModelEvent event) {
            if (event.getColumn() != 1) return;
            velocityChecked(
                event.getFirstRow(), (Boolean) velocityTable.getValueAt(event.getFirstRow(), 1));
          }
        });
  ...
  protected void velocityChecked(int index, boolean isChecked) {
    ...
    DynamicsInfluencer influencer =
        (DynamicsInfluencer) controller.findInfluencer(DynamicsInfluencer.class);
