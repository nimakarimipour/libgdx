 ...
 *
 * <p>The font file format is from the AngelCodeFont BMFont tool.
 *
 * <p>Output is nearly identical to the FreeType settting in the Hiero tool {@Link
 * com.badlogic.gdx.tools.hiero.Hiero}. BitmapFontWriter gives more flexibility, eg borders and
 * shadows can be used. Hiero is able to avoid outputting the same glyph image more than once if
 * multiple character codes have the exact same glyph.
 *
...
public class BitmapFontWriter {
   ...
   * image paths for each texture page. The glyphs in BitmapFontData have a "page" id, which
   * references the index of the pageRef you specify here.
   *
   * <p>The FontInfo parameter is useful for cleaner output; such as including a size and font face
   ...
   *
   * <p>Likewise, the scaleW and scaleH are only for cleaner output. They are currently ignored by
   ...
   * @param pageRefs the references to each texture page image file, generally in the same folder as
   *     outFntFile
  ...
  public static void writeFont(
    ...
    int packed = 0;
    int base =
        (int) ((fontData.capHeight) + (fontData.flipped ? -fontData.ascent : fontData.ascent));
    ...
    for (int i = 0; i < glyphs.size; i++) {
      ...
      buf.append(xmlTab)
          ...
          .append("x=")
          .append(quote(String.format("%-5s", empty ? 0 : g.srcX), true))
          .append("y=")
          .append(quote(String.format("%-5s", empty ? 0 : g.srcY), true))
          .append("width=")
          .append(quote(String.format("%-5s", empty ? 0 : g.width), true))
          .append("height=")
          .append(quote(String.format("%-5s", empty ? 0 : g.height), true))
          .append("xoffset=")
    ...
    // KERN info
    buf.append(xmlOpen)
        .append("kernings count=")
        .append(quote(kernCount))
        .append(xmlClose)
        .append("\n");
    buf.append(kernBuf);
    ...
    if (info.hasOverrideMetrics) {
      ...
      buf.append(xmlTab)
          .append(xmlOpen)
          .append("metrics ascent=")
          .append(quote(info.ascent, true))
          .append(" descent=")
          .append(quote(info.descent, true))
          .append(" down=")
          .append(quote(info.down, true))
          .append(" capHeight=")
          .append(quote(info.capHeight, true))
          .append(" lineHeight=")
          .append(quote(info.lineHeight, true))
          .append(" spaceXAdvance=")
          .append(quote(info.spaceXAdvance, true))
          .append(" xHeight=")
          .append(quote(info.xHeight, true))
          .append(xmlCloseSelf)
          .append("\n");
   ...
   *
   * <p>The specified pixmaps are written to the parent directory of <tt>outFntFile</tt>, using that
   * file's name without an extension for the PNG file name(s).
   *
   * <p>The specified FontInfo is optional, and can be null.
   *
   ...
   */
  public static void writeFont(
      BitmapFontData fontData, Pixmap[] pages, FileHandle outFntFile, FontInfo info) {
   ...
   * like: "fileName.png".
   *
   * <p>If the pages array is greater than length 1, the resulting file refs will be appended with
   * "_N", such as "fileName_0.png", "fileName_1.png", "fileName_2.png" etc.
   *
   * <p>The returned string array can then be passed to the <tt>writeFont</tt> method.
   *
   * <p>Note: None of the pixmaps will be disposed.
   *
