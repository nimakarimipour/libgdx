...
public class ParticleEditor extends JFrame {
  ...
  void reloadRows() {
    ...
    addEditorRow(
        new CustomShadingPanel(
            ParticleEditor.this, "Shading", "Custom shader and multi-texture preview."));
    ...
    addRow(
        new RangedNumericPanel(
            emitter.getDelay(),
            "Delay",
            "Time from beginning of effect to emission start, in milliseconds."));
    addRow(
        new RangedNumericPanel(
            emitter.getDuration(), "Duration", "Time particles will be emitted, in milliseconds."));
    addRow(
        new ScaledNumericPanel(
            emitter.getEmission(),
            "Duration",
            "Emission",
    ...
    addRow(
        new ScaledNumericPanel(
            emitter.getLife(), "Duration", "Life", "Time particles will live, in milliseconds."));
    addRow(
        new ScaledNumericPanel(
    ...
    addRow(
        new ScaledNumericPanel(
            emitter.getYScale(), "Life", "Y Size", "Particle y size, in world units."));
    addRow(
        new ScaledNumericPanel(
            emitter.getVelocity(),
            "Life",
    ...
    addRow(
        new ScaledNumericPanel(
            emitter.getAngle(), "Life", "Angle", "Particle emission angle, in degrees."));
    addRow(
        new ScaledNumericPanel(
            emitter.getRotation(), "Life", "Rotation", "Particle rotation, in degrees."));
    addRow(
        new ScaledNumericPanel(
            emitter.getWind(), "Life", "Wind", "Wind strength, in world units per second."));
    addRow(
        new ScaledNumericPanel(
            emitter.getGravity(),
            "Life",
  ...
  class Renderer implements ApplicationListener, InputProcessor {
    ...
    public void create() {
      ...
      font =
          new BitmapFont(
              Gdx.files.getFileHandle("default.fnt", FileType.Internal),
              Gdx.files.getFileHandle("default.png", FileType.Internal),
              true);
      effectPanel.newExampleEmitter("Untitled", true);
      // if (resources.openFile("/editor-bg.png") != null) bgImage = new Image(gl,
      // "/editor-bg.png");
    ...
    public void resize(int width, int height) {
      ...
      worldCamera.setToOrtho(
          false, width / pixelsPerMeter.getValue(), height / pixelsPerMeter.getValue());
      worldCamera.update();
          ...
          if (imagePath.equals(ParticleEditor.DEFAULT_PARTICLE)
            ...
            if ((imagePath.contains("/") || imagePath.contains("\\"))
              ...
              if (!file.exists()) {
                ...
                file =
                    Gdx.files.absolute(
                        new File(effectFile.getParentFile(), imageName).getAbsolutePath());
              }
            } else {
              file =
                  Gdx.files.absolute(
                      new File(effectFile.getParentFile(), imagePath).getAbsolutePath());
            }
          }
    ...
    public boolean keyDown(int keycode) {
      if (keycode == Input.Keys.SPACE) {
        effect.setPosition(
            previewImagePanel.valueX.getValue() + previewImagePanel.valueWidth.getValue() / 2f,
    ...
    public boolean touchUp(int x, int y, int pointer, int button) {
      ParticleEditor.this.dispatchEvent(
          new WindowEvent(ParticleEditor.this, WindowEvent.WINDOW_LOST_FOCUS));
      ParticleEditor.this.dispatchEvent(
          new WindowEvent(ParticleEditor.this, WindowEvent.WINDOW_GAINED_FOCUS));
