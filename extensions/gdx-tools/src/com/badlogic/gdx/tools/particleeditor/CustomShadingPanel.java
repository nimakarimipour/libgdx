...
public class CustomShadingPanel extends EditorPanel {
  ...
  private JPanel createShaderStagePanel(
      final ParticleEditor editor, JPanel contentPanel, final boolean isVertexShader) {
        ...
        new ActionListener() {
          public void actionPerformed(ActionEvent event) {
            FileDialog dialog =
                new FileDialog(
                    editor,
                    isVertexShader ? "Open Vertex Shader File" : "Open Fragment Shader File",
                    FileDialog.LOAD);
            if (lastDir != null) dialog.setDirectory(lastDir);
            dialog.setVisible(true);
            final String file = dialog.getFile();
    ...
    showButton.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent event) {
            JTextArea text =
                new JTextArea(
                    isVertexShader ? shading.vertexShaderCode : shading.fragmentShaderCode);
            text.setEditable(false);
            JOptionPane.showMessageDialog(
                editor,
                text,
                isVertexShader ? "Current vertex shader code" : "Current fragment shader code",
  ...
  protected void displayErrors() {
    if (shading.hasShaderErrors) {
      JOptionPane.showMessageDialog(
          editor, shading.shaderErrorMessage, "Shader Error", JOptionPane.ERROR_MESSAGE);
    } else if (shading.hasMissingSamplers) {
      JOptionPane.showMessageDialog(
          editor,
          shading.missingSamplerMessage,
