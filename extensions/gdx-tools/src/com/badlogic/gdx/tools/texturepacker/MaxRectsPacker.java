 ...
 * brute force binary search is used to pack into the smallest bin possible.
 *
 * @author Nathan Sweet
 */
public class MaxRectsPacker implements Packer {
  ...
  private final Comparator<Rect> rectComparator =
      new Comparator<Rect>() {
        public int compare(Rect o1, Rect o2) {
          return Rect.getAtlasName(o1.name, settings.flattenPaths)
              .compareTo(Rect.getAtlasName(o2.name, settings.flattenPaths));
        }
      };
  ...
  private Page packPage(Array<Rect> inputRects) {
      ...
      if (settings.rotation) {
        if ((width > maxWidth || height > maxHeight) && (width > maxHeight || height > maxWidth)) {
          ...
          throw new RuntimeException(
              "Image does not fit within max page size "
                  + settings.maxWidth
                  + "x"
                  + settings.maxHeight
                  + paddingMessage
        ...
        if (width > maxWidth) {
          ...
          throw new RuntimeException(
              "Image does not fit within max page width "
                  + settings.maxWidth
                  + paddingMessage
                  + ": "
                  + rect.name
        ...
        if (height > maxHeight && (!settings.rotation || width > maxHeight)) {
          ...
          throw new RuntimeException(
              "Image does not fit within max page height "
                  + settings.maxHeight
                  + paddingMessage
                  + ": "
                  + rect.name
    ...
    // BinarySearch uses the max size. Rects are packed with right and top padding, so the max size
    ...
    if (settings.square) {
      ...
      BinarySearch sizeSearch =
          new BinarySearch(
              minSize, maxSize, settings.fast ? 25 : 15, settings.pot, settings.multipleOfFour);
      ...
      if (bestResult == null)
        bestResult =
            packAtSize(
                false, settings.maxWidth + adjustX, settings.maxHeight + adjustY, inputRects);
      sort.sort(bestResult.outputRects, rectComparator);
  ...
  class MaxRects {
    ...
    private Rect scoreRect(Rect rect, FreeRectChoiceHeuristic method) {
      ...
      switch (method) {
        ...
        case ContactPointRule:
          ...
          newNode.score1 =
              -newNode
                  .score1; // Reverse since we are minimizing, but for contact point score bigger is
                           // better.
          break;
    ...
    private Rect findPositionForNewNodeBottomLeft(
        int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
      Rect bestNode = new Rect();
      ...
      for (int i = 0; i < freeRectangles.size; i++) {
        ...
        if (freeRectangles.get(i).width >= width && freeRectangles.get(i).height >= height) {
          ...
          if (topSideY < bestNode.score1
              || (topSideY == bestNode.score1 && freeRectangles.get(i).x < bestNode.score2)) {
        ...
        if (rotate
            && freeRectangles.get(i).width >= rotatedWidth
            && freeRectangles.get(i).height >= rotatedHeight) {
    ...
    private Rect findPositionForNewNodeBestShortSideFit(
        int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
      Rect bestNode = new Rect();
      bestNode.score1 = Integer.MAX_VALUE;
    ...
    private Rect findPositionForNewNodeBestLongSideFit(
        int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
      Rect bestNode = new Rect();
    ...
    private Rect findPositionForNewNodeBestAreaFit(
        int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
      Rect bestNode = new Rect();
    ...
    // / Returns 0 if the two intervals i1 and i2 are disjoint, or the length of their overlap
    // otherwise.
    ...
    private Rect findPositionForNewNodeContactPoint(
        int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
