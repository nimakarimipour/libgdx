...
public class TextureUnpacker {
  ...
  private int parseArguments(String[] args) {
    ...
    boolean extension =
        args[0]
            .substring(args[0].length() - ATLAS_FILE_EXTENSION.length())
        ...
        if (region.page == page) {
          ...
          if (region.findValue("split") == null) {
            ...
            if (region.width != region.originalWidth || region.height != region.originalHeight) {
              BufferedImage originalImg =
                  new BufferedImage(region.originalWidth, region.originalHeight, img.getType());
          ...
          File imgOutput =
              new File(
                  ...
                  String.format(
                      "%s.%s",
                      region.index == -1 ? region.name : region.name + "_" + region.index,
                      extension));
  ...
  private BufferedImage extractImage(
      BufferedImage page, Region region, File outputDirFile, int padding) {
    BufferedImage splitImage = null;

    // get the needed part of the page and rotate if needed
    if (region.rotate) {
      BufferedImage srcImage =
          page.getSubimage(region.left, region.top, region.height, region.width);
  ...
  public static void main(String[] args) {
    ...
    // parse the arguments and display the help text if there is a problem with the command line
    // arguments
    ...
    TextureAtlasData atlas =
        new TextureAtlasData(new FileHandle(atlasFile), new FileHandle(imageDir), false);
