...
public class TexturePacker {
  ...
  public TexturePacker(File rootDir, Settings settings) {
    ...
    if (settings.multipleOfFour) {
      if (settings.maxWidth % 4 != 0)
        throw new RuntimeException(
            "If mod4 is true, maxWidth must be evenly divisible by 4: " + settings.maxWidth);
      if (settings.maxHeight % 4 != 0)
        throw new RuntimeException(
            "If mod4 is true, maxHeight must be evenly divisible by 4: " + settings.maxHeight);
    }
  ...
  public void pack(File outputDir, String packFileName) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (settings.scaleResampling != null
          && settings.scaleResampling.length > i
          && settings.scaleResampling[i] != null)
    ...
    for (int p = 0, pn = pages.size; p < pn; p++) {
      ...
      for (int r = 0, rn = page.outputRects.size; r < rn; r++) {
        ...
        int rectX = page.x + rect.x,
            rectY = page.y + page.height - rect.y - (rect.height - settings.paddingY);
        if (settings.duplicatePadding) {
  ...
  private static void copy(
    ...
    if (rotated) {
      for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++) plot(dst, dx + j, dy + w - i - 1, src.getRGB(x + i, y + j));
    } else {
      for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++) plot(dst, dx + i, dy + j, src.getRGB(x + i, y + j));
    }
  }

  private void writePackFile(File outputDir, String scaledPackFileName, Array<Page> pages)
      throws IOException {
    ...
    OutputStreamWriter writer =
        new OutputStreamWriter(new FileOutputStream(packFile, true), "UTF-8");
  ...
  private void writePage(OutputStreamWriter writer, boolean appending, Page page)
    ...
    if (settings.format != Format.RGBA8888)
      writer.write(tab + "format" + colon + settings.format + "\n");
  ...
  private void writeRect(Writer writer, Page page, Rect rect, String name) throws IOException {
    ...
    writer.write(
        tab
            ...
            + (page.x + rect.x)
            + comma
            + (page.y + page.height - rect.y - (rect.height - settings.paddingY))
            + comma //
            + rect.regionWidth
            + comma
            + rect.regionHeight
    ...
    if (rect.offsetX != 0
        || offsetY != 0 //
        || rect.originalWidth != rect.regionWidth
  ...
  public static class Rect implements Comparable<Rect> {
    ...
    public Rect(
        BufferedImage source, int left, int top, int newWidth, int newHeight, boolean isPatch) {
      image =
          new BufferedImage(
    ...
    /**
     * Clears the image for this rect, which will be loaded from the specified file by {@link
     * #getImage(ImageProcessor)}.
     */
  ...
  public static void process(
    ...
    try {
      TexturePackerFileProcessor processor =
          new TexturePackerFileProcessor(settings, packFileName, progress);
  ...
  /**
   * @return true if the output file does not yet exist or its last modification date is before the
   *     last modification date of the input file
   */
  public static boolean isModified(
      String input, String output, String packFileName, Settings settings) {
    ...
    // Check against the only file we know for sure will exist and will be changed if any asset
    // changes: the atlas file.
  ...
  public static boolean processIfModified(
      Settings settings, String input, String output, String packFileName) {
    if (isModified(input, output, packFileName, settings)) {
      process(settings, input, output, packFileName);
      return true;
    }
        ...
        if (scale.length != 1) {
          packFileName =
              (scaleValue == (int) scaleValue
                      ? Integer.toString((int) scaleValue)
                      : Float.toString(scaleValue))
                  + "/"
                  + packFileName;
        }
      }
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (output == null) {
      File inputFile = new File(input);
      output =
          new File(inputFile.getParentFile(), inputFile.getName() + "-packed").getAbsolutePath();
    }
