...
public class GridPacker implements Packer {
  ...
  public Array<Page> pack(ProgressListener progress, Array<Rect> inputRects) {
    ...
    // Rects are packed with right and top padding, so the max size is increased to match. After
    // packing the padding is
  ...
  private Page packPage(
      Array<Rect> inputRects, int cellWidth, int cellHeight, int maxWidth, int maxHeight) {
    Page page = new Page();
