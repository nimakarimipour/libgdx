 ...
 *
 * <p>See http://www.blackpawn.com/texts/lightmaps/default.html for details.
 *
 * <p><b>Usage:</b> instanciate an <code>ImagePacker</code> instance, load and optionally sort the
 * images you want to add by size (e.g. area) then insert each image via a call to {@link
 * #insertImage(String, BufferedImage)}. When you are done with inserting images you can call {@link
 * #getImage()} for the {@link BufferedImage} that holds the packed images. Additionally you can get
 * a <code>Map<String, Rectangle></code> where the keys the names you specified when inserting and
 * the values are the rectangles within the packed image where that specific image is located. All
 * things are given in pixels.
 *
 * <p>See the {@link #main(String[])} method for an example that will generate 100 random images,
 * pack them and then output the packed image as a png along with a json file holding the image
 * descriptors.
 *
 * <p>In some cases it is beneficial to add padding and to duplicate the border pixels of an
 * inserted image so that there is no bleeding of neighbouring pixels when using the packed image as
 * a texture. You can specify the padding as well as whether to duplicate the border pixels in the
 * constructor.
 *
 * <p>Happy packing!
 *
...
public class ImagePacker {
  static final class Node {
    ...
    public Node(
        int x, int y, int width, int height, Node leftChild, Node rightChild, String leaveName) {
   ...
   * <code>height</code> image. <code>padding</code> specifies the minimum number of pixels to
   * insert between images. <code>border</code> will duplicate the border pixels of the inserted
  ...
  /**
   * Inserts the given image. You can later on retrieve the images position in the output image via
   * the supplied name and the method {@link #getRects()}.
   *
  ...
  public void insertImage(String name, BufferedImage image) {
    ...
    borderPixels <<= 1;
    Rectangle rect =
        new Rectangle(0, 0, image.getWidth() + borderPixels, image.getHeight() + borderPixels);
    ...
    if (duplicateBorder) {
      g.drawImage(
          image, rect.x, rect.y - 1, rect.x + rect.width, rect.y, 0, 0, image.getWidth(), 1, null);
      g.drawImage(
          image,
          rect.x,
          rect.y + rect.height,
