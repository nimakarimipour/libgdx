...
public class ANGLELoader {
    ...
    try {
      if (!outFile.getParentFile().exists() && !outFile.getParentFile().mkdirs())
        throw new GdxRuntimeException(
            "Couldn't create ANGLE native library output directory "
  ...
  public static void load() {
    if ((isARM && !isMac) || (!isWindows && !isLinux && !isMac))
      throw new GdxRuntimeException(
          "ANGLE is only supported on x86/x86_64 Windows, x64 Linux, and x64/arm64 macOS.");
    String osDir = null;
    String ext = null;
    if (isWindows) {
    ...
    String crc =
        crc(ANGLELoader.class.getResourceAsStream("/" + eglSource))
