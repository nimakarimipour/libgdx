...
public class btBroadphasePairArray extends BulletBase {
  ...
  /**
   * Construct a new btBroadphasePairArray, normally you should not need this constructor it's
   ...
   * @param other The collision object the pair must contain (which itself is excluded from the
   *     result)
   * @param tempArray A temporary array used by the method, not more object than the length of this
   *     array are added
   * @return The array specified by out
   */
  public com.badlogic.gdx.utils.Array<btCollisionObject> getCollisionObjects(
      final com.badlogic.gdx.utils.Array<btCollisionObject> out,
      final btCollisionObject other,
      final int[] tempArray) {
    final int c =
        getCollisionObjects(tempArray, tempArray.length, (int) btCollisionObject.getCPtr(other));
    for (int i = 0; i < c; i++) out.add(btCollisionObject.getInstance(tempArray[i], false));
    return out;
  }

  /**
   * Fills the given array with user value set using {@link btCollisionObject#setUserValue(int)} of
   * the collision objects within this pair array colliding with the given collision object.
   *
  ...
  public btBroadphasePair at(int n) {
    return btBroadphasePair.internalTemp(
        CollisionJNI.btBroadphasePairArray_at(swigCPtr, this, n), false);
  }

  public int getCollisionObjects(int[] result, int max, int other) {
    return CollisionJNI.btBroadphasePairArray_getCollisionObjects(
        swigCPtr, this, result, max, other);
  }

  public int getCollisionObjectsValue(int[] result, int max, int other) {
    return CollisionJNI.btBroadphasePairArray_getCollisionObjectsValue(
        swigCPtr, this, result, max, other);
  }
