...
public class DebugDrawer extends btIDebugDraw implements Disposable {
  ...
  public void drawContactPoint(
      Vector3 pointOnB, Vector3 normalOnB, float distance, int lifeTime, Vector3 color) {
    shapeRenderer.setColor(color.x, color.y, color.z, 1f);
  ...
  /**
   * Use this in case no {@code glViewport} is in use. Otherwise please supply the used {@link
   * Viewport} to {@link #begin(Viewport)}.
   *
  ...
  /**
   * Switches the {@link ShapeRenderer}. The given shape renderer won't be disposed when {@link
   * #dispose()} is called.
   */
  ...
  /**
   * Switches the {@link SpriteBatch}. The given sprite batch won't be disposed when {@link
   * #dispose()} is called.
   */
  ...
  /**
   * Switches the {@link BitmapFont}. The given font won't be disposed when {@link #dispose()} is
   * called.
   */
