...
public class Bullet {
  ...
  /**
   * Loads the native Bullet native library and initializes the gdx-bullet extension. Must be called
   * before any of the bullet classes/methods can be used.
   */
   ...
   * before any of the bullet classes/methods can be used.
   *
   * @param useRefCounting Whether to use reference counting, causing object to be destroyed when no
   *     longer referenced. You must use {@link BulletBase#obtain()} and {@link
   *     BulletBase#release()} when using reference counting.
   */
  ...
  private static final Pool<ShapePart> shapePartPool =
      new Pool<ShapePart>() {
        @Override
  ...
  public static <T extends Node> void getShapeParts(
      final Iterable<T> nodes,
      final Array<ShapePart> out,
      final int offset,
      final Pool<ShapePart> pool) {
