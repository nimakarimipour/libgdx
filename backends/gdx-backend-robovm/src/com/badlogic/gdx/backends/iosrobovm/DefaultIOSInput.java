...
public class DefaultIOSInput extends AbstractInput implements IOSInput {
  ...
  private static class NSObjectWrapper<T extends NSObject> {
    ...
    static {
      try {
        HANDLE_OFFSET =
            VM.getInstanceFieldOffset(
                VM.getFieldAddress(NativeObject.class.getDeclaredField("handle")));
      } catch (Throwable t) {
        throw new Error(t);
      }
    }
  ...
  private static final NSObjectWrapper<UITouch> UI_TOUCH_WRAPPER =
      new NSObjectWrapper<UITouch>(UITouch.class);
  static final NSObjectWrapper<UIAcceleration> UI_ACCELERATION_WRAPPER =
      new NSObjectWrapper<UIAcceleration>(UIAcceleration.class);
  ...
  protected void setupAccelerometer() {
    if (config.useAccelerometer) {
      accelerometerDelegate =
          new UIAccelerometerDelegateAdapter() {
  ...
  protected void setupPressure() {
    if (app.getVersion() >= 9) {
      UIForceTouchCapability forceTouchCapability =
          UIScreen.getMainScreen().getTraitCollection().getForceTouchCapability();
  ...
  // see:
  // https://web.archive.org/web/20171016192705/http://www.badlogicgames.com/forum/viewtopic.php?f=17&t=11788

  private UITextField textfield = null;
  private final UITextFieldDelegate textDelegate =
      new UITextFieldDelegateAdapter() {
        @Override
   ...
   */
  private UIAlertController buildUIAlertController(
      final TextInputListener listener,
      String title,
      final String text,
      final String placeholder,
      final OnscreenKeyboardType type) {
    final UIAlertController uiAlertController =
        new UIAlertController(title, text, UIAlertControllerStyle.Alert);
  ...
  public boolean isPeripheralAvailable(Peripheral peripheral) {
    ...
    if (peripheral == Peripheral.HardwareKeyboard)
      return Foundation.getMajorSystemVersion() >= 14
          ? GCKeyboard.getCoalescedKeyboard() != null
          : hadHardwareKeyEvent;
    return false;
  }
        ...
        if (!down) {
          ...
          switch (keyCode) {
            ...
            default:
              ...
              character =
                  (characters != null && characters.length() == 1) ? characters.charAt(0) : 0;
          }
  ...
  private static class NSArrayExtensions extends NSExtensions {
    ...
    public static native @Pointer long objectAtIndex$(
        @Pointer long thiz, @MachineSizedUInt long index);
