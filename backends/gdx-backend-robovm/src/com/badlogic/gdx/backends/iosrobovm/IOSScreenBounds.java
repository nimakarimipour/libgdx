 ...
 * <li>Screen is rotated - in some iOS versions the rotation reporting behavior is different and this needs to be handled
 * </ul>
 *
 * <h3>IMPLEMENTATION & WARNING - Read carefully</h3>
 *
 * Accounting for status bar is not completely clean and relies on a coincidence, related to coordinate system origins. When
 * status bar is present, x and y grows (in practice only y does) to offset the drawing area from the status bar and width and
 * height (and their backBuffer values of course) shrink so the remaining surface fits the rest of the screen.
 *
 * <p>
