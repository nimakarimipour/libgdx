...
public class IOSGraphics extends AbstractGraphics {
  ...
  public IOSGraphics(
      IOSApplication app, IOSApplicationConfiguration config, IOSInput input, boolean useGLES30) {
    this.config = config;
    ...
    view =
        new GLKView(new CGRect(0, 0, screenBounds.width, screenBounds.height), context) {
    ...
    glVersion =
        new GLVersion(Application.ApplicationType.iOS, versionString, vendorString, rendererString);
  ...
  public DisplayMode getDisplayMode() {
    return new IOSDisplayMode(
        getWidth(),
        getHeight(),
        config.preferredFramesPerSecond,
  ...
  /**
   * Sets the preferred framerate for the application. Default is 60. Is not generally advised to be
  ...
  private class IOSViewDelegate extends NSObject
      implements GLKViewDelegate, GLKViewControllerDelegate {
    @Override
    public void update(GLKViewController controller) {
