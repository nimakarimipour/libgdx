...
public class IOSMusic implements Music {
  ...
  public IOSMusic(OALAudioTrack track, String filePath) {
    ...
    this.track.setDelegate(
        new AVAudioPlayerDelegateAdapter() {
          @Override
  ...
  public void play() {
    if (track.isPaused()) {
      ...
      // Check
      // https://github.com/kstenerud/ObjectAL-for-iPhone/blob/master/ObjectAL/ObjectAL/AudioTrack/OALAudioTrack.m
      // OALAudioTrack needs to execute preloadURL() once to store the file path. From then on we
      // avoid
      // calling it again to avoid instantiating a new AVAudioPlayer every time.
      if (!initialized) {
        if (!looping) initialized = track.playFile(filePath);
        else initialized = track.playFile(filePath, -1);
        if (!initialized) {
  ...
  /**
   * Calling this method preloads audio buffers and acquires the audio hardware necessary for
   * playback. Can be noticeable on latency critical scenarios. Call with some time before {@link
   * #play()}.
   */
