...
public class IOSApplication implements Application {
  ...
  public abstract static class Delegate extends UIApplicationDelegateAdapter {
    ...
    public boolean didFinishLaunching(
      ...
      application.addStrongRef(
          this); // Prevent this from being GCed until the ObjC UIApplication is deallocated
  ...
  final boolean didFinishLaunching(UIApplication uiApp, UIApplicationLaunchOptions options) {
    ...
    Gdx.app.debug(
        "IOSApplication", "iOS version: " + UIDevice.getCurrentDevice().getSystemVersion());
  ...
  /**
   * Returns device ppi using a best guess approach when device is unknown. Overwrite to customize
   * strategy.
   */
  protected int guessUnknownPpi() {
    ...
    else ppi = 164 * (int) pixelsPerPoint;
    error(
        "IOSApplication",
  ...
  protected IOSScreenBounds computeBounds() {
    ...
    return lastScreenBounds =
        new IOSScreenBounds(offsetX, offsetY, width, height, backBufferWidth, backBufferHeight);
  }
  ...
  public IOSScreenBounds getScreenBounds() {
    if (lastScreenBounds == null) return computeBounds();
    else return lastScreenBounds;
  }
