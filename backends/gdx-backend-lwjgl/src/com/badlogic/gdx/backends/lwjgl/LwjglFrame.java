...
public class LwjglFrame extends JFrame {
  ...
  private void construct(ApplicationListener listener, LwjglApplicationConfiguration config) {
    lwjglCanvas =
        new LwjglCanvas(listener, config) {
    ...
    Dimension size =
        new Dimension(Math.round(config.width / scaleX), Math.round(config.height / scaleY));
    ...
    addWindowFocusListener(
        new WindowAdapter() {
          public void windowLostFocus(WindowEvent event) {
            ...
            // Normally Display.reshape is called from Display.update when the size changes, but
            // LwjglCanvas doesn't call
            // Display.update when rendering is not needed because it also swaps buffers and would
            // flicker.
            // After losing focus rendering may not be needed so Display.reshape must be called,
            // else the OpenGL window may be
            // left in the wrong place.
            // Display.setLocation calls Display.reshape, despite javadocs saying it's a no-op when
            // a canvas is set.
        ...
        new Runnable() {
          public void run() {
            ...
            try {
              ...
              // Fails on Linux sometimes, seems shared lib isn't loaded for
              // LinuxDisplay#callErrorHandler.
            }
          }
        });
  }
  ...
  /**
   * When true, <code>Runtime.getRuntime().halt(0);</code> is used when the JVM shuts down. This
   * prevents Swing shutdown hooks from causing a deadlock and keeping the JVM alive indefinitely.
   * Default is true.
   */
