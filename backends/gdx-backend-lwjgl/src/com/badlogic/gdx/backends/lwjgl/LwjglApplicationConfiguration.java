...
public class LwjglApplicationConfiguration {
  /**
   * If true, OpenAL will not be used. This means {@link Application#getAudio()} returns null and
   * the gdx-openal.jar and OpenAL natives are not needed.
   */
  ...
  /**
   * The OpenGL context major version (the part in front of the decimal point) used to emulate
   * OpenGL ES 3.0, when the version is not supported it will fall back to OpenGL ES 2.0 emulation.
   * Defaults to 3.2 (major=3, minor=2). Only used when {@link #useGL30} is true. OpenGL is fully
   * compatible with OpenGL ES 3.0 since version 4.3, setting the context version to a lower value
   * might cause some features not to function properly. OSX requires 3.2 though.
   *
  ...
  /**
   * The OpenGL context major version (the part after the decimal point) used to emulate OpenGL ES
   * 3.0, when the version is not supported it will fall back to OpenGL ES 2.0 emulation. Defaults
   * to 3.2 (major=3, minor=2). Only used when {@link #useGL30} is true. OpenGL is fully compatible
   * with OpenGL ES 3.0 since version 4.3, setting the context version to a lower value might cause
   * some features not to function properly. OSX requires 3.2 though.
   *
   * @see <a href="http://legacy.lwjgl.org/javadoc/org/lwjgl/opengl/ContextAttribs.html">LWJGL OSX
   *     ContextAttribs note</a>
   */
  ...
  public String title;
  /**
   * whether to call System.exit() on tear-down. Needed for Webstarts on some versions of Mac OS X
   * it seems *
   */
  ...
  /**
   * Target framerate when the window is in the foreground. The CPU sleeps as needed. Use 0 to never
   * sleep. *
   */
  public int foregroundFPS = 60;
  /**
   * Target framerate when the window is not in the foreground. The CPU sleeps as needed. Use 0 to
   * never sleep, -1 to not render. *
   */
  ...
  /**
   * {@link LifecycleListener#pause()} (but continue rendering) when the window is not minimized and
   * not the foreground window. To stop rendering when not the foreground window, use backgroundFPS
   * -1. *
   */
  ...
  /**
   * Callback used when trying to create a display, can handle failures, default value is null
   * (disabled)
   */
   ...
   * Typically three icons should be provided: 128x128 (for Mac), 32x32 (for Windows and Linux), and
   * 16x16 (for Windows).
   */
  ...
  /**
   * Sets the r, g, b and a bits per channel based on the given {@link DisplayMode} and sets the
   * fullscreen flag to true.
   *
   * @param mode
   */
  ...
  protected static class LwjglApplicationConfigurationDisplayMode extends DisplayMode {
    protected LwjglApplicationConfigurationDisplayMode(
