      ...
      while (Mouse.next()) {
        ...
        int y =
            Gdx.graphics.getHeight()
                - (int) (Mouse.getEventY() * Display.getPixelScaleFactor())
                - 1;
        ...
        if (button == -1) {
          ...
          else if (pressedButtons.size > 0) eventQueue.touchDragged(x, y, 0, time);
          else eventQueue.mouseMoved(x, y, time);
    ...
    if (Keyboard.isCreated()) {
      while (Keyboard.next()) {
        ...
        if (Keyboard.getEventKeyState()
            || (keyCode == 0 && keyChar != 0 && Character.isDefined(keyChar))) {
          switch (keyCode) {
            case Keys.DEL:
              keyChar = 8;
              break;
