...
/**
 * An OpenGL surface on an AWT Canvas, allowing OpenGL to be embedded in a Swing application. This
 * uses {@link Display#setParent(Canvas)}, which is preferred over {@link AWTGLCanvas} but is
 * limited to a single LwjglCanvas in an application. All OpenGL calls are done on the EDT. Note
 * that you may need to call {@link #stop()} or a Swing application may deadlock on System.exit due
...
public class LwjglCanvas implements LwjglApplicationBase {
  ...
  private void initialize(ApplicationListener listener, LwjglApplicationConfiguration config) {
    ...
    graphics =
        new LwjglGraphics(canvas, config) {
    ...
    EventQueue.invokeLater(
        new Runnable() {
          ...
          private boolean checkResize() {
            ...
            if (parent != null
                && (canvas.getWidth() != parent.getWidth()
                    || canvas.getHeight() != parent.getHeight()))
  ...
  protected int getFrameRate() {
    int frameRate =
        Display.isActive() ? graphics.config.foregroundFPS : graphics.config.backgroundFPS;
  ...
  /**
   * When true, {@link #postRunnable(Runnable)} keeps the stacktrace (which is an allocation) so it
   * can be included if the runnable later throws an exception. Default is false.
   */
