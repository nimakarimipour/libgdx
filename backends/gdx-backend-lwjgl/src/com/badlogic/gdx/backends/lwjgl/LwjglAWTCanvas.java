 ...
 * uses {@link AWTGLCanvas}, which allows multiple LwjglAWTCanvas to be used in a single
 * application. All OpenGL calls are done on the EDT. Note that you may need to call {@link #stop()}
 * or a Swing application may deadlock on System.exit due to how LWJGL and/or Swing deal with
 * shutdown hooks.
 *
 * @author Nathan Sweet
 */
public class LwjglAWTCanvas implements Application {
  ...
  /**
   * Make the canvas' context current. It is highly recommended that the context is only made
   * current inside the AWT thread (for example in an overridden paintGL()).
   */
