...
public class LwjglGraphics extends AbstractGraphics {
  ...
  public int getHeight() {
    if (canvas != null) return Math.max(1, canvas.getHeight());
    else return (int) (Display.getHeight() * Display.getPixelScaleFactor());
  }

  public int getWidth() {
    if (canvas != null) return Math.max(1, canvas.getWidth());
    else return (int) (Display.getWidth() * Display.getPixelScaleFactor());
  }
  ...
  /**
   * The delta time for the next frame will be 0. This can be useful if the render thread was
   * blocked for some time to prevent game state or animations from advancing.
   */
  ...
  void setupDisplay() throws LWJGLException {
    ...
    createDisplayPixelFormat(
        config.useGL30, config.gles30ContextMajorVersion, config.gles30ContextMinorVersion);
    initiateGL();
  }
  ...
  private static boolean fullCompatibleWithGLES2() {
    ...
    return glVersion.isVersionEqualToOrHigher(4, 1)
        || extensions.contains("GL_ARB_ES2_compatibility", false);
  }
  ...
  public static boolean supportsCubeMapSeamless() {
    return glVersion.isVersionEqualToOrHigher(3, 2)
        || extensions.contains("GL_ARB_seamless_cube_map", false);
  }

  /**
   * Enable or disable cubemap seamless feature. Default is true if supported. Should only be called
   * if this feature is supported. (see {@link #supportsCubeMapSeamless()})
   *
   * @param enable
   */
  ...
  private void createDisplayPixelFormat(
      boolean useGL30, int gles30ContextMajor, int gles30ContextMinor) {
    try {
      if (useGL30) {
        ContextAttribs context =
            new ContextAttribs(gles30ContextMajor, gles30ContextMinor)
  ...
  /**
   * Kindly stolen from http://lwjgl.org/wiki/index.php?title=LWJGL_Basics_5_(Fullscreen), not
   * perfect but will do.
   */
  @Override
  public boolean setWindowedMode(int width, int height) {
    boolean displaySizeUnchanged =
        getWidth() == width && getHeight() == height && !Display.isFullscreen();
  ...
  public DisplayMode getDisplayMode() {
    ...
    return new LwjglDisplayMode(
        mode.getWidth(), mode.getHeight(), mode.getFrequency(), mode.getBitsPerPixel(), mode);
  }
  ...
  /**
   * Sets the target framerate for the application, when using continuous rendering. Must be
   * positive. The cpu sleeps as needed. Use 0 to never sleep. Default is 60.
   *
   * @param fps fps
   */
  @Override
  ...
  public interface SetDisplayModeCallback {
    /**
     * If the display creation fails, this method will be called. Suggested usage is to modify the
     * passed configuration to use a common width and height, and set fullscreen to false.
     *
     * @return the configuration to be used for a second attempt at creating a display. A null value
     *     results in NOT attempting to create the display a second time
     */
