  ...
  public LwjglCursor(Pixmap pixmap, int xHotspot, int yHotspot) {
    ...
    try {
      ...
      if (xHotspot < 0 || xHotspot >= pixmap.getWidth()) {
        throw new GdxRuntimeException(
            "xHotspot coordinate of "
                + xHotspot
                + " is not within image width bounds: [0, "
                + pixmap.getWidth()
                + ").");
      }
      ...
      if (yHotspot < 0 || yHotspot >= pixmap.getHeight()) {
        throw new GdxRuntimeException(
            "yHotspot coordinate of "
                + yHotspot
                + " is not within image height bounds: [0, "
                + pixmap.getHeight()
                + ").");
      }
        ...
        for (int y = 0; y < pixmap.getHeight(); ++y) {
          for (int x = 0; x < pixmap.getWidth(); ++x) {
            ...
            pixelsARGBflipped[x + ((pixmap.getHeight() - 1 - y) * pixmap.getWidth())] =
                ((pixel >> 8) & 0x00FFFFFF) | ((pixel << 24) & 0xFF000000);
          }
        }
      } else {
        for (int y = 0; y < pixmap.getHeight(); ++y) {
      ...
      lwjglCursor =
          new org.lwjgl.input.Cursor(
              ...
              1,
              IntBuffer.wrap(pixelsARGBflipped),
              null);
