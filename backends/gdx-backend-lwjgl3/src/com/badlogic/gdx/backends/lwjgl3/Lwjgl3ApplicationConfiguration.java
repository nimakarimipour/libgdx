...
public class Lwjgl3ApplicationConfiguration extends Lwjgl3WindowConfiguration {
  ...
  /**
   * The maximum number of threads to use for network requests. Default is {@link
   * Integer#MAX_VALUE}.
   */
  ...
  /**
   * Whether to disable audio or not. If set to true, the returned audio class instances like {@link
   * Audio} or {@link Music} will be mock implementations.
   */
   ...
   *
   * @param simultaneousSources the maximum number of sources that can be played simultaniously
   *     (default 16)
  ...
  /**
   * Sets which OpenGL version to use to emulate OpenGL ES. If the given major/minor version is not
   * supported, the backend falls back to OpenGL ES 2.0 emulation through OpenGL 2.0. The default
   * parameters for major and minor should be 3 and 2 respectively to be compatible with Mac OS X.
   * Specifying major version 4 and minor version 2 will ensure that all OpenGL ES 3.0 features are
   * supported. Note however that Mac OS X does only support 3.2.
   *
   * @see <a href= "http://legacy.lwjgl.org/javadoc/org/lwjgl/opengl/ContextAttribs.html"> LWJGL OSX
   *     ContextAttribs note</a>
   ...
   */
  public void setOpenGLEmulation(
      GLEmulation glVersion, int gles3MajorVersion, int gles3MinorVersion) {
  ...
  /**
   * Set transparent window hint. Results may vary on different OS and GPUs. Usage with the ANGLE
   * backend is less consistent.
   *
  ...
  /**
   * Sets the target framerate for the application. The CPU sleeps as needed. Must be positive. Use
   * 0 to never sleep. Default is 0.
   */
  ...
  /**
   * Sets the directory where {@link Preferences} will be stored, as well as the file type to be
   * used to store them. Defaults to "$USER_HOME/.prefs/" and {@link FileType#External}.
   */
  public void setPreferencesConfig(
  ...
  /**
   * Defines how HDPI monitors are handled. Operating systems may have a per-monitor HDPI scale
   * setting. The operating system may report window width/height and mouse coordinates in a logical
   * coordinate system at a lower resolution than the actual physical resolution. This setting
   * allows you to specify whether you want to work in logical or raw pixel units. See {@link
   * HdpiMode} for more information. Note that some OpenGL functions like {@link
   * GL20#glViewport(int, int, int, int)} and {@link GL20#glScissor(int, int, int, int)} require raw
   * pixel units. Use {@link HdpiUtils} to help with the conversion if HdpiMode is set to {@link
   * HdpiMode#Logical}. Defaults to {@link HdpiMode#Logical}.
   */
  ...
  /**
   * Enables use of OpenGL debug message callbacks. If not supported by the core GL driver (since GL
   * 4.3), this uses the KHR_debug, ARB_debug_output or AMD_debug_output extension if available. By
   * default, debug messages with NOTIFICATION severity are disabled to avoid log spam.
   *
   * <p>You can call with {@link System#err} to output to the "standard" error output stream.
   *
   * <p>Use {@link
   * Lwjgl3Application#setGLDebugMessageControl(Lwjgl3Application.GLDebugMessageSeverity, boolean)}
  ...
  public static DisplayMode getDisplayMode(Monitor monitor) {
    ...
    return new Lwjgl3Graphics.Lwjgl3DisplayMode(
        ((Lwjgl3Monitor) monitor).monitorHandle,
        videoMode.width(),
        videoMode.height(),
        videoMode.refreshRate(),
        videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
  ...
  public static DisplayMode[] getDisplayModes() {
    ...
    for (int i = 0; i < result.length; i++) {
      ...
      result[i] =
          new Lwjgl3Graphics.Lwjgl3DisplayMode(
              GLFW.glfwGetPrimaryMonitor(),
              videoMode.width(),
              videoMode.height(),
              videoMode.refreshRate(),
