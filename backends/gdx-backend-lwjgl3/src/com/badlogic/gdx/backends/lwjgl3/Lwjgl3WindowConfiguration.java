...
public class Lwjgl3WindowConfiguration {
  ...
  /**
   * @param decorated whether the windowed mode window is decorated, i.e. displaying the title bars
   *     (default true)
   */
  ...
  /**
   * @param maximized whether the window starts maximized. Ignored if the window is full screen.
   *     (default false)
   */
  ...
  /**
   * @param autoIconify whether the window should automatically iconify and restore previous video
   *     mode on input focus loss. (default true) Does nothing in windowed mode.
   */
  ...
  /**
   * Sets the position of the window in windowed mode. Default -1 for both coordinates for centered
   * on primary monitor.
   */
  ...
  /**
   * Sets minimum and maximum size limits for the window. If the window is full screen or not
   * resizable, these limits are ignored. The default for all four parameters is -1, which means
   * unrestricted.
   */
  ...
  /**
   * Sets the icon that will be used in the window's title bar. Has no effect in macOS, which
   * doesn't use window icons.
   *
   * @param filePaths One or more {@linkplain FileType#Internal internal} image paths. Must be JPEG,
   *     PNG, or BMP format. The one closest to the system's desired size will be scaled. Good sizes
   ...
   * @param filePaths One or more image paths, relative to the given {@linkplain FileType}. Must be
   *     JPEG, PNG, or BMP format. The one closest to the system's desired size will be scaled. Good
  ...
  /**
   * Sets the {@link Lwjgl3WindowListener} which will be informed about iconficiation, focus loss
  ...
  /**
   * Sets the app to use fullscreen mode. Use the static methods like {@link
   * Lwjgl3ApplicationConfiguration#getDisplayMode()} on this class to enumerate connected monitors
  ...
  /**
   * Sets whether to use vsync. This setting can be changed anytime at runtime via {@link
   * Graphics#setVSync(boolean)}.
   *
   * <p>For multi-window applications, only one (the main) window should enable vsync. Otherwise,
   * every window will wait for the vertical blank on swap individually, effectively cutting the
   * frame rate to (refreshRate / numberOfWindows).
   */
