...
public class Lwjgl3Graphics extends AbstractGraphics implements Disposable {
      ...
      try {
        this.gl20 =
            window.getConfig().glEmulation == Lwjgl3ApplicationConfiguration.GLEmulation.GL20
                ? new Lwjgl3GL20()
                : (GL20)
                    Class.forName("com.badlogic.gdx.backends.lwjgl3.angle.Lwjgl3GLES20")
                        .newInstance();
      } catch (Throwable t) {
    ...
    if (!isFullscreen()) {
      ...
      if (width != logicalWidth || height != logicalHeight) {
        ...
        GLFW.glfwGetMonitorWorkarea(
            monitor.monitorHandle, tmpBuffer, tmpBuffer2, tmpBuffer3, tmpBuffer4);
      ...
      if (centerWindow) {
        window.setPosition(
            newX, newY); // on macOS the centering has to happen _after_ the new window size was set
      }
    } else {
      if (displayModeBeforeFullscreen == null) {
        storeCurrentWindowPositionAndDisplayMode();
      }
      if (width != windowWidthBeforeFullscreen
          || height != windowHeightBeforeFullscreen) { // Center window
        Lwjgl3Monitor monitor = (Lwjgl3Monitor) getMonitor();
        GLFW.glfwGetMonitorWorkarea(
            monitor.monitorHandle, tmpBuffer, tmpBuffer2, tmpBuffer3, tmpBuffer4);
        GLFW.glfwSetWindowMonitor(
   ...
   * positive. The cpu sleeps as needed. Use 0 to never sleep. If there are multiple windows, the
   * value for the first window created is used for all. Default is 0.
   *
   * @param fps fps
   */
  @Override
