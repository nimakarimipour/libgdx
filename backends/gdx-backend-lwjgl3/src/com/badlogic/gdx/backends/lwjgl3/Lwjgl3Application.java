...
public class Lwjgl3Application implements Lwjgl3ApplicationBase {
  ...
  /**
   * Creates a new {@link Lwjgl3Window} using the provided listener and {@link
   * Lwjgl3WindowConfiguration}.
   *
   * <p>This function only just instantiates a {@link Lwjgl3Window} and returns immediately. The
   * actual window creation is postponed with {@link Application#postRunnable(Runnable)} until after
   * all existing windows are updated.
   */
  ...
  private Lwjgl3Window createWindow(
  ...
  void createWindow(
      Lwjgl3Window window, Lwjgl3ApplicationConfiguration config, long sharedContext) {
  ...
  static long createGlfwWindow(Lwjgl3ApplicationConfiguration config, long sharedContextWindow) {
    ...
    GLFW.glfwWindowHint(
        GLFW.GLFW_MAXIMIZED, config.windowMaximized ? GLFW.GLFW_TRUE : GLFW.GLFW_FALSE);
    GLFW.glfwWindowHint(
        GLFW.GLFW_AUTO_ICONIFY, config.autoIconify ? GLFW.GLFW_TRUE : GLFW.GLFW_FALSE);
    ...
    if (config.fullscreenMode == null) {
      if (config.windowX == -1 && config.windowY == -1) {
        ...
        GLFW.glfwSetWindowPos(
            windowHandle,
            Math.max(0, areaXPos.get(0) + areaWidth.get(0) / 2 - windowWidth / 2),
  ...
  public enum GLDebugMessageSeverity {
    ...
    NOTIFICATION(
        GL43.GL_DEBUG_SEVERITY_NOTIFICATION, KHRDebug.GL_DEBUG_SEVERITY_NOTIFICATION, -1, -1);
  ...
  /**
   * Enables or disables GL debug messages for the specified severity level. Returns false if the
   * severity level could not be set (e.g. the NOTIFICATION level is not supported by the ARB and
   * AMD extensions).
   *
   * <p>See {@link Lwjgl3ApplicationConfiguration#enableGLDebugOutput(boolean, PrintStream)}
   */
  public static boolean setGLDebugMessageControl(GLDebugMessageSeverity severity, boolean enabled) {
    ...
    if (caps.OpenGL43) {
      GL43.glDebugMessageControl(
          GL_DONT_CARE, GL_DONT_CARE, severity.gl43, (IntBuffer) null, enabled);
      return true;
    }

    if (caps.GL_KHR_debug) {
      KHRDebug.glDebugMessageControl(
          GL_DONT_CARE, GL_DONT_CARE, severity.khr, (IntBuffer) null, enabled);
      return true;
    }

    if (caps.GL_ARB_debug_output && severity.arb != -1) {
      ARBDebugOutput.glDebugMessageControlARB(
          GL_DONT_CARE, GL_DONT_CARE, severity.arb, (IntBuffer) null, enabled);
      return true;
    }
