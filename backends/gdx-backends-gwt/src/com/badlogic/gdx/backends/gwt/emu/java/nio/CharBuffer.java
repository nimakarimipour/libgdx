 ...
 * <li>Use {@link java.nio.ByteBuffer#asCharBuffer() ByteBuffer.asCharBuffer} to create a char buffer based on a byte buffer.
 * </ul>
 *
 * @since Android 1.0 */
public abstract class CharBuffer extends Buffer implements Comparable<CharBuffer>, CharSequence, Appendable { // , Readable {
	 ...
	 * position() + end}, capacity will be the same as this buffer. The new buffer's read-only property and byte order are the same
	 * as this buffer.
	 *
	 * <p>
	 * The new buffer shares its content with this buffer, which means either buffer's change of content will be visible to the
	 ...
	 *           and not greater than the value obtained from a call to {@code
	 *     remaining()}.
	 ...
	 *
	 * <p>
	 * Calling this method has the same effect as {@code append(csq.toString())}. If the {@code
