...
/**
 * {@code Locale} represents a language/country/variant combination. Locales are used to alter the
 * presentation of information such as numbers or dates to suit the conventions in the region they
 * describe.
 *
 * <p>The language codes are two-letter lowercase ISO language codes (such as "en") as defined by <a
 * href="http://en.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>. The country codes are two-letter
 * uppercase ISO country codes (such as "US") as defined by <a
 * href="http://en.wikipedia.org/wiki/ISO_3166-1_alpha-3">ISO 3166-1</a>. The variant codes are
 * unspecified.
 *
 * <p>Note that Java uses several deprecated two-letter codes. The Hebrew ("he") language code is
 * rewritten as "iw", Indonesian ("id") as "in", and Yiddish ("yi") as "ji". This rewriting happens
 * even if you construct your own {@code Locale} object, not just for instances returned by the
 * various lookup methods.
 *
 * <p><a name="available_locales">
 *
 ...
 * and countries that don't exist, and you can create instances for combinations that don't exist
 * (such as "de_US" for "German as spoken in the US").
 *
 ...
 * available.
 *
 * <p>It is also a mistake to assume that all devices have the same locales available. A device sold
 * in the US will almost certainly support en_US and es_US, but not necessarily any locales with the
 ...
 *
 * <p><a name="locale_data">
 *
 * <h3>Locale data</h3>
 *
 * </a>
 *
 * <p>Note that locale data comes solely from ICU. User-supplied locale service providers (using the
 * {@code java.text.spi} or {@code java.util.spi} mechanisms) are not supported.
 *
 * <p>Here are the versions of ICU (and the corresponding CLDR and Unicode versions) used in various
 * Android releases:
 *
 ...
 *
 * <h3>Be wary of the default locale</h3>
 *
 * </a>
 *
 ...
 *
 * <p>The default locale is appropriate for tasks that involve presenting data to the user. In this
 ...
 *
 * <p>For example, if you're formatting integers some locales will use non-ASCII decimal digits. As
 * another example, if you're formatting floating-point numbers some locales will use {@code ','} as
 * the decimal point and {@code '.'} for digit grouping. That's correct for human-readable output,
 * but likely to cause problems if presented to another computer ({@link Double#parseDouble} can't
 * parse such a number, for example). You should also be wary of the {@link String#toLowerCase} and
...
public final class Locale implements Cloneable, Serializable {
  ...
  /**
   * Locale constant for the root locale. The root locale has an empty language, country, and
   * variant.
   *
   * @since 1.6
   */
  ...
  /**
   * There's a circular dependency between toLowerCase/toUpperCase and Locale.US. Work around this
   * by avoiding these methods when constructing the built-in locales.
   *
  ...
  /**
   * Returns the country code for this locale, or {@code ""} if this locale doesn't correspond to a
   * specific country.
   */
  ...
  /**
   * Returns the user's preferred locale. This may have been overridden for this process with {@link
   * #setDefault}.
   *
   * <p>Since the user's locale changes dynamically, avoid caching this value. Instead, use this
   * method to look it up for each use.
   */
  ...
  private static Locale initDefault() {
    ...
    if (browserLanguage != null && browserLanguage.length() > 0) {
      ...
      defaultLoc =
          new Locale(
              true, locale[0].toLowerCase(), locale.length > 1 ? locale[1].toUpperCase() : "");
    }
  ...
  /**
   * Returns the variant code for this {@code Locale} or an empty {@code String} if no variant was
   * set.
   */
  ...
  /**
   * Overrides the default locale. This does not affect system configuration, and attempts to
   * override the system-provided default locale may themselves be overridden by actual changes to
   * the system configuration. Code that calls this method is usually incorrect, and should be fixed
   * by passing the appropriate locale to each locale-sensitive method that's called.
   */
  ...
  /**
   * Returns the string representation of this {@code Locale}. It consists of the language code,
   * country code and variant separated by underscores. If the language is missing the string begins
   * with an underscore. If the country is missing there are 2 underscores between the language and
   * the variant. The variant cannot stand alone without a language and/or country code: in this
