...
public class WebAudioAPIManager implements LifecycleListener {
  ...
  public WebAudioAPIManager() {
     ...
     */
    if (isAudioContextLocked(audioContext)) hookUpSoundUnlockers();
    else setUnlocked();
  }
  ...
  public Sound createSound(FileHandle fileHandle) {
    final WebAudioAPISound newSound =
        new WebAudioAPISound(audioContext, globalVolumeNode, audioControlGraphPool);
  ...
  public static native void decodeAudioData(
      JavaScriptObject audioContextIn, ArrayBuffer audioData, WebAudioAPISound targetSound) /*-{
		audioContextIn
				.decodeAudioData(
						audioData,
						function(buffer) {
