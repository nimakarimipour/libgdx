...
public class Preloader {
  ...
  public void preload(final String assetFileUrl, final PreloaderCallback callback) {
    ...
    loader.loadText(
        baseUrl + assetFileUrl + "?etag=" + System.currentTimeMillis(),
        new AssetLoaderListener<String>() {
          ...
          public void onSuccess(String result) {
            ...
            for (String line : lines) {
              ...
              Asset asset =
                  new Asset(assetPathOrig.trim(), assetPathMd5.trim(), type, size, assetMimeType);
              assetNames.put(asset.file, asset.url);
              if (assetPreload || asset.file.startsWith("com/badlogic/")) assets.add(asset);
              else stillToFetchAssets.put(asset.file, asset);
            }
  ...
  public boolean contains(String file) {
    return texts.containsKey(file)
        ...
        || binaries.containsKey(file)
        || audio.containsKey(file)
  ...
  private boolean isChild(String filePath, String directory) {
    return filePath.startsWith(directory + "/")
        && (filePath.indexOf('/', directory.length() + 1) < 0);
  }

  public FileHandle[] list(final String file) {
    return getMatchedAssetFiles(
        new FilePathFilter() {
          @Override
  ...
  public FileHandle[] list(final String file, final FilenameFilter filter) {
    return getMatchedAssetFiles(
        new FilePathFilter() {
          ...
          public boolean accept(String path) {
            return isChild(path, file)
                && filter.accept(new File(file), path.substring(file.length() + 1));
          }
        });
  }
