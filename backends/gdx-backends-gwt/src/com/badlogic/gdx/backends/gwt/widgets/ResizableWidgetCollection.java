...
/**
 * A collection of {@link ResizableWidget} that periodically checks the outer dimensions of a widget
 * and redraws it as necessary. Every {@link ResizableWidgetCollection} uses a timer, so consider
 * the cost when adding one.
 *
 * <p>Typically, a {@link ResizableWidgetCollection} is only needed if you expect your widgets to
 * resize based on window resizing or other events. Fixed sized Widgets do not need to be added to a
 * {@link ResizableWidgetCollection} as they cannot be resized.
 */
public class ResizableWidgetCollection implements ResizeHandler, Iterable<ResizableWidget> {
  ...
  /**
   * Get the globally accessible {@link ResizableWidgetCollection}. In most cases, the global
   * collection can be used for all {@link ResizableWidget}s.
   *
  ...
  /**
   * Set whether resize checking is enabled. If disabled, elements will still be resized on window
   * events, but the timer will not check their dimensions periodically.
   *
  ...
  /**
   * Inform the {@link ResizableWidgetCollection} that the size of a widget has changed and already
   * been redrawn. This will prevent the widget from being redrawn on the next loop.
   *
