...
/**
 * Copies assets from the path specified in the modules gdx.assetpath configuration property to the
 * war/ folder and generates the assets.txt file. The type of a file is determined by an {@link
 * AssetFilter}, which is either created by instantiating the class specified in the
 * gdx.assetfilterclass property, or falling back to the {@link DefaultAssetFilter}.
 *
 * @author mzechner
 */
public class PreloaderBundleGenerator extends Generator {
  ...
  public String generate(TreeLogger logger, GeneratorContext context, String typeName)
    ...
    FileWrapper target =
        new FileWrapper("assets/"); // this should always be the war/ directory of the GWT project.
  ...
  private void copyDirectory(
      FileWrapper sourceDir, FileWrapper destDir, AssetFilter filter, ArrayList<Asset> assets) {
  ...
  private List<String> getClasspathFiles(GeneratorContext context) {
    ...
    try {
      ConfigurationProperty prop =
          context.getPropertyOracle().getConfigurationProperty("gdx.files.classpath");
