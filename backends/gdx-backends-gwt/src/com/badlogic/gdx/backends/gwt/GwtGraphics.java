...
public class GwtGraphics extends AbstractGraphics {
  ...
  private native boolean setFullscreenJSNI(
      GwtGraphics graphics, CanvasElement element, int screenWidth, int screenHeight) /*-{
  ...
  public DisplayMode getDisplayMode() {
    ...
    return new DisplayMode(
        (int) (getScreenWidthJSNI() * density),
        (int) (getScreenHeightJSNI() * density),
        60,
        getColorDepthJSNI()) {};
  }
  ...
  public boolean supportsExtension(String extensionName) {
    // Contrary to regular OpenGL, WebGL extensions need to be explicitly enabled before they can be
    // used. See
    // https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API/Using_Extensions
    // Thus, it is not safe to use an extension just because context.getSupportedExtensions() tells
    // you it is available.
  ...
  /**
   * See https://developer.mozilla.org/en-US/docs/Web/API/Window/devicePixelRatio for more
   * information
   *
   * @return value indicating the ratio of the display's resolution in physical pixels to the
   *     resolution in CSS pixels. A value of 1 indicates a classic 96 DPI (76 DPI on some
   *     platforms) display, while a value of 2 is expected for HiDPI/Retina displays.
   */
  ...
  public void setCursor(Cursor cursor) {
    ((GwtApplication) Gdx.app)
        .graphics
        .canvas
        .getStyle()
        .setProperty("cursor", ((GwtCursor) cursor).cssCursorProperty);
  }
  ...
  public void setSystemCursor(SystemCursor systemCursor) {
    ((GwtApplication) Gdx.app)
        ...
        .canvas
        .getStyle()
        .setProperty("cursor", GwtCursor.getNameForSystemCursor(systemCursor));
  }
