...
public class DefaultGwtInput extends AbstractInput implements GwtInput {
  ...
  public DefaultGwtInput(CanvasElement canvas, GwtApplicationConfiguration config) {
    ...
    if (config.useAccelerometer && GwtFeaturePolicy.allowsFeature(GwtAccelerometer.PERMISSION)) {
      if (GwtApplication.agentInfo().isFirefox()) {
        ...
        GwtPermissions.queryPermission(
            GwtAccelerometer.PERMISSION,
            new GwtPermissions.GwtPermissionResult() {
              @Override
              public void granted() {
                setupAccelerometer();
              }
  ...
  /**
   * works only for Chrome > Version 18 with enabled Mouse Lock enable in about:flags or start
   * Chrome with the --enable-pointer-lock flag
   */
  @Override
  ...
  static native void addEventListener(
      JavaScriptObject target, String name, DefaultGwtInput handler, boolean capture) /*-{
		target
				.addEventListener(
						name,
						function(e) {
  ...
  protected int getRelativeX(NativeEvent e, CanvasElement target) {
    float xScaleRatio =
        target.getWidth() * 1f / target.getClientWidth(); // Correct for canvas CSS scaling
    return Math.round(
        xScaleRatio
            * (e.getClientX()
  ...
  protected int getRelativeY(NativeEvent e, CanvasElement target) {
    float yScaleRatio =
        target.getHeight() * 1f / target.getClientHeight(); // Correct for canvas CSS scaling
    return Math.round(
        yScaleRatio
            * (e.getClientY()
