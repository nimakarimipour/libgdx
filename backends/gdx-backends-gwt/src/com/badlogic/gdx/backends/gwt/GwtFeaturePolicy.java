 ...
 * href="https://w3c.github.io/webappsec-feature-policy/#featurepolicy">Feature Policy Interface</a>
 */
class GwtFeaturePolicy {
  ...
  static String[] features() {
    if (GwtFeaturePolicy.isSupported()) return toStringArray(JSfeatures());
    else return null;
  }
  ...
  static String[] allowedFeatures() {
    if (GwtFeaturePolicy.isSupported()) return toStringArray(JSallowedFeatures());
    else return null;
  }

  private static native JsArrayString JSgetAllowlistForFeature(String feature) /*-{
		return $wnd.document.featurePolicy.getAllowlistForFeature(feature);
	}-*/;

  static String[] getAllowlistForFeature(String feature) {
    if (GwtFeaturePolicy.isSupported()) return toStringArray(JSgetAllowlistForFeature(feature));
    else return null;
  }
}
