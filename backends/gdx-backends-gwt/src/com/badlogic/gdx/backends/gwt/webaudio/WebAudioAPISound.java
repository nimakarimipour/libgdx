...
/**
 * Implementation of the {@link Sound} interface for GWT, using the Web Audio API ( <a
 * href="https://www.w3.org/TR/webaudio/">https://www.w3.org/TR/webaudio/</a>).
 *
 * @author barkholt
 */
public class WebAudioAPISound implements Sound {
  ...
  // JavaScript AudioNode representing the final destination of the sound. Typically the speakers of
  // whatever device we are
  // running on.
  private final JavaScriptObject destinationNode;

  // Maps from integer keys to active sounds. Both the AudioBufferSourceNode and the associated
  // AudioControlGraph are stored for
  // quick access
  ...
  /**
   * @param audioContext The JavaScript AudioContext object that servers as the base object of the
   *     Web Audio API
   ...
   */
  public WebAudioAPISound(
      JavaScriptObject audioContext,
      JavaScriptObject destinationNode,
  ...
  public void setAudioBuffer(JavaScriptObject audioBuffer) {
    ...
    // If play-back of sounds have been requested before we were ready, do a pause/resume to get
    // sound flowing
  ...
  protected long play(float volume, float pitch, float pan, boolean loop) {
    ...
    if (!WebAudioAPIManager.isSoundUnlocked()
        && WebAudioAPIManager.isAudioContextLocked(audioContext)) return -1;
  ...
  public native JavaScriptObject playJSNI(
      JavaScriptObject source, int key, float startOffset) /*-{
		// Get the Java values here, for readability
		var audioContext = this.@com.badlogic.gdx.backends.gwt.webaudio.WebAudioAPISound::audioContext;
  ...
  public void pause(long soundId) {
    ...
    if (activeSounds.containsKey(soundKey)) {
      ...
      // The API has no concept of pause/resume, so we do it by recording a pause time stamp, and
      // then stopping the sound. On
      // resume we play the
  ...
  public static native JavaScriptObject setPitchJSNI(
      JavaScriptObject audioBufferSourceNode, float pitch) /*-{
  ...
  public static native JavaScriptObject setLoopingJSNI(
      JavaScriptObject audioBufferSourceNode, boolean looping) /*-{
