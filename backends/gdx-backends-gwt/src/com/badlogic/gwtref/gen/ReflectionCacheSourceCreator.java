...
public class ReflectionCacheSourceCreator {
  private static final List<String> PRIMITIVE_TYPES =
      Collections.unmodifiableList(
          Arrays.asList("char", "int", "long", "byte", "short", "float", "double", "boolean"));
  ...
  public ReflectionCacheSourceCreator(
      TreeLogger logger, GeneratorContext context, JClassType type, List<JType> types) {
    this.logger = logger;
  ...
  private void createProxy(JClassType type) {
    ClassSourceFileComposerFactory composer =
        new ClassSourceFileComposerFactory(
            type.getPackage().getName(), type.getSimpleSourceName() + "Proxy");
    PrintWriter printWriter = context.tryCreate(logger, packageName, simpleName);
    if (printWriter == null) {
      return;
    }
  ...
  private void generateLookups() {
    ...
    Collections.sort(
        setterGetterStubs,
        new Comparator<SetterGetterStub>() {
          @Override
  ...
  private String generateMethodStub(MethodStub stub) {
    ...
    if ((stub.enclosingType.startsWith("java") && !stub.enclosingType.startsWith("java.util"))
      ...
      logger.log(
          Type.INFO,
          "not emitting code for accessing method "
              + stub.name
              + " in class '"
              + stub.enclosingType
  ...
  private String generateSetterGetterStub(SetterGetterStub stub) {
    ...
    if (stub.enclosingType.startsWith("java") || stub.enclosingType.contains("google")) {
      logger.log(
          Type.INFO,
          "not emitting code for accessing field "
              + stub.name
              + " in class '"
              + stub.enclosingType
              + ", either in java.* or GWT related class");
    ...
    pbn(
        "private native "
            ...
            + stub.enclosingType
            + " obj) /*-{");
    if (stub.isStatic) pbn("return @" + stub.enclosingType + "::" + stub.name + ";");
    else pbn("return obj.@" + stub.enclosingType + "::" + stub.name + ";");
    pb("}-*/;");
    ...
    if (!stub.isFinal) {
      pbn(
          "private native void s"
              ...
              + stub.type
              + " value)  /*-{");
      if (stub.isStatic) pbn("@" + stub.enclosingType + "::" + stub.name + " = value");
      else pbn("obj.@" + stub.enclosingType + "::" + stub.name + " = value;");
      pb("}-*/;");
    }
  ...
  private String createTypeGenerator(JType t) {
    ...
    if (c != null && c.getFlattenedSupertypeHierarchy() != null) {
      ...
      for (JType i : c.getFlattenedSupertypeHierarchy()) {
        if (!isVisible(i)
            || i.equals(t)
            || "java.lang.Object".equals(i.getErasedType().getQualifiedSourceName())) continue;
        if (used) assignables += ", ";
    ...
    if (c != null) {
      ...
      if (annotations != null && annotations.length > 0) {
        ...
        if (!"null"
            .equals(annotations1)) { // avoid nulling the default empty array for annotations.
  ...
  private void createTypeInvokables(
      JClassType c, String varName, String methodType, JAbstractMethod[] methodTypes) {
      ...
      for (JAbstractMethod m : methodTypes) {
        ...
        if (m.getParameters() != null && m.getParameters().length > 0) {
          ...
          for (JParameter p : m.getParameters()) {
            ...
            String paramName =
                (p.getName() + "__" + p.getType().getErasedType().getJNISignature())
                    .replaceAll("[/;\\[\\]]", "_");
            String paramInstantiation =
                "new Parameter(\""
                    + p.getName()
                    + "\", "
                    + getType(p.getType())
  ...
  private String getElementTypes(JField f) {
    ...
    if (params != null) {
      ...
      for (JClassType typeArg : typeArgs) {
        if (typeArg.isWildcard() != null) b.append("null");
        else if (!isVisible(typeArg)) b.append("null");
  ...
  private String getAnnotations(Annotation[] annotations) {
    if (annotations != null && annotations.length > 0) {
      ...
      final Class<?>[] ignoredAnnotations = {
        Nonnull.class, Nullable.class, Deprecated.class, Retention.class,
      };
      StringBuilder b = new StringBuilder();
  ...
  private void addParameters(MethodStub stub) {
    ...
    for (int i = 0; i < stub.parameterTypes.size(); i++) {
      pbn(
          cast(stub.parameterTypes.get(i), "params[" + i + "]")
  ...
  private void setArrayElementT() {
    ...
    for (String s : PRIMITIVE_TYPES) {
      ...
      pc.add(
          typeNames2typeIds.get(s + "[]"),
          "((" + s + "[])obj)[i] = " + cast(s, "value") + "; return;");
    }
