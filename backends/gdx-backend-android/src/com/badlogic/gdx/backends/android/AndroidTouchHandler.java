...
/**
 * Multitouch handler for devices running Android >= 2.0. If device is capable of (fake) multitouch
 * this will report additional pointers.
 *
...
public class AndroidTouchHandler {
    ...
    synchronized (input) {
      switch (action) {
        ...
        case MotionEvent.ACTION_POINTER_DOWN:
          realPointerIndex =
              input
                  .getFreePointerIndex(); // get a free pointer index as reported by Input.getX()
                                          // etc.
        ...
        case MotionEvent.ACTION_CANCEL:
          ...
          if (button != -1) {
            if (action == MotionEvent.ACTION_CANCEL)
              postTouchEvent(
                  input, TouchEvent.TOUCH_CANCELLED, x, y, realPointerIndex, button, timeStamp);
            else
        ...
        case MotionEvent.ACTION_MOVE:
          ...
          for (int i = 0; i < pointerCount; i++) {
            ...
            if (button != -1)
              postTouchEvent(
                  input, TouchEvent.TOUCH_DRAGGED, x, y, realPointerIndex, button, timeStamp);
            else
  ...
  private void logAction(int action, int pointer) {
    ...
    Gdx.app.log(
        "AndroidMultiTouchHandler", "action " + actionStr + ", Android pointer id: " + pointer);
  }
  ...
  private void postTouchEvent(
      DefaultAndroidInput input, int type, int x, int y, int pointer, int button, long timeStamp) {
