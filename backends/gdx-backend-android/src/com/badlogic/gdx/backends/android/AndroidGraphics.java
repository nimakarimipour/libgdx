...
public class AndroidGraphics extends AbstractGraphics implements Renderer {
   ...
   * {@link AndroidGraphics#pause()} they <b>MUST</b> enforce continuous rendering. If not, {@link
   * #onDrawFrame(GL10)} will not be called in the GLThread while {@link #pause()} is sleeping in
   * the Android UI Thread which will cause the {@link AndroidGraphics#pause} variable never be set
   * to false. As a result, the {@link AndroidGraphics#pause()} method will kill the current process
   * to avoid ANR
   */
  ...
  public AndroidGraphics(
      AndroidApplicationBase application,
  ...
  protected GLSurfaceView20 createGLSurfaceView(
      AndroidApplicationBase application, final ResolutionStrategy resolutionStrategy) {
    if (!checkGL20()) throw new GdxRuntimeException("libGDX requires OpenGL ES 2.0");

    EGLConfigChooser configChooser = getEglConfigChooser();
    GLSurfaceView20 view =
        new GLSurfaceView20(application.getContext(), resolutionStrategy, config.useGL30 ? 3 : 2);
  ...
  protected EGLConfigChooser getEglConfigChooser() {
    return new GdxEglConfigChooser(
        config.r, config.g, config.b, config.a, config.depth, config.stencil, config.numSamples);
  }
  ...
  /**
   * This instantiates the GL10, GL11 and GL20 instances. Includes the check for certain devices
   * that pretend to support GL11 but fuck up vertex buffer objects. This includes the pixelflinger
   * which segfaults when buffers are deleted as well as the Motorola CLIQ and the Samsung Behold
   * II.
   *
   * @param gl
   */
  ...
  public void onDrawFrame(javax.microedition.khronos.opengles.GL10 gl) {
    ...
    // After pause deltaTime can have somewhat huge value that destabilizes the mean, so let's cut
    // it off
    if (!resume) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
      try {
        DisplayCutout displayCutout =
            app.getApplicationWindow().getDecorView().getRootWindowInsets().getDisplayCutout();
      ...
      } // Some Application implementations (such as Live Wallpapers) do not implement
  ...
  public DisplayMode getDisplayMode() {
    ...
    if (Build.VERSION.SDK_INT >= 17) {
      DisplayManager displayManager =
          (DisplayManager) app.getContext().getSystemService(Context.DISPLAY_SERVICE);
