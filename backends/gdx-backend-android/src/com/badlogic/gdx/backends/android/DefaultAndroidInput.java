...
public class DefaultAndroidInput extends AbstractInput implements AndroidInput {
          ...
          public void run() {
            ...
            if (visible) {
              ...
              manager.hideSoftInputFromWindow(
                  ((AndroidGraphics) app.getGraphics()).getView().getWindowToken(), 0);
            }
          }
        });
  }
  ...
  private void updateOrientation() {
    ...
    } else if (!SensorManager.getRotationMatrix(
   ...
   * float[])</a>. Does not manipulate the matrix if the platform does not have an accelerometer and
   * compass, or a rotation vector sensor.
   *
   * @param matrix
   */
  ...
  void registerSensorListeners() {
    if (config.useAccelerometer) {
      ...
      if (manager.getSensorList(Sensor.TYPE_ACCELEROMETER).isEmpty()) {
        ...
        accelerometerAvailable =
            manager.registerListener(accelerometerListener, accelerometer, config.sensorDelay);
      }
    ...
    if (config.useGyroscope) {
      ...
      if (manager.getSensorList(Sensor.TYPE_GYROSCOPE).isEmpty()) {
        ...
        gyroscopeAvailable =
            manager.registerListener(gyroscopeListener, gyroscope, config.sensorDelay);
      }
        ...
        for (Sensor sensor : rotationVectorSensors) { // favor AOSP sensor
          if (sensor.getVendor().equals("Google Inc.") && sensor.getVersion() == 3) {
            rotationVectorAvailable =
                manager.registerListener(rotationVectorListener, sensor, config.sensorDelay);
            break;
          }
        }
        if (!rotationVectorAvailable)
          rotationVectorAvailable =
              manager.registerListener(
                  rotationVectorListener, rotationVectorSensors.get(0), config.sensorDelay);
      }
    }
  ...
  /**
   * Our implementation of SensorEventListener. Because Android doesn't like it when we register
   * more than one Sensor to a single SensorEventListener, we add one of these for each Sensor.
   * Could use an anonymous class, but I don't see any harm in explicitly defining it here. Correct
   * me if I am wrong.
   */
