 ...
 *
 * <p>Derive from this class. In the {@link AndroidLiveWallpaperService#onCreateApplication} method
 * call the {@link AndroidLiveWallpaperService#initialize(ApplicationListener)} method specifying
 * the configuration for the GLSurfaceView. You can also use {@link AndroidWallpaperListener} along
 * with {@link ApplicationListener} to respond for wallpaper specific events in your app listener:
 *
 * <p>MyAppListener implements ApplicationListener, AndroidWallpaperListener
 *
 * <p>Notice: Following methods are not called for live wallpapers: {@link
 * ApplicationListener#pause()} {@link ApplicationListener#dispose()} TODO add callbacks to
 * AndroidWallpaperListener allowing to notify app listener about changed visibility state of live
 * wallpaper but called from main thread, not from GL thread: for example:
 * AndroidWallpaperListener.visibilityChanged(boolean)
 *
 * <p>//obsoleted: //Notice! //You have to kill all not daemon threads you created in {@link
 * ApplicationListener#pause()} method. // {@link ApplicationListener#dispose()} is never called!
 * //If you leave live non daemon threads, wallpaper service wouldn't be able to close, //this can
 * cause problems with wallpaper lifecycle.
 *
 * <p>Notice #2! On some devices wallpaper service is not killed immediately after exiting from
 * preview. Service object is destroyed (onDestroy called) but process on which it runs remains
 * alive. When user comes back to wallpaper preview, new wallpaper service object is created, but in
 * the same process. It is important if you plan to use static variables / objects - they will be
 * shared between living instances of wallpaper services'! And depending on your implementation - it
 * can cause problems you were not prepared to.
 *
...
public abstract class AndroidLiveWallpaperService extends WallpaperService {
  ...
  // instance of libGDX Application, acts as singleton - one instance per application (per
  // WallpaperService)
  ...
  // app is initialized when engines == 1 first time, app is destroyed in
  // WallpaperService.onDestroy, but
  // ApplicationListener.dispose is not called for wallpapers
  protected int engines = 0;
  protected int visibleEngines = 0;

  // engine currently associated with app instance, linked engine serves surface handler for
  // GLSurfaceView
  protected volatile AndroidWallpaperEngine linkedEngine =
      null; // can be accessed from GL render thread by getSurfaceHolder
  ...
  /**
   * One of wallpaper engines is starting. Do not override this method, service manages them
   * internally.
   */
  @Override
  ...
  /**
   * libGDX application is starting, it occurs after first wallpaper engine had started. Override
   * this method an invoke {@link AndroidLiveWallpaperService#initialize(ApplicationListener,
   * AndroidApplicationConfiguration)} from there.
   */
  ...
  /**
   * Look at {@link AndroidLiveWallpaperService#initialize(ApplicationListener,
  ...
  /**
   * This method has to be called in the {@link AndroidLiveWallpaperService#onCreateApplication}
   * method. It sets up all the things necessary to get input, render via OpenGL and so on. You can
   * configure other aspects of the application with the rest of the fields in the {@link
   ...
   *     application (use accelerometer, etc.). Do not change contents of this object after passing
   *     to this method!
   */
  ...
  /**
   * Getter for SurfaceHolder object, surface holder is required to restore gl context in
   * GLSurfaceView
   */
  public SurfaceHolder getSurfaceHolder() {
    ...
    synchronized (sync) {
      if (linkedEngine == null) return null;
      else return linkedEngine.getSurfaceHolder();
    }
  }
  ...
  /**
   * Called when the last engine is ending its live, it can occur when: 1. service is dying 2.
   * service is switching from one engine to another 3. [only my assumption] when wallpaper is not
   * visible and system is going to restore some memory for foreground processing by disposing not
   * used wallpaper engine We can't destroy app there, because: 1. in won't work - gl context is
   * disposed right now and after app.onDestroy() app would stuck somewhere in gl thread
   * synchronizing code 2. we don't know if service create more engines, app is shared between them
   * and should stay initialized waiting for new engines
   */
  public void onDeepPauseApplication() {
    if (DEBUG) Log.d(TAG, " > AndroidLiveWallpaperService - onDeepPauseApplication()");

    // free native resources consuming runtime memory, note that it can cause some lag when resuming
    // wallpaper
    if (app != null) {
  ...
  /**
   * Service is dying, and will not be used again. You have to finish execution off all living
   * threads there or short after there, besides the new wallpaper service wouldn't be able to
   * start.
   */
  @Override
  ...
  /**
   * Bridge between surface on which wallpaper is rendered and the wallpaper service. The problem is
   * that there can be a group of Engines at one time and we must share libGDX application between
   * them.
   *
  ...
  public class AndroidWallpaperEngine extends Engine {
    ...
    public void onCreate(final SurfaceHolder surfaceHolder) {
      if (DEBUG)
        Log.d(
            TAG,
            " > AndroidWallpaperEngine - onCreate() "
                + hashCode()
                + " running: "
                + engines
                + ", linked: "
    ...
    /**
     * Called before surface holder callbacks (ex for GLSurfaceView)! This is called immediately
     * after the surface is first created. Implementations of this should start up whatever
     * rendering code they desire. Note that only one thread can ever draw into a Surface, so you
    ...
    public void onSurfaceCreated(final SurfaceHolder holder) {
      ...
      if (DEBUG)
        Log.d(
            TAG,
            " > AndroidWallpaperEngine - onSurfaceCreated() "
                + hashCode()
                + ", running: "
                + engines
                + ", linked: "
    ...
    public void onSurfaceChanged(
      ...
      if (DEBUG)
        Log.d(
            TAG,
            " > AndroidWallpaperEngine - onSurfaceChanged() isPreview: "
                + isPreview()
                + ", "
                + hashCode()
                + ", running: "
      ...
      // it shouldn't be required there (as I understand android.service.wallpaper.WallpaperService
      // impl)
      // notifyPreviewState();
    }
     ...
     * @param width
     * @param height
     * @param forceUpdate if false, surface view will be notified only if currently contains expired
     *     information
     */
    private void notifySurfaceChanged(
        final int format, final int width, final int height, boolean forceUpdate) {
    ...
    /**
     * Called to inform you of the wallpaper becoming visible or hidden. It is very important that a
    ...
    public void onVisibilityChanged(final boolean visible) {
      ...
      if (DEBUG)
        Log.d(
            TAG,
            " > AndroidWallpaperEngine - onVisibilityChanged(paramVisible: "
                + visible
                + " reportedVisible: "
                + reportedVisible
                + ") "
      ...
      // Android WallpaperService sends fake visibility changed events to force some buggy live
      // wallpapers to shut down after
      // onSurfaceChanged when they aren't visible, it can cause problems in current implementation
    ...
    private void notifyVisibilityChanged(final boolean visible) {
      if (this.engineIsVisible != visible) {
        ...
        if (this.engineIsVisible) onResume();
        else onPause();
      } else {
      ...
      if (linkedEngine != null) {
        if (linkedEngine != this) {
          ...
          view.surfaceDestroyed(
              this
                  .getSurfaceHolder()); // force gl surface reload, new instance will be created on
                                        // current
          // surface holder
    ...
    public void onPause() {
      ...
      // this shouldn't never happen, but if it will.. live wallpaper will not be stopped when
      // device will pause and lwp will
      // drain battery.. shortly!
      if (visibleEngines >= engines) {
        Log.e(
            AndroidLiveWallpaperService.TAG,
            "wallpaper lifecycle error, counted too many visible engines! repairing..");
    ...
    /**
     * Called after surface holder callbacks (ex for GLSurfaceView)! This is called immediately
     * before a surface is being destroyed. After returning from this call, you should no longer try
     * to access this surface. If you have a rendering thread that directly accesses the surface,
     * you must ensure that thread is no longer touching the Surface before returning from this
     * function.
     *
     * <p>Attention! In some cases GL context may be shutdown right now! and
     * SurfaceHolder.Surface.isVaild = false
     */
    @Override
    public void onSurfaceDestroyed(final SurfaceHolder holder) {
      ...
      if (DEBUG)
        Log.d(
            TAG,
            " > AndroidWallpaperEngine - onSurfaceDestroyed() "
                + hashCode()
                + ", running: "
                + engines
                + " ,linked: "
      ...
      // application can be in resumed state at this moment if app surface had been lost just after
      // it was created (wallpaper
    ...
    public Bundle onCommand(
      ...
      if (DEBUG)
        Log.d(
            TAG,
            " > AndroidWallpaperEngine - onCommand("
                + pAction
                + " "
                + pX
                + " "
    ...
    public void onOffsetsChanged(
      ...
      // if (DEBUG)
      // Log.d(TAG, " > AndroidWallpaperEngine - onOffsetChanged(" + xOffset + " " + yOffset + " " +
      // xOffsetStep + " "
      // + yOffsetStep + " " + xPixelOffset + " " + yPixelOffset + ") " + hashCode() + ", linkedApp:
      // " + (linkedApp != null));
    ...
    protected void notifyOffsetsChanged() {
      if (linkedEngine == this && app.listener instanceof AndroidWallpaperListener) {
        if (!offsetsConsumed) { // no need for more sophisticated synchronization - offsetsChanged
                                // can be called multiple
          // times and with various patterns on various devices - user application must be prepared
          // for that
              ...
              new Runnable() {
                ...
                public void run() {
                  ...
                  synchronized (sync) {
                    isCurrent =
                        (linkedEngine
                            == AndroidWallpaperEngine.this); // without this app can crash when fast
                    // switching between engines (tested!)
                  }
                  if (isCurrent)
                    ((AndroidWallpaperListener) app.listener)
                        .offsetChange(
                            xOffset, yOffset, xOffsetStep, yOffsetStep, xPixelOffset, yPixelOffset);
                }
              });
              ...
              public void run() {
                ...
                if (shouldNotify) {
                  AndroidLiveWallpaper currentApp =
                      app; // without this app can crash when fast switching between engines
                  // (tested!)
                  if (currentApp != null)
                    ((AndroidWallpaperListener) currentApp.listener)
                        .previewStateChange(currentPreviewState);
                }
              }
            });
      }
