 ...
 * AndroidLiveWallpaperService. Not directly constructable, instead the {@link
 * AndroidLiveWallpaperService} will create this class internally.
 *
 * @author mzechner
 */
public class AndroidLiveWallpaper implements AndroidApplicationBase {
  ...
  public void initialize(ApplicationListener listener, AndroidApplicationConfiguration config) {
    ...
    graphics =
        new AndroidGraphicsLiveWallpaper(
            this,
            config,
    ...
    // factory in use, but note: AndroidInputFactory causes exceptions when obfuscated:
    // java.lang.RuntimeException: Couldn't
    // construct AndroidInput, this should never happen, proguard deletes constructor used only by
    // reflection
    ...
    // Unlike activity, fragment and daydream applications there's no need for a specialized audio
    // listener.
  ...
  public void onPause() {
    ...
    // for example on Samsung Galaxy Tab (GT-P6800) on android 4.0.4 invoking graphics.pause causes
    // "Fatal Signal 11"
    ...
    // the problem with graphics.pause occurs while using OpenGL 2.0 and original GLSurfaceView
    // while rotating device
    // in lwp preview
    // in my opinion it is a bug of android not libgdx, even example Cubic live wallpaper from
    // Android SDK crashes on affected devices.......... and on some configurations of android
    // emulator too.
    //
    // My wallpaper was rejected on Samsung Apps because of this issue, so I decided to disable
    // graphics.pause..
    // also I moved audio lifecycle methods from AndroidGraphicsLiveWallpaper into this class

    // graphics.pause();
    // if (AndroidLiveWallpaperService.DEBUG)
    // Log.d(AndroidLiveWallpaperService.TAG, " > AndroidLiveWallpaper - onPause() application
    // paused!");
    audio.pause();
