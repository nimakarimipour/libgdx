...
public class AndroidZipFileHandle extends AndroidFileHandle {
  ...
  public FileHandle sibling(String name) {
    ...
    return Gdx.files.getFileHandle(
        new File(file.getParent(), name).getPath(), type); // this way we can find the sibling even
  ...
  public FileHandle[] list() {
    ...
    for (int i = 0, n = zipEntries.length; i < n; i++) {
      if (zipEntries[i].mFileName.length()
          == getPath().length()) // Don't include the directory itself
      continue;
