...
public class AssetManager implements Disposable {
    ...
    public synchronized <T> Array<T> getAll(Class<T> type, Array<T> out) {
        ObjectMap<String, RefCountedContainer> assetsByType = assets.get(type);
        if (assetsByType != null) {
            for (ObjectMap.Entry<String, RefCountedContainer> asset : assetsByType.entries()) out.add(asset.value.getObject(type));
        }
        return out;
    }
    ...
    public synchronized boolean contains(String fileName) {
        if (tasks.size() > 0 && tasks.firstElement().assetDesc.fileName.equals(fileName))
            return true;
        for (int i = 0; i < loadQueue.size; i++) if (loadQueue.get(i).fileName.equals(fileName))
            return true;
    ...
    public synchronized void unload(String fileName) {
        ...
        if (dependencies != null) {
            for (String dependency : dependencies) if (isLoaded(dependency))
                unload(dependency);
        }
     ...
     * @param loader the loader
     */
    public synchronized <T, P extends AssetLoaderParameters<T>> void setLoader(Class<T> type, @Nullable() String suffix, AssetLoader<T, P> loader) {
        if (type == null)
    ...
    public synchronized void clear() {
        ...
        while (assetTypes.size > 0) {
            ...
            for (String asset : assets) if (dependencyCount.get(asset, 0) == 0)
                unload(asset);
        }
        this.assets.clear();
