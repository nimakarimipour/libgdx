 ...
 * OpenGL resource, like the Pixmap, on a separate thread to then load the actual resource on the
...
public abstract class AsynchronousAssetLoader<T, P extends AssetLoaderParameters<T>>
  ...
  /**
   * Loads the non-OpenGL part of the asset and injects any dependencies of the asset into the
   * AssetManager.
   *
   * @param manager
   ...
   * AssetLoaderParameters) loadSync} is called. This method may be invoked on any thread, but will
   * not be invoked during or after {@link #loadSync(AssetManager, String, FileHandle,
   * AssetLoaderParameters) loadSync}. This method is not invoked when a task is cancelled because
   * it threw an exception, only when the asset is unloaded before loading is complete.
   *
   * <p>The default implementation does nothing. Subclasses should release any resources acquired in
   * {@link #loadAsync(AssetManager, String, FileHandle, AssetLoaderParameters) loadAsync}, which
   * may or may not have been called before this method, but never during or after this method. Note
