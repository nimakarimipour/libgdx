...
/**
 * This {@link FileHandleResolver} uses a given list of {@link Resolution}s to determine the best
 * match based on the current back buffer size. An example of how this resolver works:
 *
 * <p>Let's assume that we have only a single {@link Resolution} added to this resolver. This
 * resolution has the following properties:
 *
 * <ul>
 *   <li>{@code portraitWidth = 1920}
 *   <li>{@code portraitHeight = 1080}
 *   <li>{@code folder = "1920x1080"}
 * </ul>
 *
 * <p>One would now supply a file to be found to the resolver. For this example, we assume it is
 * "{@code textures/walls/brick.png}". Since there is only a single {@link Resolution}, this will be
 * the best match for any screen size. The resolver will now try to find the file in the following
 * ways:
 *
 * <ul>
 *   <li>{@code "textures/walls/1920x1080/brick.png"}
 *   <li>{@code "textures/walls/brick.png"}
 * </ul>
 *
 * <p>The files are ultimately resolved via the given {{@link #baseResolver}. In case the first
 * version cannot be resolved, the fallback will try to search for the file without the resolution
 * folder.
 */
public class ResolutionFileResolver implements FileHandleResolver {
  ...
  public static class Resolution {
     ...
     * @param folder The name of the folder, where the assets which fit this resolution, are
     *     located.
     */
  ...
  /**
   * Creates a {@code ResolutionFileResolver} based on a given {@link FileHandleResolver} and a list
   * of {@link Resolution}s.
   *
   * @param baseResolver The {@link FileHandleResolver} that will ultimately used to resolve the
   *     file.
