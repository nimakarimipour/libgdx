 ...
 * suffix is ".vert", it is assumed to be a vertex shader, and a fragment shader is found using the
 * same file name with a ".frag" suffix. And vice versa if the file suffix is ".frag". These default
 * suffixes can be changed in the ShaderProgramLoader constructor.
 *
 * <p>For all other file suffixes, the same file is used for both (and therefore should internally
 * distinguish between the programs using preprocessor directives and {@link
 * ShaderProgram#prependVertexCode} and {@link ShaderProgram#prependFragmentCode}).
 *
 * <p>The above default behavior for finding the files can be overridden by explicitly setting the
 * file names in a {@link ShaderProgramParameter}. The parameter can also be used to prepend code to
 * the programs.
 *
 * @author cypherdare
 */
public class ShaderProgramLoader
  ...
  public ShaderProgram loadSync(
    ...
    if (vertFileName == null && fileName.endsWith(fragmentFileSuffix)) {
      vertFileName =
          fileName.substring(0, fileName.length() - fragmentFileSuffix.length()) + vertexFileSuffix;
    }
    if (fragFileName == null && fileName.endsWith(vertexFileSuffix)) {
      fragFileName =
          fileName.substring(0, fileName.length() - vertexFileSuffix.length()) + fragmentFileSuffix;
    }
    ...
    if ((parameter == null || parameter.logOnCompileFailure) && !shaderProgram.isCompiled()) {
      manager
          .getLogger()
          .error("ShaderProgram " + fileName + " failed to compile:\n" + shaderProgram.getLog());
    }
  ...
  public static class ShaderProgramParameter extends AssetLoaderParameters<ShaderProgram> {
    ...
    /**
     * Code that is always added to the vertex shader code. This is added as-is, and you should
