...
/**
 * A single vertex attribute defined by its {@link Usage}, its number of components and its shader
 * alias. The Usage is used for uniquely identifying the vertex attribute from among its {@linkplain
 * VertexAttributes} siblings. The number of components defines how many components the attribute
 * has. The alias defines to which shader attribute this attribute should bind. The alias is used by
 * a {@link Mesh} when drawing with a {@link ShaderProgram}. The alias can be changed at any time.
 *
 * @author mzechner
 */
public final class VertexAttribute {
  ...
  /**
   * For fixed types, whether the values are normalized to either -1f and +1f (signed) or 0f and +1f
   * (unsigned)
   */
  ...
  /**
   * Constructs a new VertexAttribute. The GL data type is automatically selected based on the
   * usage.
   *
   * @param usage The attribute {@link Usage}, used to select the {@link #type} and for
   *     identification.
   ...
   * @param numComponents the number of components of this attribute, must be between 1 and 4.
   * @param type the OpenGL type of each component, e.g. {@link GL20#GL_FLOAT} or {@link
   *     GL20#GL_UNSIGNED_BYTE}. Since {@link Mesh} stores vertex data in 32bit floats, the total
   *     size of this attribute (type size times number of components) must be a multiple of four.
   * @param normalized For fixed types, whether the values are normalized to either -1f and +1f
  ...
  /**
   * @return A copy of this VertexAttribute with the same parameters. The {@link #offset} is not
   *     copied and must be recalculated, as is typically done by the {@linkplain VertexAttributes}
   *     that owns the VertexAttribute.
   */
  ...
  public static VertexAttribute TexCoords(int unit) {
    return new VertexAttribute(
        Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + unit, unit);
  }
  ...
  public static VertexAttribute ColorPacked() {
    return new VertexAttribute(
        Usage.ColorPacked, 4, GL20.GL_UNSIGNED_BYTE, true, ShaderProgram.COLOR_ATTRIBUTE);
  }

  public static VertexAttribute ColorUnpacked() {
    return new VertexAttribute(
        Usage.ColorUnpacked, 4, GL20.GL_FLOAT, false, ShaderProgram.COLOR_ATTRIBUTE);
  }
  ...
  public static VertexAttribute BoneWeight(int unit) {
    return new VertexAttribute(
        Usage.BoneWeight, 2, ShaderProgram.BONEWEIGHT_ATTRIBUTE + unit, unit);
  }
