...
/**
 * A Batch is used to draw 2D rectangles that reference a texture (region). The class will batch the
 * drawing commands and optimize them for processing by the GPU.
 *
 * <p>To draw something with a Batch one has to first call the {@link Batch#begin()} method which
 * will setup appropriate render states. When you are done with drawing you have to call {@link
 * Batch#end()} which will actually draw the things you specified.
 *
 * <p>All drawing commands of the Batch operate in screen coordinates. The screen coordinate system
 * has an x-axis pointing to the right, an y-axis pointing upwards and the origin is in the lower
 * left corner of the screen. You can also provide your own transformation and projection matrices
 * if you so wish.
 *
 * <p>A Batch is managed. In case the OpenGL context is lost all OpenGL resources a Batch uses
 * internally get invalidated. A context is lost when a user switches to another application or
 * receives an incoming call on Android. A Batch will be automatically reloaded after the OpenGL
 * context is restored.
 *
 * <p>A Batch is a pretty heavy object so you should only ever have one in your program.
 *
 * <p>A Batch works with OpenGL ES 2.0. It will use its own custom shader to draw all provided
 * sprites. You can set your own custom shader via {@link #setShader(ShaderProgram)}.
 *
 * <p>A Batch has to be disposed if it is no longer used.
 *
 * @author mzechner
 * @author Nathan Sweet
 */
public interface Batch extends Disposable {
  /**
   * Sets up the Batch for drawing. This will disable depth buffer writing. It enables blending and
   * texturing. If you have more texture units enabled than the first one you have to disable them
   * before calling this. Uses a screen coordinate system by default where everything is given in
   * pixels. You can specify your own projection and modelview matrices via {@link
  ...
  /**
   * Sets the color used to tint images when they are added to the Batch. Default is {@link
   * Color#WHITE}.
   */
  ...
  /**
   * @return the rendering color of this Batch. If the returned instance is manipulated, {@link
   ...
   * elements in this order: x, y, color, u, v. The {@link #getColor()} from the Batch is not
   * applied.
   */
  ...
  /**
   * Draws a rectangle with the texture coordinates rotated 90 degrees. The bottom left corner at
   * x,y and stretching the region to cover the given width and height. The rectangle is offset by
   * originX, originY relative to the origin. Scale specifies the scaling factor by which the
   * rectangle should be scaled around originX, originY. Rotation specifies the angle of counter
   * clockwise rotation of the rectangle around originX, originY.
   *
   * @param clockwise If true, the texture coordinates are rotated 90 degrees clockwise. If false,
  ...
  /**
   * Disables blending for drawing sprites. Calling this within {@link #begin()}/{@link #end()} will
   * flush the batch.
   */
  public void disableBlending();

  /**
   * Enables blending for drawing sprites. Calling this within {@link #begin()}/{@link #end()} will
   * flush the batch.
   */
   ...
   *
   * @param srcFunc the source function, e.g. GL20.GL_SRC_ALPHA. If set to -1, Batch won't change
   ...
   *
   * @param srcFuncColor the source color function, e.g. GL20.GL_SRC_ALPHA. If set to -1, Batch
   *     won't change the blending function.
  ...
  /**
   * Returns the current projection matrix. Changing this within {@link #begin()}/{@link #end()}
   * results in undefined behaviour.
   */
  public Matrix4 getProjectionMatrix();

  /**
   * Returns the current transform matrix. Changing this within {@link #begin()}/{@link #end()}
   * results in undefined behaviour.
   */
  public Matrix4 getTransformMatrix();

  /**
   * Sets the projection matrix to be used by this Batch. If this is called inside a {@link
   ...
   * "a_position", the texture coordinates attribute is called "a_texCoord0", the color attribute is
   * called "a_color". See {@link ShaderProgram#POSITION_ATTRIBUTE}, {@link
   * ShaderProgram#COLOR_ATTRIBUTE} and {@link ShaderProgram#TEXCOORD_ATTRIBUTE} which gets "0"
   * appended to indicate the use of the first texture unit. The combined transform and projection
   * matrx is uploaded via a mat4 uniform called "u_projTrans". The texture sampler is passed via a
   * uniform called "u_texture".
   *
   * <p>Call this method with a null argument to use the default shader.
   *
   * <p>This method will flush the batch before setting the new shader, you can call it in between
  ...
  /**
   * @return the current {@link ShaderProgram} set by {@link #setShader(ShaderProgram)} or the
   *     defaultShader
   */
