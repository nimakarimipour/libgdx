...
public class TextureAtlas implements Disposable {
	...
	public void load (TextureAtlasData data) {
		...
		for (Region region : data.regions) {
			AtlasRegion atlasRegion = new //
			//
			AtlasRegion(//
				region.page.texture, //
				region.left, //
				region.top, region.rotate ? region.height : region.width, region.rotate ? region.width : region.height);
	...
	static public class TextureAtlasData {
		...
		public void load (FileHandle packFile, FileHandle imagesDir, boolean flip) {
			...
			regionFields.put("xy", new // Deprecated, use bounds.
			Field<Region>() {
			...
			regionFields.put("size", new // Deprecated, use bounds.
			Field<Region>() {
			...
			regionFields.put("offset", new // Deprecated, use offsets.
			Field<Region>() {
			...
			regionFields.put("orig", new // Deprecated, use offsets.
			Field<Region>() {
			...
			regionFields.put("rotate", new Field<Region>() {
				...
				public void parse (Region region) {
					...
					!value.equals("false")) region.degrees = Integer.parseInt(value);
				...
				while (true) {
					...
					if (line.trim().length() == 0) {
						...
						while (true) {
							...
							if (field != null) field.parse(page);
						}
						pages.add(page);
					} else {
		...
		static public class Page {
			...
			@Null
			@SuppressWarnings("NullAway.Init") public FileHandle textureFile;

			/** May be null if the texture is not yet loaded. */
			@Null
			@SuppressWarnings("NullAway.Init") public Texture texture;
		...
		static public class Region {
			...
			@SuppressWarnings("NullAway.Init") public Page page;
			...
			@Null
			@SuppressWarnings("NullAway.Init") public String[] names;

			@Null
			@SuppressWarnings("NullAway.Init") public int[][] values;
	...
	static public class AtlasRegion extends TextureRegion {
		...
		@Override
		public /** Flips the region, adjusting the offset so the image appears to be flipped as if no whitespace has been removed
					 * for packing. */
