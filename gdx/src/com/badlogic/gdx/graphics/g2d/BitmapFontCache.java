...
public class BitmapFontCache {
    ...
    public void setColors(float color, int start, int end) {
        if (pageVertices.length == 1) {
            ...
            for (int i = start * 20 + 2, n = Math.min(end * 20, idx[0]); i < n; i += 5) vertices[i] = color;
            return;
        }
     ...
     */
    public GlyphLayout addText(@Nullable CharSequence str, float x, float y, int start, int end, float targetWidth, int halign, boolean wrap, @Nullable String truncate) {
