...
public class PixmapPackerIO {
  ...
  /**
   * Saves the provided PixmapPacker to the provided file. The resulting file will use the standard
   * TextureAtlas file format and can be loaded by TextureAtlas as if it had been created using
   * TexturePacker. Default {@link SaveParameters} will be used.
   *
   * @param file the file to which the atlas descriptor will be written, images will be written as
   *     siblings
   ...
   * @throws IOException if the atlas file can not be written
   */
  public void save(FileHandle file, PixmapPacker packer, SaveParameters parameters)
      throws IOException {
    ...
    for (Page page : packer.pages) {
      if (page.rects.size > 0) {
        ...
        writer.write(
            "filter: " + parameters.minFilter.name() + "," + parameters.magFilter.name() + "\n");
        ...
        for (String name : page.rects.keys()) {
          ...
          writer.write(
              "  offset: "
                  + rect.offsetX
                  + ", "
                  + (int) (rect.originalHeight - rect.height - rect.offsetY)
                  + "\n");
