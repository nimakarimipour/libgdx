 ...
 * color markup tags.
 *
 * <p>Where wrapping occurs is determined by {@link BitmapFontData#getWrapIndex(Array, int)}.
 * Additionally, when {@link BitmapFontData#markupEnabled} is true wrapping can occur at color start
 * or end tags.
 *
 * <p>When wrapping occurs, whitespace is removed before and after the wrap position. Whitespace is
 * determined by {@link BitmapFontData#isWhitespace(char)}.
 *
 * <p>Glyphs positions are determined by {@link BitmapFontData#getGlyphs(GlyphRun, CharSequence,
 * int, int, Glyph)}.
 *
 * <p>This class is not thread safe, even if synchronized externally, and must only be used from the
 * game thread.
 *
 * @author Nathan Sweet
 * @author davebaol
...
public class GlyphLayout implements Poolable {
   ...
   *
   * <p>Runs are pooled, so references should not be kept past the next call to {@link
   * #setText(BitmapFont, CharSequence, int, int, Color, float, int, boolean, String)} or {@link
   * #reset()}.
   */
  public final Array<GlyphRun> runs = new Array(1);

  /**
   * Determines the colors of the glpyhs in the {@link #runs}. Entries are pairs where the first is
   * the glyph index (across all runs) where the color starts and the second is the color encoded as
   * ABGR8888.
   *
   * <p>For example: <code>[0, WHITE, 4, GREEN, 5, WHITE]</code><br>
   * Glpyhs 0 to 3 are WHITE, 4 is GREEN and 5 to the end are WHITE.
   *
   * <p>The array is empty if there are no runs, otherwise it has at least two entries: <code>
   * [0, startColor]</code>
   */
   ...
   */
  public GlyphLayout(
      BitmapFont font, CharSequence str, Color color, float targetWidth, int halign, boolean wrap) {
  ...
  /**
   * Calls {@link #setText(BitmapFont, CharSequence, int, int, Color, float, int, boolean, String)
   * setText} with the whole string, the font's current color, and no alignment or wrapping.
   */
  ...
  /**
   * @param color The default color to use for the text (the BitmapFont {@link BitmapFont#getColor()
   *     color} is not used). If {@link BitmapFontData#markupEnabled} is true, color markup tags in
   *     the specified string may change the color for portions of the text.
   * @param halign Horizontal alignment of the text, see {@link Align}.
   * @param targetWidth The width used for alignment, line wrapping, and truncation. May be zero if
   *     those features are not used.
   * @param truncate If not null and the width of the glyphs exceed targetWidth, the glyphs are
   *     truncated and the glyphs for the specified truncate string are placed at the end. Empty
   *     string can be used to truncate without adding glyphs. Truncate should not be used with text
   *     that contains multiple lines. Wrap is ignored if truncate is not null.
   */
  public void setText(
      BitmapFont font,
      CharSequence str,
    ...
    Glyph lastGlyph =
        null; // Last glyph of the previous run on the same line, used for kerning between runs.
    int runStart = start;
    outer:
    while (true) {
      int runEnd;
      ...
      for (int ii = 0, nn = run.glyphs.size; ii < nn; ) {
        ...
        max =
            Math.max(
                max,
                runWidth
                    + glyphWidth); // A glyph can extend past the right edge of subsequent glyphs.
        ii++;
  ...
  private GlyphRun wrap(BitmapFontData fontData, GlyphRun first, int wrapIndex) {
    ...
    if (secondStart < glyphCount) {
      ...
      if (fontData.markupEnabled && droppedGlyphCount > 0) {
        ...
        for (int i = colors.size - 2;
            i >= 2;
            i -= 2) { // i >= 1 because first 2 values always determine the base color.
  ...
  /**
   * Returns an X offset for the first glyph so when drawn, none of it is left of the line's drawing
   * position.
   */
