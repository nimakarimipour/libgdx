...
public class GlyphLayout implements Poolable {
        ...
        outer: while (true) {
                    ...
                    '[':
                        if (markupEnabled) {
                            ...
                            if (length >= 0) {
                                ...
                                start += length + 1;
                                if (start == end)
                                    // Color tag is the last element in the string.
                                    isLastRun = true;
                                else
            ...
            runEnded: {
                ...
                if (run.glyphs.size == 0) {
                    ...
                    // Otherwise wrap and truncate must still be processed for lineRun.
                    if (lineRun == null)
                        break runEnded;
                ...
                if (newline || isLastRun) {
                    ...
                    for (int i = 2; i < lineRun.xAdvances.size; i++) {
                        ...
                        (wrapIndex == 0 && lineRun.x == 0) || wrapIndex >= lineRun.glyphs.size) {
    ...
    private void calculateWidths(BitmapFontData fontData) {
        ...
        for (int i = 0, n = runs.size; i < n; i++) {
            ...
            float runWidth = run.x + xAdvances[0], max = 0;
        ...
        if (secondStart < glyphCount) {
            ...
            if (droppedGlyphCount > 0) {
                ...
                if (fontData.markupEnabled && colors.get(colors.size - 2) > this.glyphCount) {
                    ...
                    // Update color change index.
                    colors.set(colors.size - 2, this.glyphCount);
                    // Update color entry.
                    colors.set(colors.size - 1, lastColor);
                }
            }
        }
        if (firstEnd == 0) {
