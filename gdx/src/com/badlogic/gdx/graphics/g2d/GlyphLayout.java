...
public class GlyphLayout implements Poolable {
     ...
     */
    public void setText(BitmapFont font, CharSequence str, int start, int end, Color color, float targetWidth, int halign, boolean wrap, @Null @Nullable() String truncate) {
        ...
        targetWidth <= fontData.spaceXadvance * 3)
                ...
                runEnded: if (runEnd != runStart) {
                    ...
                    if (!wrap || n == 0) {
                        ...
                        if (markupEnabled) {
                            ...
                            for (int i = 0; i < n; i++) x += xAdvances[i];
                        }
                        break runEnded;
                    }
                    // Wrap or truncate.
                    ...
                    for (int i = 2; i < n; i++) {
                        ...
                        (wrapIndex == 0 && run.x == 0) || wrapIndex >= run.glyphs.size) {
                        ...
                        if (wrapIndex == 0) {
                            ...
                            for (int glyphCount = run.glyphs.size; wrapIndex < glyphCount; wrapIndex++) if (!fontData.isWhitespace((char) run.glyphs.get(wrapIndex).id))
                                break;
                            if (wrapIndex > 0) {
                            ...
                            if (runs.size > 1) {
                                ...
                                for (; lastIndex > 0; lastIndex--) if (!fontData.isWhitespace((char) previous.glyphs.get(lastIndex).id))
                                    break;
        ...
        if ((halign & Align.left) == 0) {
            ...
            for (int i = 0; i < runsSize; i++) {
                ...
                if (run.y != lineY) {
                    ...
                    if (center)
                        shift /= 2;
                    while (lineStart < i) ((GlyphRun) runsItems[lineStart++]).x += shift;
            ...
            if (center)
                shift /= 2;
            while (lineStart < runsSize) ((GlyphRun) runsItems[lineStart++]).x += shift;
        }
    }
    ...
    private void truncate(BitmapFontData fontData, GlyphRun run, float targetWidth, String truncate, int widthIndex, Pool<GlyphRun> glyphRunPool) {
        ...
        if (truncateRun.xAdvances.size > 0) {
            ...
            for (// Skip first for tight bounds.
            // Skip first for tight bounds.
            // Skip first for tight bounds.
            int i = 1, n = truncateRun.xAdvances.size; i < n; i++) truncateWidth += xAdvances[i];
        }
    ...
    private GlyphRun wrap(BitmapFontData fontData, GlyphRun first, int wrapIndex, int widthIndex) {
        ...
        for (; secondStart < glyphCount; secondStart++) if (!fontData.isWhitespace((char) glyphs2.get(secondStart).id))
            break;
