		...
		while (true) {
			...
			runEnded:
			{
				...
				if (run.glyphs.size == 0) {
					...
					if (lineRun == null) break runEnded;
				...
				if (!wrapOrTruncate || lineRun.glyphs.size == 0) break runEnded;
				if (newline || isLastRun) {
	...
	private void truncate (BitmapFontData fontData, GlyphRun run, float targetWidth, String truncate) {
		...
		if (truncateRun.xAdvances.size > 0) {
			...
			for (// Skip first for tight bounds.
				int i = 1, n = truncateRun.xAdvances.size; // Skip first for tight bounds.
				i < n; // Skip first for tight bounds.
				i++)
		...
		case '#':
			...
			for (int i = start + 1; i < end; i++) {
				...
				if (ch == ']') {
					...
					if (i - start < 8) color = color << (9 - (i - start) << 2) | 0xff;
