...
public class ETC1 {
  ...
  public static final class ETC1Data implements Disposable {
    ...
    /**
     * the offset in bytes to the actual compressed data. Might be 16 if this contains a PKM header,
     * 0 otherwise *
     */
  ...
  /**
   * Encodes the image via the ETC1 compression scheme. Only {@link Format#RGB565} and {@link
   * Format#RGB888} are supported.
   *
  ...
  public static ETC1Data encodeImage(Pixmap pixmap) {
    ...
    ByteBuffer compressedData =
        encodeImage(pixmap.getPixels(), 0, pixmap.getWidth(), pixmap.getHeight(), pixelSize);
  ...
  /**
   * Encodes the image via the ETC1 compression scheme. Only {@link Format#RGB565} and {@link
   * Format#RGB888} are supported. Adds a PKM header in front of the compressed image data.
   *
   * @param pixmap the {@link Pixmap}
   * @return the {@link ETC1Data}
   */
  public static ETC1Data encodeImagePKM(Pixmap pixmap) {
    int pixelSize = getPixelSize(pixmap.getFormat());
    ByteBuffer compressedData =
        encodeImagePKM(pixmap.getPixels(), 0, pixmap.getWidth(), pixmap.getHeight(), pixelSize);
    BufferUtils.newUnsafeByteBuffer(compressedData);
    return new ETC1Data(pixmap.getWidth(), pixmap.getHeight(), compressedData, 16);
  }

  /**
   * Takes ETC1 compressed image data and converts it to a {@link Format#RGB565} or {@link
   * Format#RGB888} {@link Pixmap}. Does not modify the ByteBuffer's position or limit.
   *
  ...
  public static Pixmap decodeImage(@Nullable ETC1Data etc1Data, Format format) {
    ...
    decodeImage(
        etc1Data.compressedData, dataOffset, pixmap.getPixels(), 0, width, height, pixelSize);
    return pixmap;
  }
  ...
  /**
   * Writes a PKM header to the {@link ByteBuffer}. Does not modify the position or limit of the
   * ByteBuffer.
   *
  ...
  /**
   * Decodes the compressed image data to RGB565 or RGB888 pixel data. Does not modify the position
   * or limit of the {@link ByteBuffer} instances.
   *
   ...
   * @param decodedData the decoded data in a direct native order ByteBuffer, must hold width *
   *     height * pixelSize bytes.
   * @param offsetDec the offset in bytes to the decoded image data.
   * @param width the width in pixels
   * @param height the height in pixels
   * @param pixelSize the pixel size, either 2 (RBG565) or 3 (RGB888)
   */
  private static native void decodeImage(
      ByteBuffer compressedData,
      int offset,
  ...
  /**
   * Encodes the image data given as RGB565 or RGB888. Does not modify the position or limit of the
   * {@link ByteBuffer}.
   *
   ...
   */
  private static native ByteBuffer encodeImage(
   ...
   * @return a new direct native order ByteBuffer containing the compressed image data
   */
  private static native ByteBuffer encodeImagePKM(
      ByteBuffer imageData, int offset, int width, int height, int pixelSize); /*
		int compressedSize = etc1_get_encoded_data_size(width, height);
