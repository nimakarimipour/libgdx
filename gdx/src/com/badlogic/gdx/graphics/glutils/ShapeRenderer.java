 ...
 *
 * <p>By default a 2D orthographic projection with the origin in the lower left corner is used and
 * units are specified in screen pixels. This can be changed by configuring the projection matrix,
 * usually using the {@link Camera#combined} matrix. If the screen resolution changes, the
 * projection matrix may need to be updated.
 *
 * <p>Shapes are rendered in batches to increase performance. Standard usage pattern looks as
 * follows:
 *
 * <pre>{@code
 * camera.update();
 ...
 * }</pre>
 *
 * ShapeRenderer has a second matrix called the transformation matrix which is used to rotate, scale
 * and translate shapes in a more flexible manner. The following example shows how to rotate a
 * rectangle around its center using the z-axis as the rotation axis and placing it's center at (20,
 * 12, 2):
 *
 * <pre>
 * shapeRenderer.begin(ShapeType.Line);
 ...
 *
 * <p>The projection and transformation matrices are a state of the ShapeRenderer, just like the
...
public class ShapeRenderer implements Disposable {
  ...
  /**
   * Sets the projection matrix to be used for rendering. Usually this will be set to {@link
   * Camera#combined}.
   *
   * @param matrix
   */
  ...
  /**
   * Draws a point using {@link ShapeType#Point}, {@link ShapeType#Line} or {@link
   * ShapeType#Filled}.
   */
  ...
  /**
   * Draws a triangle in x/y plane with colored corners using {@link ShapeType#Line} or {@link
   * ShapeType#Filled}.
   */
  ...
  public void rect(
      ...
      float degrees) {
    rect(
        x, y, originX, originY, width, height, scaleX, scaleY, degrees, color, color, color, color);
  }
  ...
  /**
   * Calls {@link #arc(float, float, float, float, float, int)} by estimating the number of segments
   * needed for a smooth arc.
   */
  ...
  /**
   * Calls {@link #circle(float, float, float, int)} by estimating the number of segments needed for
   * a smooth circle.
   */
