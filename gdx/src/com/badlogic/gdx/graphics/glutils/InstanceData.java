...
/**
 * A InstanceData instance holds instance data for rendering with OpenGL. It is implemented as
 * either a {@link InstanceBufferObject} or a {@link InstanceBufferObjectSubData}. Both require Open
 * GL 3.3+.
 *
 * @author mrdlink
 */
public interface InstanceData extends Disposable {
  ...
  /**
   * Sets the vertices of this InstanceData, discarding the old vertex data. The count must equal
   * the number of floats per vertex times the number of vertices to be copied to this VertexData.
   * The order of the vertex attributes must be the same as specified at construction time via
   * {@link VertexAttributes}.
   *
   * <p>This can be called in between calls to bind and unbind. The vertex data will be updated
   * instantly.
   *
  ...
  /**
   * Sets the vertices of this InstanceData, discarding the old vertex data. The count must equal
   * the number of floats per vertex times the number of vertices to be copied to this InstanceData.
   * The order of the vertex attributes must be the same as specified at construction time via
   * {@link VertexAttributes}.
   *
   * <p>This can be called in between calls to bind and unbind. The vertex data will be updated
   ...
   * uploaded on the next call to bind. If you need immediate uploading use {@link
   * #setInstanceData(float[], int, int)}; Any modifications made to the Buffer *after* the call to
   * bind will not automatically be uploaded.
   *
