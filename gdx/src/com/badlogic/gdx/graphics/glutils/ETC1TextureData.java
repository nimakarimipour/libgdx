...
public class ETC1TextureData implements TextureData {
  ...
  public void consumeCustomData(int target) {
    ...
    if (data == null)
      throw new GdxRuntimeException("Data is null, cannot proceed with consumeCustomData.");
    ...
    if (!Gdx.graphics.supportsExtension("GL_OES_compressed_ETC1_RGB8_texture")) {
      ...
      if (data.compressedData != null) {
        Gdx.gl.glCompressedTexImage2D(
            ...
            height,
            0,
            Nullability.castToNonnull(data, "data is checked nonnull").compressedData.capacity()
                - Nullability.castToNonnull(data, "data is checked nonnull").dataOffset,
            Nullability.castToNonnull(data, "data is checked nonnull").compressedData);
        if (useMipMaps()) Gdx.gl20.glGenerateMipmap(GL20.GL_TEXTURE_2D);
      }
    }
    Nullability.castToNonnull(data, "checked to be nonnull").dispose();
    data = null;
    isPrepared = false;
  }
