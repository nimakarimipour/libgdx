...
public class MipMapGenerator {
  ...
  /**
   * Sets the image data of the {@link Texture} based on the {@link Pixmap}. The texture must be
   * bound for this to work. If <code>disposePixmap</code> is true, the pixmap will be disposed at
  ...
  public static void generateMipMap(
      int target, Pixmap pixmap, int textureWidth, int textureHeight) {
    if (!useHWMipMap) {
  ...
  private static void generateMipMapDesktop(
      int target, Pixmap pixmap, int textureWidth, int textureHeight) {
    if (Gdx.graphics.supportsExtension("GL_ARB_framebuffer_object")
        ...
        || Gdx.gl20
            .getClass()
            .getName()
            .equals("com.badlogic.gdx.backends.lwjgl3.Lwjgl3GLES20") // LWJGL3ANGLE
        || Gdx.gl30 != null) {
      Gdx.gl.glTexImage2D(
          target,
          0,
  ...
  private static void generateMipMapCPU(
      int target, Pixmap pixmap, int textureWidth, int textureHeight) {
    Gdx.gl.glTexImage2D(
        target,
        0,
