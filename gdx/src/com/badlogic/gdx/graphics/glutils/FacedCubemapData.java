...
public class FacedCubemapData implements CubemapData {
  ...
  /**
   * Construct an empty Cubemap. Use the load(...) methods to set the texture of each side. Every
   * side of the cubemap must be set before it can be used.
   */
  ...
  public boolean isManaged() {
    for (TextureData data : this.data) if (!data.isManaged()) return false;
    return true;
  }

  /**
   * Loads the texture specified using the {@link FileHandle} and sets it to specified side,
   * overwriting any previous data set to that side. Note that you need to reload through {@link
   * Cubemap#load(CubemapData)} any cubemap using this data for the change to be taken in account.
   *
  ...
  /**
   * Sets the specified side of this cubemap to the specified {@link Pixmap}, overwriting any
   * previous data set to that side. Note that you need to reload through {@link
   * Cubemap#load(CubemapData)} any cubemap using this data for the change to be taken in account.
   *
   * @param side The {@link CubemapSide}
  ...
  public boolean isComplete() {
    for (int i = 0; i < data.length; i++) if (data[i] == null) return false;
    return true;
  }

  /**
   * @return The {@link TextureData} for the specified side, can be null if the cubemap is
   *     incomplete.
   */
  ...
  public int getWidth() {
    ...
    if (data[CubemapSide.PositiveZ.index] != null
        && (tmp = data[CubemapSide.PositiveZ.index].getWidth()) > width) width = tmp;
    if (data[CubemapSide.NegativeZ.index] != null
        && (tmp = data[CubemapSide.NegativeZ.index].getWidth()) > width) width = tmp;
    if (data[CubemapSide.PositiveY.index] != null
        && (tmp = data[CubemapSide.PositiveY.index].getWidth()) > width) width = tmp;
    if (data[CubemapSide.NegativeY.index] != null
        && (tmp = data[CubemapSide.NegativeY.index].getWidth()) > width) width = tmp;
    return width;
  }
  ...
  public int getHeight() {
    int tmp, height = 0;
    if (data[CubemapSide.PositiveZ.index] != null
        && (tmp = data[CubemapSide.PositiveZ.index].getHeight()) > height) height = tmp;
    if (data[CubemapSide.NegativeZ.index] != null
        && (tmp = data[CubemapSide.NegativeZ.index].getHeight()) > height) height = tmp;
    if (data[CubemapSide.PositiveX.index] != null
        && (tmp = data[CubemapSide.PositiveX.index].getHeight()) > height) height = tmp;
    if (data[CubemapSide.NegativeX.index] != null
        && (tmp = data[CubemapSide.NegativeX.index].getHeight()) > height) height = tmp;
    return height;
  }
  ...
  public void prepare() {
    ...
    for (int i = 0; i < data.length; i++) if (!data[i].isPrepared()) data[i].prepare();
  }
