...
/**
 * A KTXTextureData holds the data from a KTX (or zipped KTX file, aka ZKTX). That is to say an
 * OpenGL ready texture data. The KTX file format is just a thin wrapper around OpenGL textures and
 * therefore is compatible with most OpenGL texture capabilities like texture compression,
 * cubemapping, mipmapping, etc.
 *
 * <p>For example, KTXTextureData can be used for {@link Texture} or {@link Cubemap}.
 *
...
public class KTXTextureData implements TextureData, CubemapData {
  ...
  @Nullable private ByteBuffer compressedData;
  ...
  public void prepare() {
    ...
    // We support normal ktx files as well as 'zktx' which are gzip ktx file with an int length at
    // the beginning (like ETC1).
  ...
  public void consumeCustomData(int target) {
    ...
    if (numberOfFaces == 6 && target != GL20.GL_TEXTURE_CUBE_MAP) {
      ...
      if (!(GL20.GL_TEXTURE_CUBE_MAP_POSITIVE_X <= target
          && target <= GL20.GL_TEXTURE_CUBE_MAP_NEGATIVE_Z))
        throw new GdxRuntimeException(
      ...
      // Load normal texture
      if (target != glTarget
          && !(GL20.GL_TEXTURE_CUBE_MAP_POSITIVE_X <= target
              && target <= GL20.GL_TEXTURE_CUBE_MAP_NEGATIVE_Z
              && target == GL20.GL_TEXTURE_2D))
        throw new GdxRuntimeException(
    ...
    for (int level = 0; level < numberOfMipmapLevels; level++) {
      ...
      for (int face = 0; face < numberOfFaces; face++) {
        ...
        if (textureDimensions == 1) {
          // if (compressed)
          // Gdx.gl.glCompressedTexImage1D(target + face, level, glInternalFormat, pixelWidth, 0,
          // faceLodSize,
          // data);
          // else
          // Gdx.gl.glTexImage1D(target + face, level, glInternalFormat, pixelWidth, 0, glFormat,
          // glType, data);
          ...
          // if (compressed)
          // Gdx.gl.glCompressedTexImage3D(target + face, level, glInternalFormat, pixelWidth,
          // pixelHeight, pixelDepth, 0,
          // faceLodSize, data);
          // else
          // Gdx.gl.glTexImage3D(target + face, level, glInternalFormat, pixelWidth, pixelHeight,
          // pixelDepth, 0, glFormat,
          // glType, data);
        }
      }
    }
  ...
  public Format getFormat() {
    throw new GdxRuntimeException(
        "This TextureData implementation directly handles texture formats.");
  }
