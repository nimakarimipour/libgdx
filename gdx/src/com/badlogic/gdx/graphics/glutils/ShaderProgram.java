 ...
 *
 * <p>After construction a ShaderProgram can be used to draw {@link Mesh}. To make the GPU use a
 * specific ShaderProgram the programs {@link ShaderProgram#bind()} method must be used which
 * effectively binds the program.
 *
 * <p>When a ShaderProgram is bound one can set uniforms, vertex attributes and attributes as needed
 * via the respective methods.
 *
 * <p>A ShaderProgram must be disposed via a call to {@link ShaderProgram#dispose()} when it is no
 * longer needed
 *
 * <p>ShaderPrograms are managed. In case the OpenGL context is lost all shaders get invalidated and
 * have to be reloaded. This happens on Android when a user switches to another application or
 * receives an incoming call. Managed ShaderPrograms are automatically reloaded when the OpenGL
 * context is recreated so you don't have to do this manually.
 *
 * @author mzechner
 */
public class ShaderProgram implements Disposable {
  ...
  private static final ObjectMap<Application, Array<ShaderProgram>> shaders =
      new ObjectMap<Application, Array<ShaderProgram>>();
  ...
  /**
   * @return the log info for the shader compilation and program linking stage. The shader needs to
   *     be bound for this method to have an effect.
   */
  ...
  /**
   * Sets the uniform matrix with the given name. The {@link ShaderProgram} must be bound for this
   * to work.
   *
   * @param name the name of the uniform
  ...
  /**
   * Sets an array of uniform matrices with the given name. The {@link ShaderProgram} must be bound
   * for this to work.
   *
   * @param name the name of the uniform
  ...
  /**
   * Sets the vertex attribute with the given name. The {@link ShaderProgram} must be bound for this
   * to work.
   *
  ...
  public void setVertexAttribute(
      String name, int size, int type, boolean normalize, int stride, Buffer buffer) {
    GL20 gl = Gdx.gl20;
    checkManaged();
  ...
  public void setVertexAttribute(
      int location, int size, int type, boolean normalize, int stride, Buffer buffer) {
    GL20 gl = Gdx.gl20;
    checkManaged();
    gl.glVertexAttribPointer(location, size, type, normalize, stride, buffer);
  }
   ...
   */
  public void setVertexAttribute(
      String name, int size, int type, boolean normalize, int stride, int offset) {
    GL20 gl = Gdx.gl20;
    checkManaged();
    int location = fetchAttributeLocation(name);
    if (location == -1) return;
  ...
  public void setVertexAttribute(
      int location, int size, int type, boolean normalize, int stride, int offset) {
    GL20 gl = Gdx.gl20;
    checkManaged();
    gl.glVertexAttribPointer(location, size, type, normalize, stride, offset);
  }
  ...
  /**
   * Disposes all resources associated with this shader. Must be called when the shader is no longer
   * used.
   */
  public void dispose() {
    GL20 gl = Gdx.gl20;
  ...
  /**
   * @param name the name of the attribute
   * @return the type of the attribute, one of {@link GL20#GL_FLOAT}, {@link GL20#GL_FLOAT_VEC2}
   *     etc.
   */
