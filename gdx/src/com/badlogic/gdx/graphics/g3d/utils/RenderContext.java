...
/**
 * Manages OpenGL state and tries to reduce state changes. Uses a {@link TextureBinder} to reduce
 * texture binds as well. Call {@link #begin()} to setup the context, call {@link #end()} to undo
 * all state changes. Use the setters to change state, use {@link #textureBinder} to bind textures.
 *
...
public class RenderContext {
  ...
  public void setDepthTest(
      final int depthFunction, final float depthRangeNear, final float depthRangeFar) {
    ...
    if (enabled) {
      ...
      if (!wasEnabled
        ...
        Gdx.gl.glDepthRangef(
            this.depthRangeNear = depthRangeNear, this.depthRangeFar = depthRangeFar);
    }
  }
