 ...
 * disabling blending as needed. Opaque decals are rendered first (decal color is ignored in opacity
 * check).<br>
 * Use this strategy only if the vast majority of your decals are opaque and the few transparent
 * ones are unlikely to overlap.
 *
 * <p>Can produce invisible artifacts when transparent decals overlap each other.
 *
 * <p>Needs to be explicitly disposed as it might allocate a ShaderProgram when GLSL 2.0 is used.
 *
 * <p>States (* = any, EV = entry value - same as value before flush):<br>
 *
 * <table>
 * <tr>
 * <td></td>
...
public class CameraGroupStrategy implements GroupStrategy, Disposable {
  ...
  ObjectMap<DecalMaterial, Array<Decal>> materialGroups =
      new ObjectMap<DecalMaterial, Array<Decal>>();
