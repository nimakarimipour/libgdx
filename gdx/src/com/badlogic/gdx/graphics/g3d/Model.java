 ...
 * Animations can be applied to nodes, to modify their transform (translation, rotation, scale) over time. A model can be rendered
 * by creating a {@link ModelInstance} from it. That instance has an additional transform to position the model in the world, and
 * allows modification of materials and nodes without destroying the original model. The original model is the owner of any meshes
 * and textures, all instances created from the model share these resources. Disposing the model will automatically make all
 * instances invalid! A model is created from {@link ModelData}, which in turn is loaded by a {@link ModelLoader}.
 *
...
public class Model implements Disposable {
	 ...
	 * Node. Finally, the animation bone matrices are updated accordingly. This method can be used to recalculate all transforms if
	 * any of the Node's local properties (translation, rotation, scale) was modified. */
