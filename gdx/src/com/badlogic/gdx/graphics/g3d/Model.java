...
public class Model implements Disposable {
  ...
  protected void loadNodes(Iterable<ModelNode> modelNodes) {
    ...
    for (ObjectMap.Entry<NodePart, ArrayMap<String, Matrix4>> e : nodePartBones.entries()) {
      if (e.key != null) { // Ensure that e.key is not null before dereferencing it
        NodePart nonNullKey = Nullability.castToNonnull(e.key, "checked not null");
        if (nonNullKey.invBoneBindTransforms == null) {
          nonNullKey.invBoneBindTransforms = new ArrayMap<Node, Matrix4>(Node.class, Matrix4.class);
        }
        nonNullKey.invBoneBindTransforms.clear();
        for (ObjectMap.Entry<String, Matrix4> b : e.value.entries()) {
          nonNullKey.invBoneBindTransforms.put(getNode(b.key), new Matrix4(b.value).inv());
        }
      }
    }
  }
