      ...
      if (si < ei) {
        ...
        if (ei - si <= 8) {
          for (int i = si; i <= ei; i++)
            for (int j = i;
                j > si
                    && Nullability.castToNonnull(distances, "initialized before use")[j - 1]
                        > Nullability.castToNonnull(distances, "initialized before use")[j];
                j--) {
              tmp = Nullability.castToNonnull(distances, "initialized before use")[j];
              Nullability.castToNonnull(distances, "initialized before use")[j] =
                  Nullability.castToNonnull(distances, "initialized before use")[j - 1];
              Nullability.castToNonnull(distances, "initialized before use")[j - 1] = tmp;
