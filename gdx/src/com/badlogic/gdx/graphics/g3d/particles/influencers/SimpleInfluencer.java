 ...
 * interpolation through time using {@link ScaledNumericValue}.
 *
 * @author Inferno
 */
public abstract class SimpleInfluencer extends Influencer {
  ...
  public void activateParticles(int startIndex, int count) {
    if (!value.isRelative()) {
      for (int i = startIndex * valueChannel.strideSize,
              a = startIndex * interpolationChannel.strideSize,
              c = i + count * valueChannel.strideSize;
          i < c;
          i += valueChannel.strideSize, a += interpolationChannel.strideSize) {
  ...
  public void update() {
    for (int i = 0,
      ...
      valueChannel.data[i] =
          interpolationChannel.data[a + ParticleChannels.InterpolationStartOffset]
