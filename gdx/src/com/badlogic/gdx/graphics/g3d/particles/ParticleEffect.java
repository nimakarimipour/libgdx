 ...
 * changes will be applied on all the particles controllers.
 *
 * @author inferno
 */
public class ParticleEffect implements Disposable, ResourceData.Configurable {
  ...
  /**
   * Assign one batch, among those passed in, to each controller. The batch must be compatible with
   * the controller to be assigned.
   */
  public void setBatch(Array<ParticleBatch<?>> batches) {
    for (ParticleController controller : controllers) {
      for (ParticleBatch<?> batch : batches) if (controller.renderer.setBatch(batch)) break;
    }
  }
