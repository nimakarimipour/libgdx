...
public class BillboardParticleBatch extends BufferedParticleBatch<BillboardControllerRenderData> {
    ...
    private static final VertexAttributes GPU_ATTRIBUTES = new VertexAttributes(new VertexAttribute(Usage.Position, 3, ShaderProgram.POSITION_ATTRIBUTE), new VertexAttribute(Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + "0"), new VertexAttribute(Usage.ColorUnpacked, 4, ShaderProgram.COLOR_ATTRIBUTE), new VertexAttribute(sizeAndRotationUsage, 4, "a_sizeAndRotation")), /*
	 ...
	 */
    CPU_ATTRIBUTES = new VertexAttributes(new VertexAttribute(Usage.Position, 3, ShaderProgram.POSITION_ATTRIBUTE), new VertexAttribute(Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + "0"), new VertexAttribute(Usage.ColorUnpacked, 4, ShaderProgram.COLOR_ATTRIBUTE));
    ...
    // Ext
    // Ext
    GPU_POSITION_OFFSET = (short) (GPU_ATTRIBUTES.findByUsage(Usage.Position).offset / 4), GPU_UV_OFFSET = (short) (GPU_ATTRIBUTES.findByUsage(Usage.TextureCoordinates).offset / 4), GPU_SIZE_ROTATION_OFFSET = (short) (GPU_ATTRIBUTES.findByUsage(sizeAndRotationUsage).offset / 4), GPU_COLOR_OFFSET = (short) (GPU_ATTRIBUTES.findByUsage(Usage.ColorUnpacked).offset / 4), GPU_VERTEX_SIZE = GPU_ATTRIBUTES.vertexSize / 4, /*
		 ...
		 */
    // Cpu
    CPU_POSITION_OFFSET = (short) (CPU_ATTRIBUTES.findByUsage(Usage.Position).offset / 4), CPU_UV_OFFSET = (short) (CPU_ATTRIBUTES.findByUsage(Usage.TextureCoordinates).offset / 4), CPU_COLOR_OFFSET = (short) (CPU_ATTRIBUTES.findByUsage(Usage.ColorUnpacked).offset / 4), CPU_VERTEX_SIZE = CPU_ATTRIBUTES.vertexSize / 4;
     ...
     */
    public BillboardParticleBatch(AlignMode mode, boolean useGPU, int capacity, @Nullable BlendingAttribute blendingAttribute, @Nullable DepthTestAttribute depthTestAttribute) {
    ...
    private void fillVerticesToViewPointCPU(int[] particlesOffset) {
        ...
        for (BillboardControllerRenderData data : renderData) {
            ...
            for (int p = 0, c = data.controller.particles.size; p < c; ++p, ++tp) {
                ...
                Vector3 // normal
                // tangent
                look = TMP_V3.set(camera.position).sub(px, py, pz).nor(), right = TMP_V1.set(camera.up).crs(look).nor(), up = TMP_V2.set(look).crs(right);
                right.scl(sx);
                up.scl(sy);
                if (cosRotation != 1) {
    ...
    private void fillVerticesToScreenCPU(int[] particlesOffset) {
        Vector3 // normal
        // tangent
        look = TMP_V3.set(camera.direction).scl(-1), right = TMP_V4.set(camera.up).crs(look).nor(), up = camera.up;
        int tp = 0;
        for (BillboardControllerRenderData data : renderData) {
            FloatChannel scaleChannel = data.scaleChannel;
            FloatChannel regionChannel = data.regionChannel;
