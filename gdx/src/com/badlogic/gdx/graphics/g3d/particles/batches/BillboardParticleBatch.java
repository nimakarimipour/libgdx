...
public class BillboardParticleBatch extends BufferedParticleBatch<BillboardControllerRenderData> {
	...
	private static final VertexAttributes GPU_ATTRIBUTES = new VertexAttributes(
		...
		new VertexAttribute(Usage.ColorUnpacked, 4, ShaderProgram.COLOR_ATTRIBUTE), new VertexAttribute(sizeAndRotationUsage, 4,
			"a_sizeAndRotation")), /*
	...
	GPU_POSITION_OFFSET = (short)(GPU_ATTRIBUTES.findByUsage(Usage.Position).offset / 4), // Ext
		...
		GPU_VERTEX_SIZE = GPU_ATTRIBUTES.vertexSize / 4, /*
																			 ...
																			 * GPU_EXT_UV_OFFSET =
																			 * (short)(GPU_EXT_ATTRIBUTES.findByUsage(Usage.TextureCoordinates).
																			 * offset/4), GPU_EXT_SIZE_ROTATION_OFFSET =
	...
	public static class Config {
		...
		@SuppressWarnings("NullAway.Init") AlignMode mode;
	}
	...
	@SuppressWarnings("NullAway.Init") Shader shader;
	...
	private void fillVerticesToViewPointCPU (int[] particlesOffset) {
		...
		for (BillboardControllerRenderData data : renderData) {
			...
			for (int p = 0, c = data.controller.particles.size; p < c; ++p, ++tp) {
				...
				Vector3 // normal
				// tangent
				look = TMP_V3.set(camera.position).sub(px, py, pz).nor(), right = TMP_V1.set(camera.up).crs(look).nor(),
