 ...
 * {@link Material} ({@link Attributes}), e.g. the color or texture applied, might not or not correctly be loaded. This
 * {@link ModelLoader} can be used to load very basic models without having to convert them to a more suitable format. Therefore
 * it can be used for educational purposes and to quickly test a basic model, but should not be used in production. Instead use
 * {@link G3dModelLoader}. Because of above reasons, when an OBJ file is loaded using this loader, it will log and error. To
 * prevent this error from being logged, set the {@link #logWarning} flag to false. However, it is advised not to do so. An OBJ
 * file only contains the mesh (shape). It may link to a separate MTL file, which is used to describe one or more materials. In
 * that case the MTL filename (might be case-sensitive) is expected to be located relative to the OBJ file. The MTL file might
 * reference one or more texture files, in which case those filename(s) are expected to be located relative to the MTL file.
 *
...
class MtlLoader {
	...
	private static class ObjMaterial {
		...
		@Nullable Color diffuseColor;
		@Nullable Color specularColor;
		float opacity;
		float shininess;
