 ...
 * represents the number of vertices. In other words: Regardless whether the mesh is indexed or not, when {@link #primitiveType}
 * is not a strip, then {@link #size} equals the number of primitives multiplied by the number of vertices per primitive. So if
 * the MeshPart represents 4 triangles ( {@link #primitiveType} is GL_TRIANGLES), then the {@link #size} member is 12 (4 triangles
 * * 3 vertices = 12 vertices total). Likewise, if the part represents 12 lines ({@link #primitiveType} is GL_LINES), then the
 * size is 24 (12 lines * 2 vertices = 24 vertices total). Note that some classes might require the mesh (part) to be indexed. The
 * {@link Mesh} referenced by the {@link #mesh} member must outlive the MeshPart. When the mesh is disposed, the MeshPart is
 * unusable.
 *
