...
/**
 * When enabled, collects statistics about GL calls and checks for GL errors. Enabling will wrap
 * Gdx.gl* instances with delegate classes which provide described functionality and route GL calls
 * to the actual GL instances.
 *
 * @see GL20Interceptor
 * @see GL30Interceptor
...
public class GLProfiler {
  ...
  /**
   * Create a new instance of GLProfiler to monitor a {@link com.badlogic.gdx.Graphics} instance's
   * gl calls
   *
   * @param graphics instance to monitor with this instance, With Lwjgl 2.x you can pass in
   *     Gdx.graphics, with Lwjgl3 use Lwjgl3Window.getGraphics()
   */
  ...
  /**
   * Disables profiling by resetting the {@code GL20} and {@code GL30} instances with the original
   * ones.
   */
  public void disable() {
    if (!enabled) return;
  ...
  /**
   * @return {@link FloatCounter} containing information about rendered vertices since the last
   *     reset
   */
  ...
  /**
   * Will reset the statistical information which has been collected so far. This should be called
   * after every frame. Error listener is kept as it is.
   */
  public void reset() {
    glInterceptor.reset();
  }
