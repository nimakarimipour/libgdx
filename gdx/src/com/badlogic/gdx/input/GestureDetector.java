...
/**
 * {@link InputProcessor} implementation that detects gestures (tap, long press, fling, pan, zoom,
 * pinch) and hands them to a {@link GestureListener}.
 *
 * @author mzechner
 */
public class GestureDetector extends InputAdapter {
   ...
   * longPressDuration=1.1f, maxFlingDelay=Integer.MAX_VALUE.
   */
   ...
   * @param longPressDuration time in seconds that must pass for the detector to fire a {@link
   *     GestureListener#longPress(float, float)} event.
   * @param maxFlingDelay no fling event is fired when the time in seconds the finger was dragged is
   *     larger than this, see {@link GestureListener#fling(float, float, int)}
   */
  ...
  /**
   * @param halfTapRectangleWidth half width in pixels of the rectangle around an initial touch
   *     event, see {@link GestureListener#tap(float, float, int, int)}.
   * @param halfTapRectangleHeight half height in pixels of the rectangle around an initial touch
   *     event, see {@link GestureListener#tap(float, float, int, int)}.
   * @param tapCountInterval time in seconds that must pass for two touch down/up sequences to be
   *     detected as consecutive taps.
   * @param longPressDuration time in seconds that must pass for the detector to fire a {@link
  ...
  public boolean touchDragged(float x, float y, int pointer) {
    ...
    if (pinching) {
      if (listener != null) {
        ...
        return listener.zoom(initialPointer1.dst(initialPointer2), pointer1.dst(pointer2))
            || result;
      }
      return false;
    }
  ...
  /**
   * Register an instance of this class with a {@link GestureDetector} to receive gestures such as
   * taps, long presses, flings, panning or pinch zooming. Each method returns a boolean indicating
   * if the event should be handed to the next listener (false to hand it to the next listener, true
   * otherwise).
   *
   * @author mzechner
   */
  public static interface GestureListener {
    ...
    /**
     * Called when a tap occured. A tap happens if a touch went down on the screen and was lifted
     * again without moving outside of the tap square. The tap square is a rectangular area around
     * the initial touch position as specified on construction time of the {@link GestureDetector}.
     *
    ...
    /**
     * Called when the user dragged a finger over the screen and lifted it. Reports the last known
     * velocity of the finger in pixels per second.
     *
    ...
    /**
     * Called when the user performs a pinch zoom gesture. The original distance is the distance in
     * pixels when the gesture started.
     *
    ...
    /**
     * Called when a user performs a pinch zoom gesture. Reports the initial positions of the two
     * involved fingers and their current positions.
     *
