...
/**
 * An {@link Input} implementation that receives touch, key, accelerometer and compass events from a
 * remote Android device. Just instantiate it and specify the port it should listen on for incoming
 * connections (default 8190). Then store the new RemoteInput instance in Gdx.input. That's it.
 *
 * <p>On your Android device you can use the gdx-remote application available on the Google Code
 * page as an APK or in SVN (extensions/gdx-remote). Open it, specify the IP address and the port of
 * the PC your libGDX app is running on and then tap away.
 *
 * <p>The touch coordinates will be translated to the desktop window's coordinate system, no matter
...
public class RemoteInput implements Runnable, Input {
        ...
        if (touchEvent != null) {
          switch (touchEvent.type) {
            case TouchEvent.TOUCH_DOWN:
              ...
              processor.touchDown(
                  touchEvent.x, touchEvent.y, touchEvent.pointer, Input.Buttons.LEFT);
  ...
  public boolean isButtonPressed(int button) {
    ...
    for (int i = 0; i < isTouched.length; i++) if (isTouched[i]) return true;
    return false;
  }
  ...
  /**
   * @return the IP addresses {@link RemoteSender} or gdx-remote should connect to. Most likely the
   *     LAN addresses if behind a NAT.
   */
