...
public class ObjectMap<K, V> implements Iterable<ObjectMap.Entry<K, V>> {
    ...
    @Null
    @Nullable()
    public V put(@Nullable() K key, @Null @Nullable() V value) {
        int i = locateKey(key);
        if (i >= 0) {
     ...
     */
    @Null
    public <T extends K> V get(@Nullable() T key) {
        int i = locateKey(key);
     ...
     */
    @Nullable()
    public V get(K key, @Null @Nullable() V defaultValue) {
        int i = locateKey(key);
    ...
    public boolean containsValue(@Null Object value, boolean identity) {
        ...
        if (value == null) {
            K[] keyTable = this.keyTable;
            for (int i = valueTable.length - 1; i >= 0; i--) if (keyTable[i] != null && valueTable[i] == null)
                return true;
        } else if (identity) {
            for (int i = valueTable.length - 1; i >= 0; i--) if (valueTable[i] == value)
                return true;
        } else {
            for (int i = valueTable.length - 1; i >= 0; i--) if (value.equals(valueTable[i]))
                return true;
        }
        return false;
    }
