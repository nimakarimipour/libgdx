...
public class QuadTreeFloat implements Poolable {
  ...
  private static final Pool<QuadTreeFloat> pool =
      new Pool(128, 4096) {
  ...
  public int depth;
  @Nullable public @Null QuadTreeFloat nw, ne, sw, se;
  ...
  /**
   * @param maxValues The maximum number of values stored in each quad tree node. When exceeded, the
   *     node is split into 4 child nodes. If the maxDepth has been reached, more than maxValues may
   *     be stored.
   * @param maxDepth The maximum depth of the tree nodes. Nodes at the maxDepth will not be split
   *     and may store more than maxValues number of entries.
   */
  ...
  private void split(float value, float valueX, float valueY) {
    ...
    for (int i = 0; i < maxValues; i += 3) addToChild(values[i], values[i + 1], values[i + 2]);
      ...
      if (valueY < y + halfHeight)
        ...
        child =
            ne != null
                ? ne
                : (ne =
                    obtainChild(x + halfWidth, y + halfHeight, halfWidth, halfHeight, depth + 1));
    }
  ...
  /**
   * Returns a new length for {@link #values} when it is not enough to hold all the entries after
   * {@link #maxDepth} has been reached.
   */
  ...
  /**
   * @param results For each entry found within the radius, if any, the value, x, y, and square of
   *     the distance to the entry are added to this array. See {@link #VALUE}, {@link #X}, {@link
   *     #Y}, and {@link #DISTSQR}.
   */
  public void query(float centerX, float centerY, float radius, FloatArray results) {
    query(
        centerX, centerY, radius * radius, centerX - radius, centerY - radius, radius * 2, results);
  }
  ...
  private void query(
      ...
      float rectSize,
      FloatArray results) {
    if (!(x < rectX + rectSize && x + width > rectX && y < rectY + rectSize && y + height > rectY))
      return;
    int count = this.count;
    if (count != -1) {
      float[] values = this.values;
  ...
  /**
   * @param results For each entry found within the rectangle, if any, the value, x, and y of the
   *     entry are added to this array. See {@link #VALUE}, {@link #X}, and {@link #Y}.
   */
  public void query(Rectangle rect, FloatArray results) {
    if (x >= rect.x + rect.width
        || x + width <= rect.x
        || y >= rect.y + rect.height
        || y + height <= rect.y) return;
    int count = this.count;
    if (count != -1) {
      float[] values = this.values;
      for (int i = 1; i < count; i += 3) {
  ...
  /**
   * @param result For the entry nearest to the specified point, the value, x, y, and square of the
   *     distance to the value are added to this array after it is cleared. See {@link #VALUE},
   *     {@link #X}, {@link #Y}, and {@link #DISTSQR}.
   * @return false if no entry was found because the quad tree was empty or the specified point is
   *     farther than the larger of the quad tree's width or height from an entry. If false is
   *     returned the result array is empty.
   */
