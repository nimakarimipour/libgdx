...
public class IntFloatMap implements Iterable<IntFloatMap.Entry> {
    ...
    public boolean containsValue(float value) {
        ...
        int[] keyTable = this.keyTable;
        float[] valueTable = this.valueTable;
        for (int i = valueTable.length - 1; i >= 0; i--) if (keyTable[i] != 0 && valueTable[i] == value)
            return true;
        return false;
    }
    ...
    public boolean containsValue(float value, float epsilon) {
        ...
        int[] keyTable = this.keyTable;
        float[] valueTable = this.valueTable;
        for (int i = valueTable.length - 1; i >= 0; i--) if (keyTable[i] != 0 && Math.abs(valueTable[i] - value) <= epsilon)
            return true;
        return false;
    }
