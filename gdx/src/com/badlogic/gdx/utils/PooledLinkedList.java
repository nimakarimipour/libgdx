...
public class PooledLinkedList<T> {
  ...
  @Initializer
    public void add(T object) {
      ...
      item.prev = tail;
      Nullability.castToNonnull(tail, "head initialized if null").next = item;
      tail = item;
      size++;
    }
  ...
  public void remove() {
            ...
            if (c == tail) {
              Nullability.castToNonnull(p, "c is not head").next = null;
              tail = p;
              return;
            }
        
            Nullability.castToNonnull(p, "c is neither head nor tail").next = n;
