...
/**
 * Class with static helper methods related to currently bound OpenGL frame buffer, including access
 * to the current OpenGL FrameBuffer. These methods can be used to get the entire screen content or
 * a portion thereof.
 *
 * @author espitz
 */
public final class ScreenUtils {
  ...
  /**
   * Returns the current framebuffer contents as a {@link TextureRegion} with a width and height
   * equal to the current screen size. The base {@link Texture} always has {@link
   * MathUtils#nextPowerOfTwo} dimensions and RGBA8888 {@link Format}. It can be accessed via {@link
   * TextureRegion#getTexture}. The texture is not managed and has to be reloaded manually on a
   * context loss. The returned TextureRegion is flipped along the Y axis by default.
   */
   ...
   * {@link TextureRegion} with the same dimensions. The base {@link Texture} always has {@link
   * MathUtils#nextPowerOfTwo} dimensions and RGBA8888 {@link Format}. It can be accessed via {@link
   * TextureRegion#getTexture}. This texture is not managed and has to be reloaded manually on a
   * context loss. If the width and height specified are larger than the framebuffer dimensions, the
   * Texture will be padded accordingly. Pixels that fall outside of the current screen will have
   * RGBA values of 0.
   *
  ...
  /**
   * Returns the current framebuffer contents as a byte[] array with a length equal to screen width
   * * height * 4. The byte[] will always contain RGBA8888 data. Because of differences in screen
   * and image origins the framebuffer contents should be flipped along the Y axis if you intend
   * save them to disk as a bitmap. Flipping is not a cheap operation, so use this functionality
   * wisely.
   *
  ...
  /**
   * Returns a portion of the current framebuffer contents specified by x, y, width and height, as a
   * byte[] array with a length equal to the specified width * height * 4. The byte[] will always
   * contain RGBA8888 data. If the width and height specified are larger than the framebuffer
   ...
   * framebuffer contents should be flipped along the Y axis if you intend save them to disk as a
   * bitmap. Flipping is not a cheap operation, so use this functionality wisely.
   *
   * @param flipY whether to flip pixels along Y axis
   */
