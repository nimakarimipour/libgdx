...
public class ObjectFloatMap<K> implements Iterable<ObjectFloatMap.Entry<K>> {
    ...
    public boolean containsValue(float value) {
        K[] keyTable = this.keyTable;
        float[] valueTable = this.valueTable;
        for (int i = valueTable.length - 1; i >= 0; i--) if (keyTable[i] != null && valueTable[i] == value)
            return true;
        return false;
    }
    ...
    public boolean containsValue(float value, float epsilon) {
        K[] keyTable = this.keyTable;
        float[] valueTable = this.valueTable;
        for (int i = valueTable.length - 1; i >= 0; i--) if (keyTable[i] != null && Math.abs(valueTable[i] - value) <= epsilon)
            return true;
        return false;
    }
