 ...
 * {@link #begin()} between indexes 0 and {@link #size} at the time begin was called will not be
 * modified until {@link #end()} is called. If modification of the SnapshotArray occurs between
 * begin/end, the backing array is copied prior to the modification, ensuring that the backing array
 * that was returned by {@link #begin()} is unaffected. To avoid allocation, an attempt is made to
 * reuse any extra array created as a result of this copy on subsequent copies.
 *
 * <p>Note that SnapshotArray is not for thread safety, only for modification during iteration.
 *
 * <p>It is suggested iteration be done in this specific way:
 *
 * <pre>
