...
public class SocketHints {
   ...
   * importance of short connection time, low latency, and high bandwidth. The absolute values of
   * the integers are irrelevant; in order to choose a protocol the values are simply compared, with
   * larger values indicating stronger preferences. Negative values represent a lower priority than
   * positive values. If the application prefers short connection time over both low latency and
   * high bandwidth, for example, then it could invoke this method with the values (1, 0, 0). If the
   * application prefers high bandwidth above low latency, and low latency above short connection
  ...
  /**
   * The traffic class describes the type of connection that shall be established. The traffic class
   * must be in the range 0 <= trafficClass <= 255.
   *
   * <p>The traffic class is bitset created by bitwise-or'ing values such the following :
   *
   * <ul>
  ...
  /**
   * Enable/disable SO_TIMEOUT with the specified timeout, in milliseconds. With this option set to
   * a non-zero timeout, a read() call on the InputStream associated with this Socket will block for
