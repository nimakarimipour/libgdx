...
public class BSpline<T extends Vector<T>> implements Path<T> {
  ...
  public static <T extends Vector<T>> T cubic(
      final T out, final float t, final T[] points, final boolean continuous, final T tmp) {
   ...
   *
   * @param out The Vector to set to the result.
   * @param i The span (0<=i<spanCount) spanCount = continuous ? points.length : points.length - 3
   *     (cubic degree)
   ...
   * @return The value of out
   */
  public static <T extends Vector<T>> T calculate(
      final T out,
      final float t,
      final T[] points,
      final int degree,
   ...
   * @return The value of out
   */
  public static <T extends Vector<T>> T derivative(
      final T out,
      final float t,
      final T[] points,
      final int degree,
   ...
   *
   * @param out The Vector to set to the result.
   * @param i The span (0<=i<spanCount) spanCount = continuous ? points.length : points.length -
   *     degree
   * @param u The position (0<=u<=1) on the span
   * @param points The control points
  ...
  public BSpline set(final T[] controlPoints, final int degree, final boolean continuous) {
    ...
    if (knots == null) knots = new Array<T>(spanCount);
    else {
      knots.clear();
