...
/**
 * This class represents a cumulative distribution. It can be used in scenarios where there are
 * values with different probabilities and it's required to pick one of those respecting the
 * probability. For example one could represent the frequency of the alphabet letters using a
 * cumulative distribution and use it to randomly pick a letter respecting their probabilities
 * (useful when generating random words). Another example could be point generation on a mesh
 * surface: one could generate a cumulative distribution using triangles areas as interval size, in
 * this way triangles with a large area will be picked more often than triangles with a smaller one.
 * See <a href="http://en.wikipedia.org/wiki/Cumulative_distribution_function">Wikipedia</a> for a
...
public class CumulativeDistribution<T> {
  ...
  /**
   * Generate the cumulative distribution in [0,1] where each interval will get a frequency between
   * [0,1]
   */
  ...
  /**
   * Generate the cumulative distribution in [0,1] where each value will have the same frequency and
   * interval size
   */
  ...
  /**
   * Finds the value whose interval contains the given probability Binary search algorithm is used
   * to find the value.
   *
   * @param probability
