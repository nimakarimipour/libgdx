...
/**
 * Encapsulates a general vector. Allows chaining operations by returning a reference to itself in
 * all modification methods. See {@link Vector2} and {@link Vector3} for specific implementations.
 *
 * @author Xoppa
 */
public interface Vector<T extends Vector<T>> {
  ...
  /**
   * This method is faster than {@link Vector#len()} because it avoids calculating a square root. It
   * is useful for comparisons, but not for getting exact lengths, as the return value is the square
   ...
   *
   * <p>This method is slightly faster than limit().
   *
  ...
  /**
   * Sets the length of this vector, based on the square of the desired length. Does nothing if this
   * vector is zero.
   *
   * <p>This method is slightly faster than setLength().
   *
  ...
  /**
   * This method is faster than {@link Vector#dst(Vector)} because it avoids calculating a square
   * root. It is useful for comparisons, but not for getting accurate distances, as the return value
   * is the square of the actual distance.
   *
   * @param v The other vector
  ...
  /**
   * Interpolates between this vector and the given target vector by alpha (within range [0,1])
   * using the given Interpolation method. the result is stored in this vector.
   *
   * @param target The target vector
   * @param alpha The interpolation coefficient
  ...
  /**
   * @return true if this vector is in line with the other vector (either in the same or the
   *     opposite direction)
   */
   ...
   *     float)} && {@link #hasSameDirection(Vector)}).
   */
  ...
  /**
   * @return true if this vector is opposite collinear with the other vector ({@link
   *     #isOnLine(Vector, float)} && {@link #hasOppositeDirection(Vector)}).
   */
  boolean isCollinearOpposite(T other, float epsilon);

  /**
   * @return true if this vector is opposite collinear with the other vector ({@link
   *     #isOnLine(Vector)} && {@link #hasOppositeDirection(Vector)}).
   */
  boolean isCollinearOpposite(T other);

  /**
   * @return Whether this vector is perpendicular with the other vector. True if the dot product is
   *     0.
   */
  ...
  /**
   * @return Whether this vector has similar direction compared to the other vector. True if the
   *     normalized dot product is > 0.
   */
  boolean hasSameDirection(T other);

  /**
   * @return Whether this vector has opposite direction compared to the other vector. True if the
   *     normalized dot product is < 0.
   */
