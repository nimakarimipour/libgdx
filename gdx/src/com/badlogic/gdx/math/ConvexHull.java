...
public class ConvexHull {
  ...
  /**
   * Returns a list of points on the convex hull in counter-clockwise order. Note: the last point in
   * the returned list is the same as the first one.
   */
  /**
   ...
   * @param sorted If false, the points will be sorted by the x coordinate then the y coordinate,
   *     which is required by the convex hull algorithm. If sorting is done the input array is not
   *     modified and count additional working memory is needed.
   * @return pairs of coordinates that describe the convex hull polygon in counterclockwise order.
  ...
  /**
   * Computes a hull the same as {@link #computePolygon(float[], int, int, boolean)} but returns
   * indices of the specified points.
   */
  public IntArray computeIndices(
      float[] points, int offset, int count, boolean sorted, boolean yDown) {
  ...
  /**
   * Sorts x,y pairs of values by the x value, then the y value and stores unsorted original
   * indices.
   *
   * @param count Number of indices, must be even.
   */
  ...
  private int quicksortPartitionWithIndices(
      final float[] values, int lower, int upper, boolean yDown, short[] originalIndices) {
    float x = values[lower];
    float y = values[lower + 1];
    int up = upper;
    int down = lower;
