 ...
 *
 * @see <a
 *     href="http://en.wikipedia.org/wiki/Quaternion">http://en.wikipedia.org/wiki/Quaternion</a>
...
public class Quaternion implements Serializable {
  ...
  /**
   * Constructor, sets the quaternion from the given axis vector and the angle around that axis in
   * degrees.
   *
   * @param axis The axis
   ...
   *
   * @return positive (+1) for north pole, negative (-1) for south pole, zero (0) when no gimbal
   *     lock
   */
  ...
  /**
   * Get the roll euler angle in radians, which is the rotation around the z axis. Requires that
   * this quaternion is normalized.
   *
  ...
  /**
   * Get the roll euler angle in degrees, which is the rotation around the z axis. Requires that
   * this quaternion is normalized.
   *
  ...
  /**
   * Get the pitch euler angle in radians, which is the rotation around the x axis. Requires that
   * this quaternion is normalized.
   *
  ...
  /**
   * Get the pitch euler angle in degrees, which is the rotation around the x axis. Requires that
   * this quaternion is normalized.
   *
  ...
  /**
   * Get the yaw euler angle in radians, which is the rotation around the y axis. Requires that this
  ...
  /**
   * Get the yaw euler angle in degrees, which is the rotation around the y axis. Requires that this
   * quaternion is normalized.
   *
  ...
  // TODO : the matrix4 set(quaternion) doesnt set the last row+col of the matrix to 0,0,0,1 so...
   ...
   * @return This quaternion for chaining.
   */
  public Quaternion setFromAxisRad(
      final float x, final float y, final float z, final float radians) {
   ...
   *
   * <p>Taken from Bones framework for JPCT, see http://www.aptalkarga.com/bones/ which in turn took
  ...
  public Quaternion setFromCross(final Vector3 v1, final Vector3 v2) {
    ...
    return setFromAxisRad(
        v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x, angle);
  }
   ...
   * @return This quaternion for chaining
   */
  public Quaternion setFromCross(
      final float x1,
      final float y1,
      final float z1,
      final float x2,
  ...
  /**
   * Spherical linear interpolation between this quaternion and the other quaternion, based on the
   * alpha value in the range [0,1]. Taken from Bones framework for JPCT, see
   ...
   * Will not destroy the data previously inside the elements of q. result = (q_1^w_1)*(q_2^w_2)*
   * ... *(q_n^w_n) where w_i=1/n.
   *
  ...
  /**
   * Spherical linearly interpolates multiple quaternions by the given weights and stores the result
   * in this Quaternion. Will not destroy the data previously inside the elements of q or w. result
   * = (q_1^w_1)*(q_2^w_2)* ... *(q_n^w_n) where the sum of w_i is 1. Lists must be equal in length.
   *
   * @param q List of quaternions
  ...
  /**
   * Calculates (this quaternion)^alpha where alpha is a real number and stores the result in this
   * quaternion. See http://en.wikipedia.org/wiki/Quaternion#Exponential.2C_logarithm.2C_and_power
   *
  ...
  public Quaternion exp(float alpha) {
    ...
    float coeff = 0;
    if (Math.abs(theta)
        < 0.001) // If theta is small enough, use the limit of sin(alpha*theta) / sin(theta) instead
                 // of actual
      // value
  ...
  /**
   * Get the axis angle representation of the rotation in degrees. The supplied vector will receive
   * the axis (x, y and z values) of the rotation and the value returned is the angle in degrees
   * around that axis. Note that this method will alter the supplied vector, the existing value of
   * the vector is ignored. This will normalize this quaternion if needed. The received axis is a
   * unit vector. However, if this is an identity quaternion (no rotation), then the length of the
   * axis may be zero.
   *
   ...
   * @see <a
   *     href="http://www.euclideanspace.com/maths/geometry/rotations/conversions/quaternionToAngle">calculation</a>
   */
  ...
  /**
   * Get the axis-angle representation of the rotation in radians. The supplied vector will receive
   * the axis (x, y and z values) of the rotation and the value returned is the angle in radians
   * around that axis. Note that this method will alter the supplied vector, the existing value of
   * the vector is ignored. This will normalize this quaternion if needed. The received axis is a
   * unit vector. However, if this is an identity quaternion (no rotation), then the length of the
   * axis may be zero.
  ...
  public float getAxisAngleRad(Vector3 axis) {
    if (this.w > 1)
      this
          .nor(); // if w>1 acos and sqrt will produce errors, this cant happen if quaternion is
                  // normalised
    ...
    double s =
        Math.sqrt(
            1
                - this.w
                    * this
                        .w); // assuming quaternion normalised then w is less than 1, so term always
                             // positive.
    if (s
        < MathUtils
            .FLOAT_ROUNDING_ERROR) { // test to avoid divide by zero, s is always positive due to
                                     // sqrt
  ...
  /**
   * Get the angle in radians of the rotation this quaternion represents. Does not normalize the
   * quaternion. Use {@link #getAxisAngleRad(Vector3)} to get both the axis and the angle of this
   * rotation. Use {@link #getAngleAroundRad(Vector3)} to get the angle around a specific axis.
   *
  ...
  /**
   * Get the angle in degrees of the rotation this quaternion represents. Use {@link
   * #getAxisAngle(Vector3)} to get both the axis and the angle of this rotation. Use {@link
   * #getAngleAround(Vector3)} to get the angle around a specific axis.
   *
  ...
  /**
   * Get the swing rotation and twist rotation for the specified axis. The twist rotation represents
   * the rotation around the specified axis. The swing rotation represents the rotation of the
   * specified axis itself, which is the rotation around an axis perpendicular to the specified
   * axis. The swing and twist rotation can be used to reconstruct the original quaternion: this =
   * swing * twist
   *
   * @param axisX the X component of the normalized axis for which to get the swing and twist
   *     rotation
   * @param axisY the Y component of the normalized axis for which to get the swing and twist
   *     rotation
   * @param axisZ the Z component of the normalized axis for which to get the swing and twist
   *     rotation
   * @param swing will receive the swing rotation: the rotation around an axis perpendicular to the
   *     specified axis
  ...
  /**
   * Get the angle in radians of the rotation around the specified axis. The axis must be
   * normalized.
   *
  ...
  /**
   * Get the angle in degrees of the rotation around the specified axis. The axis must be
   * normalized.
   *
   * @param axisX the x component of the normalized axis for which to get the angle
   * @param axisY the y component of the normalized axis for which to get the angle
