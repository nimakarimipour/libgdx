...
public class Quaternion implements Serializable {
	...
	public Quaternion exp (float alpha) {
		...
		float coeff = 0;
		if (Math.abs(theta) < 0.001) // If theta is small enough, use the limit of sin(alpha*theta) / sin(theta) instead
												// of actual
			// value
	...
	public static final float dot (final float x1, final float y1, final float z1, final float w1, final float x2, final float y2,
	 ...
	 * supplied vector, the existing value of the vector is ignored. This will normalize this quaternion if needed. The received
	 * axis is a unit vector. However, if this is an identity quaternion (no rotation), then the length of the axis may be zero.
	 *
	...
	public float getAxisAngleRad (Vector3 axis) {
		if (this.w > 1) this.nor(); // if w>1 acos and sqrt will produce errors, this cant happen if quaternion is
												// normalised
		float angle = (float)(2.0 * Math.acos(this.w));
		double s = Math.sqrt(1 - this.w * this.w); // assuming quaternion normalised then w is less than 1, so term always
																	// positive.
		if (s < MathUtils.FLOAT_ROUNDING_ERROR) { // test to avoid divide by zero, s is always positive due to
																// sqrt
	 ...
	 * axis perpendicular to the specified axis. The swing and twist rotation can be used to reconstruct the original quaternion:
	 * this = swing * twist
	 *
