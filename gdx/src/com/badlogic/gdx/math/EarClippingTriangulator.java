 ...
 * holes. For more information:
 *
 * <ul>
 *   <li><a
 *       href="http://cgm.cs.mcgill.ca/~godfried/teaching/cg-projects/97/Ian/algorithm2.html">http://cgm.cs.mcgill.ca/~godfried/
 *       teaching/cg-projects/97/Ian/algorithm2.html</a>
 *   <li><a href=
 *       "http://www.geometrictools.com/Documentation/TriangulationByEarClipping.pdf">http://www.geometrictools.com/Documentation
 *       /TriangulationByEarClipping.pdf</a>
 * </ul>
 *
 ...
 *
 * <p>If the polygon vertices are very large or very close together then {@link
 * GeometryUtils#isClockwise(float[], int, int)} may not be able to properly assess the winding
 * (because it uses floats). In that case the vertices should be adjusted, eg by finding the
...
public class EarClippingTriangulator {
   ...
   */
  @Initializer public ShortArray computeTriangles(float[] vertices, int offset, int count) {
  ...
  private boolean isEarTip(int earTipIndex) {
    ...
    for (int i = nextIndex(nextIndex); i != previousIndex; i = nextIndex(i)) {
      ...
      if (vertexTypes[i] != CONVEX) {
        ...
        // note: check the edge defined by p1->p3 first since this fails _far_ more then the other 2
        // checks.
