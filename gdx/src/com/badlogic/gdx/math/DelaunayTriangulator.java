...
public class DelaunayTriangulator {
  ...
  /**
   * Triangulates the given point cloud to a list of triangle indices that make up the Delaunay
   * triangulation.
   *
   * @param points x,y pairs describing points. Duplicate points will result in undefined behavior.
   * @param sorted If false, the points will be sorted by the x coordinate, which is required by the
   *     triangulation algorithm. If sorting is done the input array is not modified, the returned
   *     indices are for the input array, and count*2 additional working memory is needed.
   * @return triples of indices into the points that describe the triangles in clockwise order. Note
   *     the returned array is reused for later calls to the same method.
   */
  public ShortArray computeTriangles(float[] points, int offset, int count, boolean sorted) {
    ...
    for (int pointIndex = offset; pointIndex < end; pointIndex += 2) {
      ...
      // If x,y lies inside the circumcircle of a triangle, the edges are stored and the triangle
      // removed.
      ...
      for (int i = 0, n = edges.size; i < n; i += 2) {
        // Skip multiple edges. If all triangles are anticlockwise then all interior edges are
        // opposite pointing in direction.
        int p1 = edgesArray[i];
        if (p1 == -1) continue;
  ...
  /**
   * Returns INSIDE if point xp,yp is inside the circumcircle made up of the points x1,y1, x2,y2,
   * x3,y3. Returns COMPLETE if xp is to the right of the entire circumcircle. Otherwise returns
   * INCOMPLETE. Note: a point on the circumcircle edge is considered inside.
   */
  private int circumCircle(
      float xp, float yp, float x1, float y1, float x2, float y2, float x3, float y3) {
    float xc, yc;
  ...
  private int quicksortPartition(
      final float[] values, int lower, int upper, short[] originalIndices) {
  ...
  /**
   * Removes all triangles with a centroid outside the specified hull, which may be concave. Note
   * some triangulations may have triangles whose centroid is inside the hull but a portion is
   * outside.
   */
