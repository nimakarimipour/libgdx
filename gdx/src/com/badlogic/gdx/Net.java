 ...
 * communication. To perform an HTTP request create a {@link HttpRequest} with the HTTP method (see {@link HttpMethods} for common
 * methods) and invoke {@link #sendHttpRequest(HttpRequest, HttpResponseListener)} with it and a {@link HttpResponseListener}.
 * After the HTTP request was processed, the {@link HttpResponseListener} is called with a {@link HttpResponse} with the HTTP
 * response values and an status code to determine if the request was successful or not. To create a TCP client socket to
 * communicate with a remote TCP server, invoke the {@link #newClientSocket(Protocol, String, int, SocketHints)} method. The
 * returned {@link Socket} offers an {@link InputStream} and {@link OutputStream} to communicate with the end point. To create a
 * TCP server socket that waits for incoming connections, invoke the {@link #newServerSocket(Protocol, int, ServerSocketHints)}
 * method. The returned {@link ServerSocket} offers an {@link ServerSocket#accept(SocketHints options)} method that waits for an
 * incoming connection.
 *
 * @author mzechner
 * @author noblemaster
...
public interface Net {
	 ...
	 *
	 * <ul>
	 * <li><b>HEAD</b> Asks for a response identical to that of a GET request but without the response body.
	 * <li><b>GET</b> requests a representation of the specified resource. Requests using GET should only retrieve data.
	 * <li><b>POST</b> is used to submit an entity to the specified resource, often causing a change in state or side effects on
	 * the server.
	 * <li><b>PUT</b> replaces all current representations of the target resource with the request payload.
	 ...
	 * <li><strong>content:</strong> A string containing the data to be used when processing the HTTP request.
	 * </ul>
	 *
