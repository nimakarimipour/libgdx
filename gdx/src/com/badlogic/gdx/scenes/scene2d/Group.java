 ...
 *
 * <p>Actors have a z-order equal to the order they were inserted into the group. Actors inserted
 * later will be drawn on top of actors added earlier. Touch events that hit more than one actor are
...
public class Group extends Actor implements Cullable {
  ...
  /**
   * Draws the group and its children. The default implementation calls {@link
   * #applyTransform(Batch, Matrix4)} if needed, then {@link #drawChildren(Batch, float)}, then
  ...
  /**
   * Draws all children. {@link #applyTransform(Batch, Matrix4)} should be called before and {@link
   * #resetTransform(Batch)} after this method if {@link #setTransform(boolean) transform} is true.
   * If {@link #setTransform(boolean) transform} is false these methods don't need to be called,
   * children positions are temporarily offset by the group position when drawn. This method avoids
   * drawing children completely outside the {@link #setCullingArea(Rectangle) culling area}, if
   * set.
   */
      ...
      if (transform) {
        for (int i = 0, n = children.size; i < n; i++) {
          ...
          if (cx <= cullRight
              ...
              && cy + child.height >= cullBottom) child.draw(batch, parentAlpha);
        }
      } else {
  ...
  /**
   * Draws this actor's debug lines if {@link #getDebug()} is true and, regardless of {@link
   * #getDebug()}, calls {@link Actor#drawDebug(ShapeRenderer)} on each child.
   */
  ...
  /**
   * Set the batch's transformation matrix, often with the result of {@link #computeTransform()}.
   * Note this causes the batch to be flushed. {@link #resetTransform(Batch)} will restore the
  ...
  /**
   * Restores the batch transform to what it was before {@link #applyTransform(Batch, Matrix4)}.
  ...
  /**
   * Set the shape renderer transformation matrix, often with the result of {@link
   * #computeTransform()}. Note this causes the shape renderer to be flushed. {@link
   * #resetTransform(ShapeRenderer)} will restore the transform to what it was before this call.
   */
  ...
  /**
   * Restores the shape renderer transform to what it was before {@link #applyTransform(Batch,
   * Matrix4)}. Note this causes the shape renderer to be flushed.
   */
  ...
  /**
   * Children completely outside of this rectangle will not be drawn. This is only valid for use
   * with unrotated and unscaled actors.
   *
  ...
  /**
   * Adds an actor as a child of this group, removing it from its previous parent. If the actor is
   * already a child of this group, no changes are made.
   */
  ...
  /**
   * Adds an actor as a child of this group at a specific index, removing it from its previous
  ...
  /**
   * Adds an actor as a child of this group immediately before another child actor, removing it from
   * its previous parent. If the actor is already a child of this group, no changes are made.
   */
  ...
  /**
   * Adds an actor as a child of this group immediately after another child actor, removing it from
   * its previous parent. If the actor is already a child of this group, no changes are made. If
  ...
  public void addActorAfter(Actor actorAfter, Actor actor) {
    ...
    if (index == children.size || index == -1) children.add(actor);
  ...
  /**
   * Removes an actor from this group and unfocuses it. Calls {@link #removeActor(Actor, boolean)}
   * with true.
   */
  ...
  /**
   * Removes an actor from this group. Calls {@link #removeActorAt(int, boolean)} with the actor's
   * child index.
   */
  ...
  /**
   * Removes all actors from this group and unfocuses them. Calls {@link #clearChildren(boolean)}
   * with true.
   */
  ...
  /**
   * Returns the first actor found with the specified name. Note this recursively compares the name
   * of every actor in the group.
   */
  @Nullable
  ...
  protected void setStage(@Nullable Stage stage) {
    ...
    Actor[] childrenArray = children.items;
    for (int i = 0, n = children.size; i < n; i++)
      childrenArray[i].setStage(
          stage); // StackOverflowError here means the group is its own ascendant.
  }
  ...
  /**
   * When true (the default), the Batch is transformed so children are drawn in their parent's
   * coordinate system. This has a performance impact because {@link Batch#flush()} must be done
   * before and after the transform. If the actors in a group are not rotated or scaled, then the
   * transform for the group can be set to false. In this case, each child's position will be offset
   * by the group's position for drawing, causing the children to appear in the correct location
   * even though the Batch has not been transformed.
   */
  ...
  /**
   * If true, {@link #drawDebug(ShapeRenderer)} will be called for this group and, optionally, all
  ...
  void toString(StringBuilder buffer, int indent) {
    ...
    for (int i = 0, n = children.size; i < n; i++) {
      ...
      if (actor instanceof Group) ((Group) actor).toString(buffer, indent + 1);
      else {
        buffer.append(actor);
        buffer.append('\n');
      }
