...
/**
 * Detects mouse or finger touch drags on an actor. A touch must go down over the actor and a drag
 * won't start until it is moved outside the {@link #setTapSquareSize(float) tap square}. Any touch
...
public class DragListener extends InputListener {
  ...
  public void touchDragged(InputEvent event, float x, float y, int pointer) {
    ...
    if (!dragging
        && (Math.abs(touchDownX - x) > tapSquareSize || Math.abs(touchDownY - y) > tapSquareSize)) {
      dragging = true;
      dragStartX = x;
      dragStartY = y;
