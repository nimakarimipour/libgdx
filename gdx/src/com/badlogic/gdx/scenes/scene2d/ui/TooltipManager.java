...
public class TooltipManager {
   ...
   * #hideAll()} after changing to reset internal state.
   */
  ...
  /**
   * The maximum width of a {@link TextTooltip}. The label will wrap if needed. Default is
   * Integer.MAX_VALUE.
   */
  ...
  /**
   * The distance from the tooltip actor position to the edge of the screen where the actor will be
   * shown on the other side of the mouse cursor. Default is 7.
   */
  ...
  public void enter(Tooltip tooltip) {
    ...
    showTask.cancel();
    if (enabled || tooltip.always) {
      if (time == 0 || tooltip.instant) showTask.run();
      else Timer.schedule(showTask, time);
    }
  }
  ...
  protected void showAction(Tooltip tooltip) {
    ...
    tooltip.container.addAction(
        parallel(fadeIn(actionTime, fade), scaleTo(1, 1, actionTime, Interpolation.fade)));
  }

  /**
   * Called when tooltip is hidden. Default implementation sets actions to animate hiding and to
   * remove the actor from the stage when the actions are complete. A subclass must at least remove
   * the actor.
   */
  protected void hideAction(Tooltip tooltip) {
    tooltip.container.addAction(
        sequence(
            parallel(alpha(0.2f, 0.2f, fade), scaleTo(0.05f, 0.05f, 0.2f, Interpolation.fade)),
            removeActor()));
  }
  ...
  @Nullable public static TooltipManager getInstance() {
