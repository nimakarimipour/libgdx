...
public class HorizontalGroup extends WidgetGroup {
	...
	@SuppressWarnings("NullAway.Init") private FloatArray rowSizes;
	...
	public void layout () {
		...
		(align & Align.left) == 0) x += (getWidth() - prefWidth) / 2;
		float startY;
		...
		for (int r = 0; i != n; i += incr) {
			...
			(align & Align.bottom) == 0) y += (rowHeight - height) / 2;
			if (round)
	...
	private void layoutWrapped () {
		...
		(align & Align.bottom) == 0) rowY += (getHeight() - prefHeight) / 2;
		if (wrapReverse) {
		...
		(align & Align.left) == 0) xStart += (groupWidth - prefWidth) / 2;
		groupWidth -= padRight;
		align = this.rowAlign;
		FloatArray rowSizes = this.rowSizes;
		SnapshotArray<Actor> children = getChildren();
		...
		for (int r = 0; i != n; i += incr) {
			...
			if (x + width > groupWidth || r == 0) {
				...
				(align & Align.left) == 0) x += (maxWidth - rowSizes.get(r)) / 2;
