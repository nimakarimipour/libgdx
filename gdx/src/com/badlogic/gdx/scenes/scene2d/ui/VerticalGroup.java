...
public class VerticalGroup extends WidgetGroup {
	...
	@SuppressWarnings("NullAway.Init") private FloatArray columnSizes;
	...
	public void layout () {
		...
		(align & Align.bottom) == 0) y += (getHeight() - prefHeight) / 2;
		float startX;
		...
		for (int r = 0; i != n; i += incr) {
			...
			(align & Align.left) == 0) x += (columnWidth - width) / 2;
			y -= height + space;
			if (round)
	...
	private void layoutWrapped () {
		...
		(align & Align.left) == 0) columnX += (getWidth() - prefWidth) / 2;
		if ((align & Align.top) != 0)
			yStart += groupHeight - prefHeight;
		else if (// center
		(align & Align.bottom) == 0) yStart += (groupHeight - prefHeight) / 2;
		groupHeight -= padTop;
		align = columnAlign;
		FloatArray columnSizes = this.columnSizes;
		SnapshotArray<Actor> children = getChildren();
		...
		for (int r = 0; i != n; i += incr) {
			...
			if (y - height - space < padBottom || r == 0) {
				...
				(align & Align.top) == 0) y -= (maxHeight - columnSizes.get(r)) / 2;
				if (r > 0) {
					columnX += wrapSpace;
					columnX += columnWidth;
				}
