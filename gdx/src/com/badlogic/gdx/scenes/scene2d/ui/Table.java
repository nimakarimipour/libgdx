...
public class Table extends WidgetGroup {
	...
	@SuppressWarnings("NullAway.Init") static private float[] columnWeightedWidth, rowWeightedHeight;
	...
	@SuppressWarnings("NullAway.Init") private Cell rowDefaults;
	...
	@Null
	@SuppressWarnings("NullAway.Init") Drawable background;
	...
	public void setBackground (@Null Drawable background) {
		...
		// The default pad values use the background's padding.
	...
	public Cell row () {
		if (cells.size > 0) {
			if (!implicitEndRow) {
				...
				if (cells.peek().endRow) return rowDefaults;
				endRow();
			}
			invalidate();
		}
	...
	public void layout () {
		...
		(align & Align.left) == 0) x += (layoutWidth - tableWidth) / 2;
		float y = padTop;
		if ((align & Align.bottom) != 0)
			y += layoutHeight - tableHeight;
		else if (// Center
		(align & Align.top) == 0) y += (layoutHeight - tableHeight) / 2;
	...
	static public class DebugRect extends Rectangle {
		...
		@SuppressWarnings("NullAway.Init") Color color;
	}
