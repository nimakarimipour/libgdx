...
public class Table extends WidgetGroup {
  ...
  private void computeSize() {
    ...
    for (int i = 0; i < cellCount; i++) {
      ...
      if (c.spaceLeft == null) throw new IllegalArgumentException("spaceLeft cannot be null.");
      if (c.padLeft == null) throw new IllegalArgumentException("padLeft cannot be null.");
      c.computedPadLeft =
          c.padLeft.get(a)
              + (column == 0
                  ? 0
                  : Math.max(
                      0,
                      Nullability.castToNonnull(c.spaceLeft, "checked before use").get(a)
                          - spaceRightLast));
      if (c.padTop == null) throw new IllegalArgumentException("padTop cannot be null.");
      c.computedPadTop = c.padTop.get(a);
      if (c.cellAboveIndex != -1) {
        Cell above = (Cell) cells[c.cellAboveIndex];
        if (c.spaceTop == null) throw new IllegalArgumentException("spaceTop cannot be null.");
        if (above.spaceBottom != null) {
          c.computedPadTop +=
              Math.max(
                  0,
                  c.spaceTop.get(a)
                      - Nullability.castToNonnull(above.spaceBottom, "checked before use").get(a));
        }
      }
      if (c.spaceRight == null) throw new IllegalArgumentException("spaceRight cannot be null.");
      if (c.padRight == null) throw new IllegalArgumentException("padRight cannot be null.");
      float spaceRight = Nullability.castToNonnull(c.spaceRight, "checked before use").get(a);

      c.computedPadRight = c.padRight.get(a) + ((column + colspan) == columns ? 0 : spaceRight);
      if (c.padBottom == null) throw new IllegalArgumentException("padBottom cannot be null.");
      if (c.spaceBottom == null) throw new IllegalArgumentException("spaceBottom cannot be null.");
