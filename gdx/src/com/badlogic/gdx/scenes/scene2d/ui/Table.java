  ...
  private void computeSize() {
    ...
    for (int i = 0; i < cellCount; i++) {
      ...
      if (c.padLeft != null && c.spaceLeft != null) {
        c.computedPadLeft =
            Nullability.castToNonnull(c.padLeft, "checked not null").get(a)
                + (column == 0 ? 0 : Math.max(0, c.spaceLeft.get(a) - spaceRightLast));
      } else {
        c.computedPadLeft = column == 0 ? 0 : Math.max(0, -spaceRightLast);
      }

      if (c.padTop != null) {
        c.computedPadTop = Nullability.castToNonnull(c.padTop, "explicit null check").get(a);
        if (c.cellAboveIndex != -1) {
          Cell above = (Cell) cells[c.cellAboveIndex];
          if (c.spaceTop != null && above.spaceBottom != null) {
      ...
      if (c.spaceRight != null && c.padRight != null) {
        float spaceRight = c.spaceRight.get(a);
        c.computedPadRight =
            Nullability.castToNonnull(c.padRight, "null checked already").get(a)
                + ((column + colspan) == columns ? 0 : spaceRight);
        spaceRightLast = spaceRight;
      } else {
        c.computedPadRight = (column + colspan) == columns ? 0 : spaceRightLast;
      }

      if (c.padBottom != null && c.spaceBottom != null) {
    ...
    for (int i = 0; i < cellCount; i++) {
      ...
      if (expandX != 0) {
        ...
        for (int ii = column; ii < nn; iii++) expandWidth[ii] = expandX;
      }
