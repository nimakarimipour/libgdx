...
public class ScrollPane extends WidgetGroup {
     ...
     * @param widget May be null.
     */
    public ScrollPane(@Null @Nullable() Actor widget, ScrollPaneStyle style) {
        if (style == null)
        ...
        flickScrollListener = new ActorGestureListener() {
            ...
            public boolean handle(Event event) {
                ...
                event instanceof InputEvent && ((InputEvent) event).isTouchFocusCancel())
                    cancel();
                return false;
            }
        };
    ...
    public void act(float delta) {
        ...
        // 
        (!touchScrollH || (scrollX && maxX / (hScrollBounds.width - hKnobBounds.width) > widgetArea.width * 0.1f)) && (!touchScrollV || (scrollY && maxY / (vScrollBounds.height - vKnobBounds.height) > widgetArea.height * 0.1f)))) {
    ...
    public void layout() {
        ...
        if (scrollX) {
            if (hScrollKnob != null) {
                ...
                if (variableSizeKnobs)
                    hKnobBounds.width = Math.max(hScrollKnob.getMinWidth(), (int) (hScrollBounds.width * widgetArea.width / widgetWidth));
                else
        ...
        if (scrollY) {
            if (vScrollKnob != null) {
                ...
                if (variableSizeKnobs)
                    vKnobBounds.height = Math.max(vScrollKnob.getMinHeight(), (int) (vScrollBounds.height * widgetArea.height / widgetHeight));
                else
     ...
     */
    public void setActor(@Null @Nullable() Actor actor) {
        if (widget == this)
