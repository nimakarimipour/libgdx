...
public class Tree<N extends Node, V> extends WidgetGroup {
    ...
    private void initialize() {
        addListener(clickListener = new ClickListener() {
            ...
            public void exit(InputEvent event, float x, float y, int pointer, @Null @Nullable Actor toActor) {
     ...
     */
    protected float drawIcons(Batch batch, float r, float g, float b, float a, @Null @Nullable N parent, Array<N> nodes, float indent, float plusMinusWidth) {
    ...
    protected Drawable getExpandIcon(N node, float iconX) {
        ...
        // 
        // 
        node == overNode && Gdx.app.getType() == ApplicationType.Desktop && (!selection.getMultiple() || (!UIUtils.ctrl() && !UIUtils.shift()))) {
    ...
    private float getNodeAt(Array<N> nodes, float y, float rowY) {
        for (int i = 0, n = nodes.size; i < n; i++) {
            N node = nodes.get(i);
            float height = node.height;
            // Node subclass may increase getHeight.
    ...
    public void updateRootNodes() {
        ...
        for (int i = 0, n = rootNodes.size, actorIndex = 0; i < n; i++) actorIndex += rootNodes.get(i).addToTree(this, actorIndex);
    }
     ...
     */
    public void setOverNode(@Null @Nullable N overNode) {
    ...
    static abstract public class Node<N extends Node, V, A extends Actor> {
        ...
        public void setExpanded(boolean expanded) {
            ...
            if (expanded) {
                for (int i = 0, n = this.children.size; i < n; i++) actorIndex += ((N) children[i]).addToTree(tree, actorIndex);
            } else {
                for (int i = 0, n = this.children.size; i < n; i++) ((N) children[i]).removeFromTree(tree, actorIndex);
            }
        }
        ...
        protected int addToTree(Tree<N, V> tree, int actorIndex) {
            ...
            Object[] children = this.children.items;
            for (int i = 0, n = this.children.size; i < n; i++) childIndex += ((N) children[i]).addToTree(tree, childIndex);
