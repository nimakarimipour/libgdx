...
public class Actor {
    ...
    public boolean notify(Event event, boolean capture) {
        ...
        try {
            listeners.begin();
            for (int i = 0, n = listeners.size; i < n; i++) if (listeners.get(i).handle(event))
                event.handle();
            listeners.end();
        } catch (RuntimeException ex) {
            String context = toString();
     ...
     */
    public void removeAction(@Null @Nullable Action action) {
     ...
     */
    public boolean isAscendantOf(@Nullable Actor actor) {
        if (actor == null)
            throw new IllegalArgumentException("actor cannot be null.");
        do {
            if (actor == this)
     ...
     */
    protected void setParent(@Null @Nullable Group parent) {
        this.parent = parent;
    }
    ...
    public boolean isTouchFocusTarget() {
        ...
        if (stage == null)
            return false;
        for (int i = 0, n = stage.touchFocuses.size; i < n; i++) if (stage.touchFocuses.get(i).target == this)
            return true;
        return false;
    }
    ...
    public boolean isTouchFocusListener() {
        ...
        if (stage == null)
            return false;
        for (int i = 0, n = stage.touchFocuses.size; i < n; i++) if (stage.touchFocuses.get(i).listenerActor == this)
            return true;
        return false;
    }
     ...
     */
    public Vector2 localToAscendantCoordinates(@Null @Nullable Actor ascendant, Vector2 localCoords) {
        Actor actor = this;
        do {
