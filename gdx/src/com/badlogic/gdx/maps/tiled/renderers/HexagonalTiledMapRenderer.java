...
public class HexagonalTiledMapRenderer extends BatchTiledMapRenderer {
  ...
  /**
   * the parameter defining the shape of the hexagon from tiled. more specifically it represents the
   * length of the sides that are parallel to the stagger axis. e.g. with respect to the stagger
   * axis a value of 0 results in a rhombus shape, while a value equal to the tile length/height
   * represents a square shape and a value of 0.5 represents a regular hexagon if tile length equals
   * tile height
   */
  ...
  public void renderTileLayer(TiledMapTileLayer layer) {
    ...
    if (staggerAxisX) {
      ...
      final int row1 =
          Math.max(0, (int) ((viewBounds.y - layerTileHeight50 - layerOffsetX) / layerTileHeight));
      final int row2 =
          Math.min(
              layerHeight,
              (int)
                  ((viewBounds.y + viewBounds.height + layerTileHeight - layerOffsetX)
                      / layerTileHeight));
      ...
      final int col1 =
          Math.max(
              0,
              (int)
                  (((viewBounds.x - tileWidthLowerCorner - layerOffsetY) / tileWidthUpperCorner)));
      final int col2 =
          Math.min(
              layerWidth,
              (int)
                  ((viewBounds.x + viewBounds.width + tileWidthUpperCorner - layerOffsetY)
      ...
      final int row1 =
          Math.max(
              0,
              (int)
                  (((viewBounds.y - tileHeightLowerCorner - layerOffsetX)
                      / tileHeightUpperCorner)));
      final int row2 =
          Math.min(
              layerHeight,
              (int)
                  ((viewBounds.y + viewBounds.height + tileHeightUpperCorner - layerOffsetX)
                      / tileHeightUpperCorner));

      final int col1 =
          Math.max(0, (int) (((viewBounds.x - layerTileWidth50 - layerOffsetY) / layerTileWidth)));
      final int col2 =
          Math.min(
              layerWidth,
              (int)
                  ((viewBounds.x + viewBounds.width + layerTileWidth - layerOffsetY)
                      / layerTileWidth));

      float shiftX = 0;
      for (int row = row2 - 1; row >= row1; row--) {
        // depending on the stagger index either shift for even or uneven indexes
        if ((row % 2 == 0) == staggerIndexEven) shiftX = layerTileWidth50;
        else shiftX = 0;
  ...
  private void renderCell(
      @Nullable final TiledMapTileLayer.Cell cell,
      final float x,
      final float y,
      final float color) {
    if (cell != null) {
