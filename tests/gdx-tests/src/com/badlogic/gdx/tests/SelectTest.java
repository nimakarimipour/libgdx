...
public class SelectTest extends GdxTest {
  ...
  public static void correctnessTest(int runs, int k) {
    String msg =
        String.format("[%d runs with %dx%d dummy game units] - ", runs, player.size, enemy.size);
    verify = true;
    test(runs, k);
    print(msg + "VERIFIED");
  }
    ...
    print(
        msg
            + String.format(
                ...
                allPerf.time.min,
                allPerf.time.max,
                allPerf.time.average * 1000,
                allPerf.time.total * 1000,
                comparisonsMade));
  }
  ...
  public static void getKthNearestEnemy(boolean silent, int k) {
    ...
    print(
        String.format(
            ...
            perf.time.min * 1000,
            perf.time.max * 1000,
            perf.time.average * 1000,
            perf.time.total * 1000));
  }
  ...
  static Comparator<Dummy> distComp =
      new Comparator<Dummy>() {
        @Override
