...
public class PixmapPackerTest extends GdxTest {
  ...
  public void create() {
    ...
    animation =
        new Animation<TextureRegion>(
            0.33f, atlas.findRegions("badlogic-anim"), Animation.PlayMode.LOOP);

    textureRegions = new Array<TextureRegion>();
    packer.updateTextureRegions(
        textureRegions, TextureFilter.Nearest, TextureFilter.Nearest, false);
  ...
  public void render() {
    ...
    batch.draw(
        animation.getKeyFrame(stateTime), 30 + (quarterSize * 2), 10, quarterSize, quarterSize);
    batch.end();
