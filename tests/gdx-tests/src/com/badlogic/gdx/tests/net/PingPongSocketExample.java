...
/**
 * Demonstrates how to do very simple socket programming. Implements a classic PING-PONG sequence,
 * client connects to server, sends message, server sends message back to client. Both client and
 * server run locally. We quit as soon as the client received the PONG message from the server. This
 * example won't work in HTML. Messages are delimited by the new line character, so we can use a
 * {@link BufferedReader}.
 *
 * @author badlogic
 */
