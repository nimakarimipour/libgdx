...
public class TextAreaTest3 extends GdxTest {
  ...
  public void create() {
    ...
    // default font in the skin has line height == text height, so its impossible to see updated
    ...
    styleLSans15.font =
        new BitmapFont(
            Gdx.files.internal("data/lsans-15.fnt"),
            Gdx.files.internal("data/lsans-15_00.png"),
            false);
    ...
    styleLSans32.font =
        new BitmapFont(
            Gdx.files.internal("data/lsans-32.fnt"),
            Gdx.files.internal("data/lsans-32.png"),
            false);
  ...
  private Button newStyleButton(
      final String label, final TextField.TextFieldStyle style, ButtonGroup<TextButton> group) {
  ...
  public void render() {
    ...
    stage.act();
    stage.draw();
    // getLines() does not return correct count before first draw happens and there is no other way
    // to trigger update
    textArea.setPrefRows(textArea.getLines());
    // change does not propagate on its own, number of lines does not change so calling this each
