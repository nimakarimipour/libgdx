...
/**
 * This tests both {@link Actor#parentToLocalCoordinates(Vector2)} and {@link
...
public class GroupTest extends GdxTest {
  ...
  public void render() {
    ...
    if (MathUtils
        .randomBoolean()) { // So we see when they are drawn on top of each other (which should be
                            // always).
      renderer.setColor(Color.GREEN);
      renderer.circle(
          group1.toScreenCoordinates.x, Gdx.graphics.getHeight() - group1.toScreenCoordinates.y, 5);
      renderer.setColor(Color.RED);
      renderer.circle(
          group1.localToParentCoordinates.x,
          Gdx.graphics.getHeight() - group1.localToParentCoordinates.y,
          5);
    } else {
      renderer.setColor(Color.RED);
      renderer.circle(
  ...
  class TestGroup extends Group {
    ...
    public void draw(Batch batch, float parentAlpha) {
      ...
      toScreenCoordinates
          ...
          .scl(getScaleX(), getScaleY())
          .rotateDeg(getRotation())
