  ...
  public void create() {
    ...
    cam =
        new OrthographicCamera(
            45, 45 * (Gdx.graphics.getWidth() / (float) Gdx.graphics.getHeight()));
  ...
  public void render() {
    ...
    for (int i = 0; i < instances.length; i++) {
      ...
      if (cam.frustum.sphereInFrustum(pos, 1)) {
        ((ColorAttribute) instances[i].materials.get(0).get(ColorAttribute.Diffuse))
    ...
    font.draw(
        batch,
        "visible: " + visible + "/100" + ", fps: " + Gdx.graphics.getFramesPerSecond(),
        0,
        20);
    batch.end();
  }
