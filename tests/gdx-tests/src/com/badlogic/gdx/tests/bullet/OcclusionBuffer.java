...
/**
 * Software rasterizer used for depth rendering and testing of bounding box triangles. Stores depth
 * values inside a {@link FloatBuffer}. CPU rendering is used in order to avoid the frequent GPU to
 * CPU synchronization which would be needed if hardware rendering were to be used for occlusion
 * culling queries.
 *
 * <p>Based on the algorithm from the Bullet CDTestFramework, BulletSAPCompleteBoxPruningTest.cpp,
...
public class OcclusionBuffer implements Disposable {
  ...
  private enum Policy {
    ...
    /**
     * Compare the current value in the depth buffer with a new value. If draw policy is used, write
     * the new value if it is larger than current depth (closer to camera). If query is used, return
     * true if new depth not occluded by old depth.
     *
  ...
  static class Quaternion extends com.badlogic.gdx.math.Quaternion {
    ...
    /**
     * Multiply the x,y,z,w components of the passed in quaternion with the scalar and add them to
  ...
  private static final int[] WINDING = {
    1, 0, 3, 2, 4, 5, 6, 7, 4, 7, 3, 0, 6, 5, 1, 2, 7, 6, 2, 3, 5, 4, 0, 1
  };
  /** Depth buffer */
  ...
  /**
   * Renders a bounding box to the depth buffer. Does not need to be axis aligned, but will use the
   * translation, rotation and scale from the matrix parameter.
   *
  ...
  /**
   * Draws a bounding box to the depth buffer, or queries the depth buffer at the pixels the box
   * occupies, depending on policy.
   *
   * @param vertices Vertices of box
   * @param policy Rasterization policy to use
   * @return True if query policy is used, and any part of the box passes a depth test. False
   *     otherwise.
   */
  private boolean drawBox(Vector3[] vertices, Policy policy) {
    for (int i = 0; i < 8; i++) {
      // Multiply the world coordinates by the camera combined matrix, but do not divide by w
      // component yet.
   ...
   *
   * <p>If used with {@link Policy#DRAW} the triangle will be drawn to the depth buffer wherever it
   * passes a depth test.
   *
   * <p>If {@link Policy#QUERY} is used, the depth values in the triangle will be compared with
   * existing depth buffer values. If any pixel passes a depth test the rasterization will be
   * aborted and the method will return true.
   *
   ...
   * @return With query policy, true if any pixel in the triangle passes a depth test. False
   *     otherwise.
   */
  private boolean drawTriangle(Vector3 a, Vector3 b, Vector3 c, Policy policy) {
    ...
    triX.set(
        (int) (a.x * bufferHalfExt.x + bufferOffset.x),
        (int) (b.x * bufferHalfExt.x + bufferOffset.x),
        (int) (c.x * bufferHalfExt.x + bufferOffset.x));
    triY.set(
        (int) (a.y * bufferHalfExt.y + bufferOffset.y),
        (int) (b.y * bufferHalfExt.y + bufferOffset.y),
    ...
    triDY.set(
        triX.y - triX.x - triDX.x * width,
        triX.z - triX.y - triDX.y * width,
        triX.x - triX.z - triDX.z * width);
    cursor.set(
        yMin * (triX.y - triX.x) + xMin * (triY.x - triY.y) + triX.x * triY.y - triX.y * triY.x,
    ...
    float dzy =
        ia * (triX.x * (b.z - c.z) + triX.y * (c.z - a.z) + triX.z * (a.z - b.z)) - (dzx * width);
    ...
    for (int iy = yMin; iy < yMax; iy++) {
      for (int ix = xMin; ix < xMax; ix++) {
        ...
        if (cursor.x >= 0
            && cursor.y >= 0
            && cursor.z >= 0
            && policy.process(buffer, bufferIndex, drawDepth)) return true;
        cursor.add(triDX);
        drawDepth += dzx;
      }
      cursor.add(triDY);
  ...
  /**
   * Queries the depth buffer as to whether an AABB (axis aligned bounding box) is completely
   * occluded by a previously rendered object. If any part of the AABB is visible (not occluded),
