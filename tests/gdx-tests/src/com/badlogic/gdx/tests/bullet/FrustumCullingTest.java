...
public class FrustumCullingTest extends BaseBulletTest {
  ...
  static final int CULL_FRUSTUM = 1;
  /** Transform the render cam with the frustum */
  static final int FRUSTUM_CAM = 2;

  static final boolean USE_BULLET_FRUSTUM_CULLING = true;

  int state = 0; // 0 = No culling, look from above

  static final int BOXCOUNT = 200;

  static final float BOX_X_MIN = -25;
  static final float BOX_Y_MIN = -25;
  static final float BOX_Z_MIN = -25;

  static final float BOX_X_MAX = 25;
  static final float BOX_Y_MAX = 25;
  static final float BOX_Z_MAX = 25;

  static final float SPEED_X = 360f / 7f;
  static final float SPEED_Y = 360f / 19f;
  static final float SPEED_Z = 360f / 13f;

  static final Vector3 tmpV = new Vector3();
  static final Matrix4 tmpM = new Matrix4();

  static final int ptrs[] = new int[512];
  static final Array<btCollisionObject> visibleObjects = new Array<btCollisionObject>();
  ...
  public static btPairCachingGhostObject createFrustumObject(final Vector3... points) {
    ...
    // Using a compound shape is not necessary, but it's good practice to create shapes around the
    // center.
    if (USE_COMPOUND) {
  ...
  public static Array<BulletEntity> getEntitiesCollidingWithObject(
      final BulletWorld world,
      final btCollisionObject object,
      final Array<BulletEntity> out,
      final btPersistentManifoldArray tmpArr) {
    // Fetch the array of contacts
    btBroadphasePairArray arr =
        world.broadphase.getOverlappingPairCache().getOverlappingPairArray();
    // Get the user values (which are indices in the entities array) of all objects colliding with
    // the object
  ...
  public static Model createFrustumModel(final Vector3... p) {
    ...
    mpb.vertex(
        p[0].x, p[0].y, p[0].z, 0, 0, 1, p[1].x, p[1].y, p[1].z, 0, 0, 1, p[2].x, p[2].y, p[2].z, 0,
        0, 1, p[3].x, p[3].y, p[3].z, 0, 0, 1, // near
        p[4].x, p[4].y, p[4].z, 0, 0, -1, p[5].x, p[5].y, p[5].z, 0, 0, -1, p[6].x, p[6].y, p[6].z,
        0, 0, -1, p[7].x, p[7].y, p[7].z, 0, 0, -1);
    mpb.index(
        (short) 0, (short) 1, (short) 1, (short) 2, (short) 2, (short) 3, (short) 3, (short) 0);
    mpb.index(
        (short) 4, (short) 5, (short) 5, (short) 6, (short) 6, (short) 7, (short) 7, (short) 4);
    mpb.index(
        (short) 0, (short) 4, (short) 1, (short) 5, (short) 2, (short) 6, (short) 3, (short) 7);
    return builder.end();
  }
  ...
  public void create() {
    ...
    frustumCam =
        new PerspectiveCamera(camera.fieldOfView, camera.viewportWidth, camera.viewportHeight);
  ...
  public boolean tap(float x, float y, int count, int button) {
    state = (state + 1) % 3;
    if ((state & FRUSTUM_CAM) == FRUSTUM_CAM) camera = frustumCam;
