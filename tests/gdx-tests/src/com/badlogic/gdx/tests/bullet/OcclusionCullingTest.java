...
public class OcclusionCullingTest extends BaseBulletTest {
  ...
  private enum CullingPolicy {
    /**
     * Occlusion culling, renders only entities which are visible from the viewpoint of a camera.
     * Objects which are hidden behind other objects (occluded) do not need to be rendered.
     */
    OCCLUSION,
    /** No culling, renders all objects. */
    NONE,
    /**
     * Simple culling which loops through all entities in the world, and checks if the radius of the
     * object bounding box is inside the camera frustum. Hidden surfaces are not taken into account.
     * Bullet is not used.
     */
    SIMPLE,
    /**
     * Same as {@link CullingPolicy#SIMPLE}, except instead of checking each object in the world,
     * the process is accelerated by the Bullet broadphase bounding volume tree.
     */
    KDOP;
  ...
  public static final short CF_OCCLUDER_OBJECT = 512;
  /** Amount of occludee entities to spawn at program start */
  private static final int STARTING_OCCLUDEE_AMOUNT = 300;
  /** Number of occludee entities to spawn at key press */
  private static final int KEY_SPAWN_OCCLUDEE_AMOUNT = 100;
  ...
  private static final float FRUSTUM_CAMERA_FAR = 50f;
  private static final float FRUSTUM_CAMERA_FOV = 60f;
  private static final float FRUSTUM_ANG_SPEED = 360f / 15f;
  private static final float FRUSTUM_LIN_SPEED = -6f;
  private static final float FRUSTUM_MOVE_RADIUS = 12;

  // Occludee models and textures used in test
  private static final String DEFAULT_TEX_PATH = "data/g3d/checkboard.png";
  private static final String[] OCCLUDEE_PATHS_DYNAMIC =
      new String[] {
        "data/car.obj",
        "data/wheel.obj",
        "data/cube.obj",
        "data/g3d/ship.obj",
        "data/g3d/shapes/sphere.g3dj",
        "data/g3d/shapes/torus.g3dj",
      };
  private static final String[] OCCLUDEE_PATHS_STATIC = new String[OCCLUDEE_PATHS_DYNAMIC.length];
  private static final float OCCLUDEE_MAX_EXTENT = 1.5f;
  private static final Vector3 OCCLUDER_DIM = new Vector3(1f, 6f, 20f);
  private static final Vector3 GROUND_DIM = new Vector3(120, 1, 120);

  private static final int USE_FRUSTUM_CAM = 1;
  private static final int PAUSE_FRUSTUM_CAM = 2;
  private static final int SHOW_DEBUG_IMAGE = 4;
  ...
  private BulletEntity addOccluder(String type, float rotationY, Vector3 position) {
    ...
    e.body.setWorldTransform(
        e.transform.setToRotation(Vector3.Y, rotationY).setTranslation(position));
  ...
  public void create() {
    ...
    frustumCam =
        new PerspectiveCamera(FRUSTUM_CAMERA_FOV, camera.viewportWidth, camera.viewportHeight);
    frustumCam.far = FRUSTUM_CAMERA_FAR;
    frustumCam.update(true);
    final Model frustumModel =
        FrustumCullingTest.createFrustumModel(frustumCam.frustum.planePoints);
    frustumModel
        .materials
        .first()
        .set(new ColorAttribute(ColorAttribute.AmbientLight, Color.WHITE));
    ...
    world.addConstructor(
        "wall",
        new BulletConstructor(occluderModel, 0, new btBoxShape(tmpV1.set(OCCLUDER_DIM).scl(0.5f))));
    ...
    TextureAttribute defaultTexture =
        new TextureAttribute(TextureAttribute.Diffuse, assets.get(DEFAULT_TEX_PATH, Texture.class));
  ...
  private boolean entityInFrustum(BulletEntity entity) {
    ...
    return frustumCam.frustum.sphereInFrustum(
        tmpV1.add(entity.boundingBox.getCenter(tmpV2)), entity.boundingBoxRadius);
  }
  ...
  public void update() {
    ...
    frustumInstance
        .transform
        .idt()
        .rotate(Vector3.Y, frustumCamAngleY = (frustumCamAngleY + dt * FRUSTUM_ANG_SPEED) % 360);
    ...
    frustumCamPos
        .add(tmpV1.set(Vector3.Y).crs(tmpV2.set(frustumCamPos).nor()).scl(dt * FRUSTUM_LIN_SPEED))
        .nor()
