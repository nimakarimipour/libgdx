 ...
 * actors that do not scale or rotate. It is not a general solution as it assumes that actors and
 * groups are only translated (moved, change their x/y coordinates). NOTE: This has been obsoleted
 * by {@link Cullable}.
 *
 * @author mzechner
 */
public class SimpleStageCullingTest extends GdxTest {
  ...
  /**
   * We need to extend a base actor class so we can add the culling in the render method. We also
   * add a method to get the stage coordinates of the actor so we can cull it against the camera's
   * view volume.
   *
   * @author mzechner
   */
  ...
  public void create() {
    ...
    ;
    camController =
        new OrthoCamController(
            (OrthographicCamera) stage.getCamera()); // we know it's an ortho cam at this point!
    ...
    // we also want to output the number of visible actors, so we need a SpriteBatch and a
    // BitmapFont
  ...
  public void render() {
    ...
    batch.begin();
    font.draw(
        batch, "Visible: " + numVisible + ", fps: " + Gdx.graphics.getFramesPerSecond(), 20, 30);
    batch.end();
  }
