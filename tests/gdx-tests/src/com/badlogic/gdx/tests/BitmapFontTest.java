...
public class BitmapFontTest extends GdxTest {
  ...
  public void create() {
    ...
    // font = new FreeTypeFontGenerator(Gdx.files.internal("data/lsans.ttf")).generateFont(new
    ...
    // Notice that the last [] has been deliberately added to test the effect of excessive pop
    // operations.
    // They are silently ignored, as expected.
    label =
        new Label(
            "<<[BLUE]M[RED]u[YELLOW]l[GREEN]t[OLIVE]ic[]o[]l[]o[]r[]*[MAROON]Label[][] [Unknown Color]>>",
            skin);
  ...
  public void render() {
    ...
    if (true) {
      ...
      for (int i = 0, n = layout.runs.size; i < n; i++) {
        if (i % 3 == 0) renderer.setColor(c, 0, c, 1);
        else if (i % 2 == 0) renderer.setColor(0, c, c, 1);
        else renderer.setColor(c, c, 0, 1);
    ...
    if (false) {
      ...
      textX +=
          cache.addText(
                  "[#A52A2A]br[#A52A2ADF]ow[#A52A2ABF]n f[#A52A2A9F]ad[#A52A2A7F]in[#A52A2A5F]g o[#A52A2A3F]ut ",
                  textX,
                  150)
              .width;
      ...
      final String trogdor =
          "TROGDOR! TROGDOR! Trogdor was a man! Or maybe he was a... Dragon-Man!";
      cache.clear();
