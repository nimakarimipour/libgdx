...
public class TextureDownloadTest extends GdxTest {
  ...
  public void create() {
    new Thread(
            new Runnable() {
              /**
               * Downloads the content of the specified url to the array. The array has to be big
               * enough.
               */
              ...
              public void run() {
                byte[] bytes =
                    new byte[200 * 1024]; // assuming the content is not bigger than 200kb.
                int numBytes =
                    download(
                        bytes,
                        "https://www.badlogicgames.com/wordpress/wp-content/uploads/2012/01/badlogic-new.png");
                if (numBytes != 0) {
                  // load the pixmap, make it a power of two if necessary (not needed for GL ES
                  // 2.0!)
                  ...
                  Gdx.app.postRunnable(
                      new Runnable() {
                        ...
                        public void run() {
                          image =
                              new TextureRegion(
                                  new Texture(potPixmap), 0, 0, originalWidth, originalHeight);
                        }
                      });
                }
              }
