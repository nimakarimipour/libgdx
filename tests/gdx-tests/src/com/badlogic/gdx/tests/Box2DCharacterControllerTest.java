...
public class Box2DCharacterControllerTest extends GdxTest implements ApplicationListener {
  ...
  static final float MAX_VELOCITY = 14f;
  boolean jump = false;
  World world;
  Body player;
  ...
  private void createWorld() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      box.setTransform(
          (float) Math.random() * 10f - (float) Math.random() * 10f,
  ...
  public void render() {
    ...
    if (!grounded) {
      ...
      if (groundedPlatform != null
          && groundedPlatform instanceof MovingPlatform
    ...
    font.draw(
        batch,
        "friction: " + playerPhysicsFixture.getFriction() + "\ngrounded: " + grounded,
        point.x + 20,
        point.y);
    batch.end();
  }
  ...
  class MovingPlatform extends Platform {
    ...
    public MovingPlatform(
        float x, float y, float width, float height, float dx, float dy, float da, float maxDist) {
