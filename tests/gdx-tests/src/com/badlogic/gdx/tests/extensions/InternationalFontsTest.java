 ...
 * their huge amount of glyphs which don't fit into a single texture. Note that you don't have to use the FreeType extension for
 * this, you can generate fonts with Hiero as well. The FreeType extension allows you to generate fonts at runtime for different
 * screen densities. It is not portable to GWT! For each script examplified below we only use a few characters from the respective
 * alphabet. You'll have to pass in a string containing all the printable characters of that language that you want to use!
 *
 * @author mzechner */
