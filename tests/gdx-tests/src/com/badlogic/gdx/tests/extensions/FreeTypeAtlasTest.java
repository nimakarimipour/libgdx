 ...
 *
 * <p>This example uses enum ordinals for fast access to a two-dimensional array, which stores
 * BitmapFonts by size and style. A more flexible solution might be to use an ObjectMap and and
 * IntMap instead.
 *
 * <p>This test uses a less efficient but more convenient way to pack multiple generated fonts into
 * a single texture atlas compared with FreeTypePackTest - each texture page will be generated and
 * refreshed once. Refreshing textures is slow on mobile devices.
 */
public class FreeTypeAtlasTest extends GdxTest {
  ...
  static enum FontStyle {
    Regular("data/lsans.ttf"),
    Italic("data/lsans-italic.ttf");
  ...
  public void create() {
    ...
    text =
        glyphCount + " glyphs packed in " + packer.getPages().size + " page(s) in " + time + " ms";
  }
  ...
  public void render() {
    ...
    for (FontStyle style : FontStyle.values()) {
      for (FontSize size : FontSize.values()) {
        ...
        fnt.draw(
            batch,
            style.name() + " " + size.size + "pt: The quick brown fox jumps over the lazy dog",
            x,
            y);
  ...
  protected int createFonts() {
    // This test uses a less efficient but more convenient way to pack multiple generated fonts into
    // a single
    // texture atlas.
    //
    ...
    // 3. For each size and style, call generator.generateFont() with the packer set on the
    // parameter
