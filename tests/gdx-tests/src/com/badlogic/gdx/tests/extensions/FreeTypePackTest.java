...
/**
 * An advanced example of packing many glyphs into a single texture atlas, using
 * FreeTypeFontGenerator.
 *
 * <p>This example uses enum ordinals for fast access to a two-dimensional array, which stores
 * BitmapFonts by size and style. A more flexible solution might be to use an OjectMap and and
 * IntMap instead.
 *
...
public class FreeTypePackTest extends GdxTest {
  ...
  public void dispose() {
    super.dispose();
    for (TextureRegion r : regions) r.getTexture().dispose(); // dispose the texture since we own it
    batch.dispose();
  }
  ...
  protected int createFonts() {
    // //////////////////////////////////////////////////////////////////////////////////////////////////////
    // //////Steps to use multiple FreeTypeFontGenerators with a single texture
    // atlas://////////////////////
    ...
    // 8. Use each BitmapFontData to construct a new BitmapFont, and specify your TextureRegion(s)
    ...
    PixmapPacker packer =
        new PixmapPacker(FONT_ATLAS_WIDTH, FONT_ATLAS_HEIGHT, Format.RGBA8888, 2, false);
