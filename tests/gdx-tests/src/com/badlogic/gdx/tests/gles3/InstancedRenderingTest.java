...
public class InstancedRenderingTest extends GdxTest {
  ...
  private static final int INSTANCE_COUNT_SQRT = 100;
  private static final int INSTANCE_COUNT = INSTANCE_COUNT_SQRT * INSTANCE_COUNT_SQRT;
  ...
  public void create() {
    ...
    shader =
        new ShaderProgram(
            Gdx.files.internal("data/shaders/instanced-rendering.vert"),
    ...
    float[] vertices =
        new float[] {0.0f, 0.0f, size, 0.0f, 0.0f, size, size, 0.0f, size, size, 0.0f, size};
    ...
    mesh.enableInstancedRendering(
        true,
        INSTANCE_COUNT,
        new VertexAttribute(Usage.Position, 2, "i_offset"),
    ...
    for (int x = 1; x <= INSTANCE_COUNT_SQRT; x++) {
      for (int y = 1; y <= INSTANCE_COUNT_SQRT; y++) {
        offsets.put(
            new float[] {
              x / (INSTANCE_COUNT_SQRT * 0.5f) - 1f,
              y / (INSTANCE_COUNT_SQRT * 0.5f) - 1f,
              x / (float) INSTANCE_COUNT_SQRT,
              y / (float) INSTANCE_COUNT_SQRT,
              1f,
              1f
            });
      }
