...
public class AssetManagerTest extends GdxTest implements AssetErrorListener {
  ...
  public void create() {
    ...
    ResolutionFileResolver resolver =
        new ResolutionFileResolver(new InternalFileHandleResolver(), resolutions);
  ...
  private void load() {
    ...
    shader =
        new ShaderProgram(
            Gdx.files.internal("data/g2d/batchCommon.vert").readString(),
    ...
    manager.load(
        "data/i18n/message2",
        I18NBundle.class,
        new I18NBundleLoader.I18NBundleParameter(
            reloads % 2 == 0 ? Locale.ITALIAN : Locale.ENGLISH));
    manager.load(
        "data/g2d/monochrome.frag",
        ShaderProgram.class,
  ...
  public void render() {
    ...
    if (manager.isLoaded("data/pack.atlas"))
      batch.draw(
          manager.get("data/pack.atlas", TextureAtlas.class).findRegion("particle-star"), 164, 100);
    ...
    if (manager.isLoaded("data/multipagefont.fnt")) {
      manager
          .get("data/multipagefont.fnt", BitmapFont.class)
          .draw(batch, "This is a test qpRPN multi page!", 100, 80);
    }
    // System.out.println(Arrays.toString(manager.getAssetNames().items));

    // if (manager.isLoaded("data/test.etc1")) batch.draw(manager.get("data/test.etc1",
    // Texture.class), 0, 0);
    // if (manager.isLoaded("data/tiledmap/tilemap csv.tmx")) manager.get("data/tiledmap/tilemap
    // csv.tmx",
