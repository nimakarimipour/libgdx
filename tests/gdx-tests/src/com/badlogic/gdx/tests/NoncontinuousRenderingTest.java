...
public class NoncontinuousRenderingTest extends GdxTest {
  ...
  public void render() {
    ...
    value =
        value < 0.5f
            ? Interpolation.fade.apply(2 * value)
            : 1 - Interpolation.fade.apply(2 * value - 1);
    ...
    batch.draw(
        region,
        cam.position.x - texture.getWidth() / 2,
        cam.position.y - texture.getHeight() / 2,
        texture.getWidth() / 2f,
  ...
  private void populateTable() {
        ...
        new ChangeListener() {
          public void changed(ChangeEvent event, Actor actor) {
            final Graphics graphics =
                Gdx.graphics; // caching necessary to ensure call on this window
            new Thread(
                    new Runnable() {
                      public void run() {
                        try {
    ...
    actionsRequestRendering.addListener(
        new ChangeListener() {
          public void changed(ChangeEvent event, Actor actor) {
