...
public abstract class BaseShadowSystem implements ShadowSystem, Disposable {
  ...
  public static class PointLightProperties {
    public ObjectMap<CubemapSide, LightProperties> properties =
        new ObjectMap<CubemapSide, LightProperties>(6);
  }
  ...
  public BaseShadowSystem() {
    this(
        ...
        new FixedShadowMapAllocator(
            FixedShadowMapAllocator.QUALITY_MED, FixedShadowMapAllocator.QUANTITY_MAP_MED),
  ...
  @Override
  public <T extends RenderableProvider> void begin(
      Camera camera, final Iterable<T> renderableProviders) {
  ...
  protected LightProperties nextPoint() {
    ...
    if (currentPointProperties.properties.containsKey(
        Cubemap.CubemapSide.values()[currentPointSide])) {
      LightProperties lp =
          currentPointProperties.properties.get(Cubemap.CubemapSide.values()[currentPointSide]);
      currentLightProperties = lp;
      currentPointSide += 1;

      if (!lightFilter.filter(
          pointCameras.findKey(currentPointProperties, true), lp.camera, this.camera)) {
        return nextPoint();
      }
