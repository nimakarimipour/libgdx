...
public class VoxelWorld implements RenderableProvider {
  ...
  public VoxelWorld(TextureRegion[] tiles, int chunksX, int chunksY, int chunksZ) {
    ...
    this.vertices =
        new float[VoxelChunk.VERTEX_SIZE * 6 * CHUNK_SIZE_X * CHUNK_SIZE_Y * CHUNK_SIZE_Z];
  ...
  public void set(float x, float y, float z, byte voxel) {
    ...
    chunks[chunkX + chunkZ * chunksX + chunkY * chunksX * chunksZ].set(
        ix % CHUNK_SIZE_X, iy % CHUNK_SIZE_Y, iz % CHUNK_SIZE_Z, voxel);
  }
  ...
  public byte get(float x, float y, float z) {
    ...
    return chunks[chunkX + chunkZ * chunksX + chunkY * chunksX * chunksZ].get(
        ix % CHUNK_SIZE_X, iy % CHUNK_SIZE_Y, iz % CHUNK_SIZE_Z);
  }
  ...
  public void setCube(
      float x, float y, float z, float width, float height, float depth, byte voxel) {
    int ix = (int) x;
    int iy = (int) y;
    int iz = (int) z;
