...
/**
 * The test shows the effects of TextureAttribute.Emissive and ColorAttribute.Emissive on a models
 * Material.
 *
 * <p>Visually the test must show a rotating cube and background that have an orange glowing cross
 * over the "badlogic" image. The cross must be visible even when the cube side has rotated away
 * from the light.
 */
public class MaterialEmissiveTest extends GdxTest {
  ...
  public void create() {
    ...
    Material material =
        new Material(diffuseTextureAttribute, emissiveTextureAttribute, emissiveColorAttribute);

    ModelBuilder builder = new ModelBuilder();
    model =
        builder.createBox(
            1, 1, 1, material, Usage.Position | Usage.Normal | Usage.TextureCoordinates);
    ...
    MeshPartBuilder mpb =
        builder.part(
            "back", GL20.GL_TRIANGLES, Usage.Position | Usage.TextureCoordinates, material);
    ...
    environment.set(
        new ColorAttribute(
            ColorAttribute.AmbientLight, ambientLight, ambientLight, ambientLight, 1f));
