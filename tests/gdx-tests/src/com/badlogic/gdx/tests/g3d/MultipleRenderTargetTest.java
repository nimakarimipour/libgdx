...
/**
 * MRT test compliant with GLES 3.0, with per pixel lighting and normal and specular mapping. Thanks
 * to http://www.blendswap.com/blends/view/73922 for the cannon model, licensed under CC-BY-SA
 *
 * <p>/** @author Tomski
 */
...
public class MultipleRenderTargetTest extends GdxTest {
  ...
  public void create() {
    ...
    GLFrameBuffer.FrameBufferBuilder frameBufferBuilder =
        new GLFrameBuffer.FrameBufferBuilder(Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
    frameBufferBuilder.addColorTextureAttachment(
        GL30.GL_RGBA8, GL30.GL_RGBA, GL30.GL_UNSIGNED_BYTE);
    ...
    for (int i = 0; i < NUM_LIGHTS; i++) {
      ...
      light.position.set(
          MathUtils.random(-10f, 10f), MathUtils.random(10f, 15f), MathUtils.random(-10f, 10f));
  ...
  public void render() {
    ...
    mrtSceneShader.setUniformi(
        ...
        renderContext.textureBinder.bind(
            frameBuffer.getTextureAttachments().get(DIFFUSE_ATTACHMENT)));
    mrtSceneShader.setUniformi(
        "u_normalTexture",
        renderContext.textureBinder.bind(
            frameBuffer.getTextureAttachments().get(NORMAL_ATTACHMENT)));
    mrtSceneShader.setUniformi(
        "u_positionTexture",
        renderContext.textureBinder.bind(
            frameBuffer.getTextureAttachments().get(POSITION_ATTACHMENT)));
    mrtSceneShader.setUniformi(
        "u_depthTexture",
        renderContext.textureBinder.bind(
            frameBuffer.getTextureAttachments().get(DEPTH_ATTACHMENT)));
    ...
    public void render(Renderable renderable) {
      ...
      if (diffuseTexture != null) {
        shaderProgram.setUniformi(
            "u_diffuseTexture",
            context.textureBinder.bind(diffuseTexture.textureDescription.texture));
      }
      if (normalTexture != null) {
        shaderProgram.setUniformi(
            "u_normalTexture",
            context.textureBinder.bind(normalTexture.textureDescription.texture));
      }
      if (specTexture != null) {
        shaderProgram.setUniformi(
            "u_specularTexture",
            context.textureBinder.bind(specTexture.textureDescription.texture));
      }

      shaderProgram.setUniformMatrix("u_worldTrans", renderable.worldTransform);
      shaderProgram.setUniformMatrix(
          "u_normalMatrix", matrix3.set(renderable.worldTransform).inv().transpose());
