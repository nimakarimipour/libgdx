...
public class ShaderLoader {
  ...
  public ObjectMap<String, ObjectMap<String, String>> snippets =
      new ObjectMap<String, ObjectMap<String, String>>();
  ...
  protected void load(final StringBuilder out, final String name) {
    ...
    if (result == null)
      throw new GdxRuntimeException(
          "No snippet [" + snipName + "] in file " + root.child(fileName).path());
  ...
  protected ObjectMap<String, String> parse(final FileHandle file) {
    ...
    try {
      while ((line = reader.readLine()) != null) {
        ...
        } else stringBuilder.append(line.trim()).append("\r\n");
      }
    } catch (IOException e) {
