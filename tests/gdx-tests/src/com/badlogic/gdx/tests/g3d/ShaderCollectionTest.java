...
public class ShaderCollectionTest extends BaseG3dHudTest {
  /**
   * Desktop only: Set this to an absolute path to load the shader files from an alternative
   * location.
   */
  ...
  protected String environments[] =
      new String[] {"<none>", "debug", "environment_01", "environment_02"};
  ...
  public static class TestShaderProvider extends DefaultShaderProvider {
      ...
      try {
        ...
        if (tempFolder != null && Gdx.app.getType() == ApplicationType.Desktop)
          Gdx.files
              .absolute(tempFolder)
              .child(name + ".log.txt")
              .writeString(e.getMessage(), false);
        if (!revert()) {
          Gdx.app.error("ShaderCollectionTest", e.getMessage());
          throw new GdxRuntimeException(
              "Error creating shader, cannot revert to default shader", e);
        }
        error = true;
    ...
    protected Shader createShader(Renderable renderable) {
      if (config.vertexShader != null
        ...
        Gdx.files
            .absolute(tempFolder)
            .child(name + ".vertex.glsl")
            .writeString(prefix + config.vertexShader, false);
        Gdx.files
            .absolute(tempFolder)
            .child(name + ".fragment.glsl")
            .writeString(prefix + config.fragmentShader, false);
      }
      ...
      if (tempFolder != null && Gdx.app.getType() == ApplicationType.Desktop)
        Gdx.files
            .absolute(tempFolder)
            .child(name + ".log.txt")
            .writeString(result.program.getLog(), false);
      return result;
    }
  }
  ...
  protected ObjectMap<ModelInstance, AnimationController> animationControllers =
  ...
  public void create() {
    ...
    shaderRoot =
        (hotLoadFolder != null && Gdx.app.getType() == ApplicationType.Desktop)
            ? Gdx.files.absolute(hotLoadFolder)
  ...
  public void setEnvironment(String name) {
    ...
    if (name.equals("<none>")) {
      ...
      FacedCubemapData faces =
          new FacedCubemapData(
              root.child(name + "_PX.png"),
              root.child(name + "_NX.png"),
              root.child(name + "_PY.png"),
              root.child(name + "_NY.png"),
              root.child(name + "_PZ.png"),
              root.child(name + "_NZ.png"),
  ...
  protected void getStatus(StringBuilder stringBuilder) {
    ...
    if (shaderProvider.error) stringBuilder.append(" ERROR CREATING SHADER, REVERTED TO DEFAULT");
    else {
  ...
  protected void onLoaded() {
    ...
    if (loadingMaterial) {
      ...
      final ModelInstance instance =
          new ModelInstance(assets.get(currentModel, Model.class), transform);
      ...
      cam.position
          .set(1, 1, 1)
          .nor()
          .scl(bounds.getDimensions(tmpV).len() * 0.75f)
          .add(bounds.getCenter(tmpV));
      cam.up.set(0, 1, 0);
  ...
  protected void createHUD() {
    ...
    environmentsWindow =
        addListWindow("Environments", environmentsList, materialsWindow.getRight(), -1);
  }
