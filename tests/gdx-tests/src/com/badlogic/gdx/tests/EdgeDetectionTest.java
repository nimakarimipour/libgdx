...
public class EdgeDetectionTest extends GdxTest {
  ...
  float[] filter = {
    0, 0.25f, 0, 0.25f, -1f, 0.6f, 0, 0.25f, 0,
  };
  ...
  public void create() {
    ...
    batchShader =
        new ShaderProgram(
            Gdx.files.internal("data/shaders/batch.vert").readString(),
            Gdx.files.internal("data/shaders/convolution.frag").readString());
    if (!batchShader.isCompiled()) {
      Gdx.app.log(
          "EdgeDetectionTest", "couldn't compile post-processing shader: " + batchShader.getLog());
    }
